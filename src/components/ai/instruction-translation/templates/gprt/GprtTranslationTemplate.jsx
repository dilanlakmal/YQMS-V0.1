...
function GprtTranslationTemplate({ editable, step, production, setProduction, currentLanguage }) {
  const [activePage, setActivePage] = useState(1);
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const totalPages = 19;
  console.log("Rendering GprtTranslationTemplate with step:", step);
  if (step === "Preview") {
  ...
  return (
    <div className="flex h-full w-full bg-slate-100 relative">
      ...
      {/* Content Area */}
      <div className={`flex-1 overflow-y-auto bg-slate-50/50 relative transition-all duration-300 ${step === 'complete' ? 'p-0' : 'p-6 md:p-8 pt-12'}`}>
        <div className="max-w-5xl mx-auto" id="printable-content">
          {activePage === 1 ? (
            <GprtFirstPage
              production={production}
              setProduction={setProduction}
