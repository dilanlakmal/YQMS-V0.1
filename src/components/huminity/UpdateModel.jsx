...
export default function UpdateModel({ open, onCancel, report, onUpdate }) {
    ...
    const updateSectionData = (recordIndex, section, field, value) => {
        setFormData(prev => {
            ...
            // Body Status
            if (bodyStr.length >= 2 && !Number.isNaN(bodyVal) && !Number.isNaN(specNumBody)) {
            ...
            // Ribs Status
            if (ribsStr.length >= 2 && !Number.isNaN(ribsVal) && !Number.isNaN(specNumRibs)) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const payload = {
                ...formData,
                history: newHistory,
                updatedBy: user ? { empId: user.emp_id, engName: user.eng_name || user.name || user.username } : null,
            ...
            if (response.ok) {
                ...
                setMessage({ type: 'error', text: result.message || 'Failed to update report' });
            }
        } catch (err) {
            console.error(err);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4 transition-opacity">
            ...
            {message.text && (
                <div className={`fixed bottom-6 right-6 z-[200] flex items-center gap-4 p-4 pl-5 pr-6 rounded-2xl shadow-2xl border backdrop-blur-md transition-all duration-500 animate-in fade-in slide-in-from-right-8 ${message.type === 'success' ? 'text-emerald-900 bg-white/95 border-emerald-100 ring-8 ring-emerald-500/5' : 'text-rose-900 bg-white/95 border-rose-100 ring-8 ring-rose-500/5'
                    }`} role="alert">
                    <div className="relative">
                    ...
                    <div className="flex flex-col min-w-0 pr-4">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-0.5">Notification</span>
                        <div className="text-sm font-black tracking-tight leading-tight">{message.text}</div>
                    </div>
                    ...
                    {/* Tiny Progress Bar */}
                    <div className={`absolute bottom-0 left-0 h-1 rounded-full opacity-30 ${message.type === 'success' ? 'bg-emerald-500' : 'bg-rose-500'}`}
                        style={{ width: '100%', animation: 'shrink-width 3s linear forwards' }}
                    ></div>
                </div>
            )}
