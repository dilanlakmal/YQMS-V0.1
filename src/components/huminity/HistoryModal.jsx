...
import { MessageSquare, CheckCircle2, ShieldCheck, AlertCircle, Send } from 'lucide-react';
import { useAuth } from '../authentication/AuthContext';

const HistoryModal = ({ open, onCancel, report, formatDate, formatTime, onApprove }) => {
                ...
                <div className="bg-gradient-to-r from-green-50 to-green-50 border border-gray-200 border-b-0 px-8 py-6 relative overflow-hidden w-full shadow-lg">
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold text-green-500 mb-1 tracking-tight">Inspection History</h3>
                                <div className="flex items-center gap-2 text-green-500 text-sm font-medium">
                                    <span className="flex items-center gap-1.5 px-2.5 py-1 bg-white/40 border border-green-200/50 rounded-lg shadow-sm backdrop-blur-sm">{report.factoryStyleNo || 'N/A'}</span>
                                    <span className="opacity-100">•</span>
                                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/40 border border-green-200/50 rounded-lg shadow-sm backdrop-blur-sm">{report.buyerStyle || 'N/A'}</div>
                                    ...
                                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/40 border border-green-200/50 rounded-lg shadow-sm backdrop-blur-sm">
                                        <div className="text-[12px] uppercase text-green-500 tracking-wider">Spec (Body):</div>
                                    ...
                                    {ribsVisible && (
                                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-white/40 border border-green-200/50 rounded-lg shadow-sm backdrop-blur-sm">
                                            <div className="text-[12px] uppercase text-green-500 tracking-wider">Spec (Ribs):</div>
                                            <div className="text-green-700 text-[12px]">{report.aquaboySpecRibs ? `${report.aquaboySpecRibs}%` : 'N/A'}</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24" stroke="none">
                                    <path d="M12 2a2 2 0 00-2 2v6H6l6 6 6-6h-4V4a2 2 0 00-2-2z" />
                                    <path d="M6 18v2a2 2 0 002 2h8a2 2 0 002-2v-2" fill="#fff" opacity="0.15" />
                                </svg>
                                Export PDF
                            </button>
                            <button
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-gray-100/80">
                                    <th className="px-4 py-3 text-center text-sm font-bold text-gray-800 uppercase tracking-widest border-b border-gray-200" rowSpan={2}>Nº</th>
                                    ...
                                    <th className="px-4 py-3 text-center text-sm font-bold text-gray-800 uppercase tracking-widest border-b border-l border-gray-200" rowSpan={2}>Customer</th>
                                    <th className="px-4 py-3 text-center text-sm font-bold text-gray-800 uppercase tracking-widest border-b border-l border-gray-200" rowSpan={2}>Fabrication</th>
                                    <th className="px-4 py-3 text-center text-sm font-bold text-gray-800 uppercase tracking-widest border-b border-l border-gray-200" rowSpan={2}>Color</th>
                            ...
                            <tbody className="divide-y divide-gray-100 border-b">
                                {history.map((check, checkIdx) => (
                                    <tr key={checkIdx} className="hover:bg-green-50/50 transition-colors group border-b">
                                        <td className="px-4 py-3.5 text-center font-bold text-gray-400 group-hover:text-green-600 transition-colors">{checkIdx + 1}</td>
                                        ...
                                        <td className="px-4 py-3.5 text-center text-gray-600 border-l border-gray-50 font-medium">
                                            {check.colorName || check.color || report.colorName || 'N/A'}
                                        </td>
                        ...
                        {history.length > 0 && history[history.length - 1]?.generalRemark && (
                            <div className="p-4 bg-slate-50/50 border border-slate-200 rounded-xl shadow-sm flex flex-col h-full transition-all hover:bg-slate-100/50 hover:shadow-md">
                                <div className="flex items-center gap-3 mb-4 text-slate-600">
                                    ...
                                    <h3 className="text-sm font-black uppercase tracking-widest m-0">Latest Remark</h3>
                                </div>
                        ...
                        {report.approvalStatus === 'approved' ? (
                            <div className="p-5 bg-emerald-50/50 border border-emerald-200 rounded-2xl shadow-sm backdrop-blur-sm flex flex-col h-full transition-all hover:shadow-md group/approve">
                                <div className="flex items-center justify-between mb-5">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-emerald-800 m-0">Report Approved</h3>
                                    </div>
                                    <div className="text-[10px] font-black text-emerald-500 bg-emerald-100/50 px-2 py-1 rounded-md border border-emerald-200/50 uppercase">Official Seal</div>
                                </div>
                                ...
                                <div className="bg-white border border-emerald-100 rounded-2xl p-5 flex flex-col gap-4 flex-grow shadow-sm relative overflow-hidden group/card shadow-inner-white">
                                    ...
                                    <div className="flex items-center gap-4 relative z-10">
                                        <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center border-2 border-emerald-100 shrink-0 select-none">
                                            <span className="text-lg font-black text-emerald-600">
                                                {(report.approvedBy?.engName || report.approvedBy?.empId || 'A').charAt(0).toUpperCase()}
                                            </span>
                                        </div>
                                        <div className="flex flex-col min-w-0">
                                            <span className="text-sm font-black text-slate-800 truncate tracking-tight">{report.approvedBy?.engName || report.approvedBy?.empId}</span>
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                                ...
                                                <span className="text-[10px] text-emerald-500 font-black uppercase tracking-widest leading-none">Verified</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {report.approvedRemark ? (
                                        <div className="relative z-10 text-[13px] text-slate-600 leading-relaxed bg-emerald-50/30 p-3 rounded-xl border border-emerald-100/50 italic">
                                            <span className="text-emerald-300 mr-2 font-serif text-lg leading-none">"</span>
                                            {report.approvedRemark}
                                            <span className="text-emerald-300 ml-2 font-serif text-lg leading-none">"</span>
                                        </div>
                                    ) : (
                                        <div className="text-[11px] text-slate-400 italic">No approval remarks provided.</div>
                                    )}
                                </div>
                            </div>
                        ) : (
                            <div className="p-5 bg-blue-50/50 border border-blue-200 rounded-2xl shadow-sm backdrop-blur-sm flex flex-col h-full transition-all hover:bg-blue-50/80 group/pending">
                                <div className="flex items-center gap-3 mb-5">
                                    ...
                                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-blue-800 m-0">Pending Approval</h3>
                                </div>
                                ...
                                <div className="bg-white border border-blue-100 rounded-2xl p-5 flex flex-col gap-4 flex-grow shadow-sm">
                                    ...
                                    {(() => {
                                        ...
                                        const isAuthorized = currentUser?.emp_id && AUTHORIZED_APPROVERS.some(id =>
                                            id.toLowerCase() === String(currentUser.emp_id).trim().toLowerCase()
                                        );
                                        ...
                                        if (isAuthorized) {
                                            return (
                                                <div className="mt-auto space-y-3">
                                                    ...
                                                    <button
                                                        onClick={() => onApprove(report._id, approvalRemark)}
