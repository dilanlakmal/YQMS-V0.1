...
export default function UpdateModelReimans({ open, onCancel, report, onUpdate }) {
            ...
            const safeRecords = sourceRecords.map(rec => {
                const processSection = (sec) => {
                    ...
                    return {
                        body,
                        ribs: '',
                        pass: !Number.isNaN(bodyVal) && !Number.isNaN(specLimit) ? bodyVal <= specLimit : (sec?.pass === true || sec?.status === 'pass'),
                        fail: !Number.isNaN(bodyVal) && !Number.isNaN(specLimit) ? bodyVal > specLimit : (sec?.fail === true || sec?.status === 'fail')
                    };
                };
    ...
    const updateSectionData = (recordIndex, section, field, value) => {
        setFormData(prev => {
            ...
            if (!Number.isNaN(bodyVal) && bodyStr.length >= 2 && !Number.isNaN(specNum)) {
                if (bodyVal <= specNum) {
            ...
            if (recordIndex === newRecords.length - 1) {
                ...
                if (maxReading > 0) {
                    return { ...prev, inspectionRecords: newRecords, moistureRateAfter: maxReading.toString() };
                }
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const currentEditRecords = formData.inspectionRecords.map(rec => ({
                top: { body: rec.top.body, ribs: '', status: rec.top.pass ? 'pass' : (rec.top.fail ? 'fail' : '') },
                middle: { body: rec.middle.body, ribs: '', status: rec.middle.pass ? 'pass' : (rec.middle.fail ? 'fail' : '') },
                bottom: { body: rec.bottom.body, ribs: '', status: rec.bottom.pass ? 'pass' : (rec.bottom.fail ? 'fail' : '') },
                images: rec.images || [],
                ...
                saveTime: new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })
            }));
            ...
            if (response.ok) {
                Swal.fire({ icon: 'success', title: 'Success', text: 'Reitmans Report updated successfully!', timer: 2000, showConfirmButton: false });
                onUpdate();
                onCancel();
            } else {
                Swal.fire({ icon: 'error', title: 'Error', text: result.message || 'Failed to update report' });
            }
        } catch (err) {
            console.error(err);
            Swal.fire({ icon: 'error', title: 'Error', text: 'Error updating report' });
        } finally {
            setIsSaving(false);
        }
    };
            ...
            >
                <div className="p-6 border-b flex justify-between items-center bg-white z-10 shrink-0">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-2xl font-bold text-blue-500">Edit Reitmans Inspection</h2>
                    </div>
                    ...
                    <form id="update-form" onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                            <div>
                                ...
                                <input type="text" value={formData.factoryStyleNo} readOnly className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 bg-gray-50" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Buyer Style</label>
                                <input type="text" value={formData.buyerStyle} readOnly className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 bg-gray-50" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Customer</label>
                                <input type="text" value={formData.customer} readOnly className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 bg-gray-50" />
                            </div>
                            <div ref={colorRef}>
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        <svg className={`fill-current h-4 w-4 transition-transform ${showColorDropdown ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                    ...
                                    {showColorDropdown && (
                                        <div className="absolute z-[60] w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-y-auto min-w-[200px]">
                                            {availableColors.length > 0 ? (
                                                ...
                                                <div className="px-4 py-2 text-xs text-gray-400 italic">No suggested colors for this style.</div>
                                            )}
                                        </div>
                                    )}
                                </div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">PO Line</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Upper Centisimal Index (Spec)</label>
                                <input
                                    type="text"
                                    value={formData.upperCentisimalIndex}
                                    onChange={e => setFormData({ ...formData, upperCentisimalIndex: e.target.value })}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Time Checked</label>
                                <input type="time" value={formData.timeChecked} onChange={e => setFormData({ ...formData, timeChecked: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Moisture Rate (Before)</label>
                                <input type="number" value={formData.moistureRateBeforeDehumidify} onChange={e => setFormData({ ...formData, moistureRateBeforeDehumidify: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">No. Pcs Checked</label>
                                <input type="number" value={formData.noPcChecked} onChange={e => setFormData({ ...formData, noPcChecked: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Time In</label>
                                <input type="time" value={formData.timeIn} onChange={e => setFormData({ ...formData, timeIn: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Time Out</label>
                                <input type="time" value={formData.timeOut} onChange={e => setFormData({ ...formData, timeOut: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Moisture Rate (After)</label>
                                <input type="number" value={formData.moistureRateAfter} onChange={e => setFormData({ ...formData, moistureRateAfter: e.target.value })} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Date</label>
                                <input type="date" value={formData.date} disabled className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2 bg-gray-50" />
                            </div>
                                ...
                                {formData.inspectionRecords.map((record, index) => {
                                    ...
                                    return (
                                        <div key={index} className={`border rounded-lg p-4 ${isPassed ? 'bg-green-50 border-green-200' : 'bg-gray-50'}`}>
                                            <div className="flex justify-between items-center cursor-pointer mb-2" onClick={() => setExpandedRecordIndex(index)}>
                                            ...
                                            {isExpanded && (
                                                <div className="space-y-4">
                                                    {['top', 'middle', 'bottom'].map(section => (
                                                        <div key={section} className="flex flex-col md:flex-row gap-4 w-full items-center p-3 rounded-xl shadow-sm bg-white border border-gray-100 mb-2">
                                                ...
                                                <div className="space-y-3">
                                                    <label className="flex justify-center w-full h-32 px-4 transition bg-white border-2 border-gray-300 border-dashed rounded-md appearance-none cursor-pointer hover:border-blue-400 focus:outline-none hover:bg-gray-50">
                                                        <span className="flex items-center space-x-2">
                                                            ...
                                                            <span className="font-medium text-gray-600">Upload images</span>
                                                        </span>
                                                        ...
                                                        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2 mt-2">
                                                            {record.images.map((img, imgIdx) => (
                                                                <div key={img.id || imgIdx} className="relative group aspect-square">
                                                                    <img src={img.preview} alt={img.name} className="h-full w-full object-cover rounded border" />
                                                                    <button type="button" onClick={() => removeImage(index, img.id)} className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full p-0.5 shadow hover:bg-red-600">
                        ...
                        <div className="pt-4 flex justify-end gap-3 sticky bottom-0 bg-white border-t p-4 -mx-6 -mb-6">
                            <button type="button" onClick={onCancel} className="px-6 py-2 border rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
