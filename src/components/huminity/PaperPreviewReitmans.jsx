...
const PaperPreviewReitmans = ({ data }) => {
    ...
    const getMoistureBefore = () => {
        if (formData.moistureRateBeforeDehumidify) return formData.moistureRateBeforeDehumidify;
        ...
        <div className="reitmans-preview-container p-4 font-sans text-sm bg-white text-black">
            ...
            <div className="flex justify-between items-start mb-2 px-2">
                <div className="flex flex-col">
                    <h1 className="text-2xl font-black tracking-tighter leading-none">RCL ASIA</h1>
                    <span className="text-[12px] font-bold tracking-widest mt-1 pt-1">LIMITED</span>
                </div>
                <div className="text-center flex-1 mx-10 flex flex-col items-center">
                    <h2 className="text-2xl font-bold inline-block px-10 mb-1">大貨抽濕記錄表</h2>
                </div>
                <div className="flex items-start gap-4">
                    <div className="text-right flex flex-col">
                        <span className="text-2xl font-bold">
                            {formData.date ? new Date(formData.date).toLocaleDateString('en-US', { month: 'short', year: 'numeric' }).replace(' ', ' , ') : new Date().toLocaleDateString('en-US', { month: 'short', year: 'numeric' }).replace(' ', ' , ')}
                        </span>
                        <span className="text-[12px] font-bold mt-1">Style#: {formData.moNo || formData.factoryStyleNo || 'N/A'}</span>
                    </div>
            ...
            <table className="reitmans-table w-full border-collapse">
                <thead>
                    <tr>
                        ...
                        <th className="w-[7%] text-[14px] font-bold leading-tight">Time Checked</th>
                        <th className="w-[6%] text-[14px] font-bold leading-tight">No.pc checked</th>
                        <th className="w-[11%] text-[14px] font-bold leading-tight">Moisture rate before</th>
                        <th className="w-[6%] text-[14px] font-bold">Time In</th>
                        <th className="w-[6%] text-[14px] font-bold">Time out</th>
                        <th className="w-[7%] text-[14px] font-bold leading-tight">Moisture rate after</th>
                        <th className="w-[8%] text-[14px] leading-tight font-bold">Upper Centisimal Index</th>
                ...
                <tbody>
                    {history.map((rec, idx) => {
                        ...
                        const moistureBefore = rowMax ? `${rowMax}%` : (formData.moistureRateBeforeDehumidify ? `${formData.moistureRateBeforeDehumidify}%` : 'N/A');

                        return (
                            <tr key={idx} className="h-12 text-[15px] font-medium">
                                <td>{formatDate(rec.date || formData.date)}</td>
                                <td className="font-bold">{formData.buyerStyle || formData.factoryStyleNo || 'N/A'}</td>
                                <td>{formData.colorName || 'N/A'}</td>
                                <td>{formData.poLine || 'N/A'}</td>
                                <td className="text-[14px] leading-tight px-1">{formData.composition || 'N/A'}</td>
                                <td>{formatTime(rec.saveTime || formData.timeChecked)}</td>
                                <td className="font-bold text-[16px]">{formData.noPcChecked || 'N/A'}</td>
                                <td className="font-bold text-gray-700">
                                    {moistureBefore}
                                </td>
                                <td>{formatTime(formData.timeIn)}</td>
                                <td>{formatTime(formData.timeOut)}</td>
                                <td className="font-bold text-blue-800 text-[16px]">{formData.moistureRateAfter || 'N/A'}%</td>
                                <td className="font-black text-rose-700 text-[16px]">{formData.upperCentisimalIndex || 'N/A'}%</td>
                                <td className="relative p-0">
                                    {formData.inspectorSignature && (
                                        <img src={formData.inspectorSignature} className="max-h-10 mx-auto grayscale" alt="sig" />
                                    )}
                                </td>
                            </tr>
                        );
            ...
            <div className="mt-4">
                ...
                <div className="border border-black p-4 rounded flex flex-col w-full">
                    <h4 className="font-bold text-[12px] uppercase border-b border-black mb-2 p-1 bg-gray-50">General Remarks</h4>
                    ...
                    {formData.qamSignature && (
                        <div className="mt-2 border-t border-gray-200 pt-2 text-right">
                            ...
                            <img src={formData.qamSignature} className="max-h-12 ml-auto grayscale opacity-80" alt="supervisor" />
                        </div>
                    )}
                </div>
            </div>
