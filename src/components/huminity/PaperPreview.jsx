        ...
        <div className="mx-6 my-4 p-6 bg-white border rounded shadow-sm print:shadow-none print:border-none print:m-0 print:p-0 page-break-after-always">
            ...
            <div className="flex justify-between mb-3 text-sm">
                <div className="w-1/2">
                    ...
                    <div><span className="font-bold">Aquaboy spec (Body):</span> {formData.aquaboySpecBody || formData.aquaboySpec}</div>
                </div>
                            ...
                            return (
                                <tr key={idx}>
                                    ...
                                    {['top', 'middle', 'bottom'].map(section => {
                                        ...
                                        const bodyStatus = getSpecStatus(s.body, formData.aquaboySpecBody || formData.aquaboySpec, s.bodyStatus || (s.bodyPass ? 'pass' : (s.bodyFail ? 'fail' : '')));
                                        const ribsStatus = getSpecStatus(s.ribs, formData.aquaboySpecRibs || formData.aquaboySpec, s.ribsStatus || (s.ribsPass ? 'pass' : (s.ribsFail ? 'fail' : '')));

                                        // Section status
                                        let sectionStatus = s.status || (s.pass ? 'pass' : (s.fail ? 'fail' : ''));
                                        if (!sectionStatus && (bodyStatus || ribsStatus)) {
                                            if (bodyStatus === 'fail' || ribsStatus === 'fail') sectionStatus = 'fail';
                                            else if (bodyStatus === 'pass' && (ribsVisible ? ribsStatus === 'pass' : true)) sectionStatus = 'pass';
                                        }
                                        ...
                                        return (
                                            <React.Fragment key={section}>
                                                <td className="border border-black px-2 py-2 text-center">
                                                    {s.body || ''}
                                                    {bodyStatus && (
                                                        <div className={`text-[8px] font-bold uppercase ${bodyStatus === 'fail' ? 'text-red-600' : 'text-green-600'}`}>
                                                            {bodyStatus}
                                                        </div>
                                                    )}
                                                </td>
                                                {ribsVisible && (
                                                    <td className="border border-black px-2 py-2 text-center">
                                                        {s.ribs || ''}
                                                        {ribsStatus && (
                                                            <div className={`text-[8px] font-bold uppercase ${ribsStatus === 'fail' ? 'text-red-600' : 'text-green-600'}`}>
                                                                {ribsStatus}
                                                            </div>
                                                        )}
                                                    </td>
                                                )}
                                                <td className={`border border-black px-2 py-2 text-center ${sectionStatus === 'fail' ? 'font-bold text-red-600' : (sectionStatus === 'pass' ? 'font-bold text-green-600' : '')}`}>
                                                    {sectionStatus === 'fail' ? 'Fail' : (sectionStatus === 'pass' ? 'Pass' : '')}
                                                </td>
                                            </React.Fragment>
                                        );
                                    })}
            ...
            <div className="mt-2 flex justify-end px-16 gap-8">
                <div className="flex flex-col items-center">
                    ...
                    {formData.inspectorSignature && (
                        <div className="mt-2 text-center">
                            <img src={formData.inspectorSignature} className="max-h-12 grayscale opacity-80" alt="inspector" />
                        </div>
                    )}
                ...
                <div className="flex flex-col items-center">
                    <div className="w-64 text-center p-6 font-bold pb-2">QAM / Supervisor</div>
                    {formData.approvalStatus === 'approved' && formData.approvedBy && (
                        <div className="mt-2 text-center">
                            ...
                            <div className="text-xs text-gray-600 mt-1">
                                by {formData.approvedBy.engName || formData.approvedBy.empId} {formData.approvedAt ? `on ${formatDate(formData.approvedAt)}` : ''}
                            </div>
