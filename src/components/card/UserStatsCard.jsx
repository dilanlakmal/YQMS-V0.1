...
const UserStatsCard = ({
  ...
  const userFacePhotoUrl = useMemo(() => {
    if (user && user.face_photo && apiBaseUrl) {
      try {
        ...
        console.error(`Error constructing face photo URL for '${user.face_photo}' with base '${apiBaseUrl}':`, e);
        return null;
      }
    }
    return null;
  ...
  const displayableTasks = useMemo(() => {
    return stats.tasks ? stats.tasks.filter(task => task.value !== undefined && task.value > 0) : [];
  }, [stats.tasks]);

  return (
     <div className={`bg-white shadow-lg rounded-xl p-3 sm:p-4 grid grid-cols-3 gap-2 sm:gap-3 items-start ${className}`}>
      {/* Column 1: User Info */}
      <div className="flex flex-col items-center text-center space-y-1 border-r border-gray-200 pr-2 sm:pr-3">
          ...
          <img
            ...
            alt={`${user.emp_id || 'User'}'s photo`}
             className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border-2 border-indigo-300 shadow-sm"
              onError={() => {
              console.error("Error loading user face photo. URL attempted:", userFacePhotoUrl);
        ...
        <p className="text-sm sm:text-base font-semibold text-indigo-700">{user.emp_id}</p>
      </div>
      ...
      <div className="flex flex-col items-center text-center space-y-1 h-full justify-center border-r border-gray-200 pr-2 sm:pr-3">
        ...
        {stats.totalUnit && <p className="text-xs text-gray-500">{stats.totalUnit}</p>}
      </div>

      {/* Column 3: Task No Related Count */}
      <div className="flex flex-col items-center text-center space-y-1 h-full justify-center">
        <p className="text-xs sm:text-sm text-gray-500 font-medium">By Task (Today)</p>
        <div className="space-y-0.5">
          {displayableTasks.length > 0 ? (
            displayableTasks.map((task, index) => (
              task.label && ( // Ensure task.label exists
                <p key={index} className="text-xs sm:text-sm text-gray-700 leading-tight">
                 {formatTaskLabelForDisplay(task.label)}:
                  <span className={`font-bold ${getTaskColor(task.label)} ml-1`}>
                    {task.value}
                  </span>
                </p>
              )
