...
const UserStatsCard = ({ user, apiBaseUrl, stats, className = "" }) => {
  ...
  return (
    <div
      className={`group relative overflow-hidden bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl rounded-xl border dark:border-gray-700 p-4 sm:p-6 grid grid-cols-3 gap-4 sm:gap-6 items-start transition-all duration-300 hover:scale-[1.02] ${className}`}
    >
                ...
                (task, index) =>
                  task.label && ( // Ensure task.label exists
                    ...
                    >
                      ...
                      <span
                        className={`text-sm sm:text-base font-bold ${getTaskColor(
                          task.label
                        )} px-2 py-1 rounded-md bg-white dark:bg-gray-800 shadow-sm`}
                      >
