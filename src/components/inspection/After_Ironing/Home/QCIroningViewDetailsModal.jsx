...
const QCIroningViewDetailsModal = ({
  isOpen,
  onClose,
  itemData,
  allRecords = []
  ...
  const calculateWashQuantities = () => {
    ...
    const matchingRecords = allRecords.filter((record) => {
      const matches =
        record.orderNo === itemData.orderNo &&
    ...
    if (currentRecordIndex === -1) {
      currentRecordIndex = sortedRecords.findIndex((record) => {
        ...
        const dateB = new Date(
          itemData.createdAt || itemData.date || itemData.submittedAt
        );
  ...
  const measurementSummary =
    itemData.measurementDetails?.measurementSizeSummary?.[0] || {};
                  ...
                  {selectedRecords.map((record, index) => {
                    ...
                    const isCurrentRecord =
                      (typeof record._id === "string" &&
                        typeof itemData._id === "string" &&
                        record._id === itemData._id) ||
                      (record._id?.$oid &&
                        itemData._id?.$oid &&
                        record._id.$oid === itemData._id.$oid) ||
                      (record._id?.$oid &&
                        typeof itemData._id === "string" &&
                        record._id.$oid === itemData._id) ||
                      (typeof record._id === "string" &&
                        itemData._id?.$oid &&
                        record._id === itemData._id.$oid);
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div className="text-center">
                  ...
                  <p className="font-semibold text-lg dark:text-white">
                    {washQuantityData.currentRecordPosition} /{" "}
                    {washQuantityData.totalRecords}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    ...
                    <p className="text-2xl font-bold text-green-900 dark:text-green-100">
                      {washQuantityData.alreadyWashedQty}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-4">
              <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-2">
                <span>
                  Progress: {washQuantityData.alreadyWashedQty} /{" "}
                  {itemData.colorOrderQty}
                </span>
                <span>
                  {(
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                                  {machine.temperature && (
                                    <div className="flex items-center">
                                      ...
                                      ></div>
                                      <span>
                                        Temp: {machine.temperature.actualValue}
                                        °C
                                      </span>
                                    </div>
                                  )}
                                  {machine.time && (
                                    <div className="flex items-center">
                                      ...
                                      ></div>
                                      <span>
                                        Time: {machine.time.actualValue}min
                                      </span>
                                    </div>
                                  )}
                                  {machine.timeCool &&
                                  ...
                                  {machine.silicon &&
                                    machine.silicon.actualValue && (
                                      <div className="flex items-center">
                                        ...
                                        ></div>
                                        <span>
                                          Silicon: {machine.silicon.actualValue}
                                          g
                                        </span>
                                      </div>
                                    )}
                                  {machine.softener &&
                                    machine.softener.actualValue && (
                                      <div className="flex items-center">
                                        ...
                                        <span>
                                          Softener:{" "}
                                          {machine.softener.actualValue}g
                                        </span>
                                      </div>
                                    )}
                                </div>
                ...
                return (
                  <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                      ...
                      <tr>
                        {sortedValues.map((value, idx) => (
                          <th
                            key={idx}
                            className="px-2 py-2 text-center text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider border-l border-gray-200 dark:border-gray-600"
                          >
                            {value}
                          </th>
                        ))}
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {Object.values(measurementPointSummary).map(
                        (point, index) => {
                            ...
                            >
                              ...
                              {sortedValues.map((value, idx) => {
                                ...
                                return (
                                  <td
                                    key={idx}
                                    className="px-2 py-3 text-center text-sm border-l border-gray-200 dark:border-gray-600"
                                  >
                                    {valueData ? (
                                      <span
