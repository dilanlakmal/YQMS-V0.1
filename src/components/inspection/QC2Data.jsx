...
const QC2Data = () => {
  ...
  const fetchDataCards = async (page, limit, filters = {}) => {
    try {
      ...
      setTotalRecords(
        Number.isInteger(responseData.total) ? responseData.total : 0
      );
    } catch (error) {
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      setEmpIdOptions(
        Array.isArray(data.emp_id_inspection) ? data.emp_id_inspection : []
      );
  ...
  const handleResetFilters = () => {
    ...
    setSearchPackageNo("");
    if (user?.emp_id) {
      // Reset to default logged-in user ID
      ...
      >
        ...
        {showFilters && (
          <form
            ...
            className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4 mb-1 transition-all duration-300 ease-in-out"
          >
            <div className="flex flex-col">
              <label
                htmlFor="filterDateQC2"
