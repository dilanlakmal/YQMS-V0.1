    ...
    <div className="p-2 sm:p-3 lg:p-3 bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 min-h-screen">
      <div className="max-w-screen-2xl mx-auto">
        <h1 className="text-lg font-bold mb-2">
          ANF Measurement - Buyer Style View
        </h1>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                {isLoading ? (
                  ...
                  paginatedData.map((item, index) => {
                    ...
                    const orderQty = item.orderQty_style || 0;

                    // --- START: MODIFIED CALCULATIONS FOR BUYER VIEW ---

                    // 1. Garment Details Recalculation
                    const newCheckedQty = orderQty;
                    const rejectedGarment =
                      originalSummary.rejectedGarment || 0;
                    const newOkGarment = newCheckedQty - rejectedGarment;

                    // 2. Measurement Details Recalculation
                    const originalCheckedQty = originalSummary.checkedQty || 0;
                    const originalTotalPoints =
                      originalSummary.totalPoints || 0;
                    const issuePoints = originalSummary.issuePoints || 0;
                    const tolPlus = originalSummary.tolPlus || 0;
                    const tolNeg = originalSummary.tolNeg || 0;

                    const newTotalPoints =
                      originalCheckedQty > 0
                        ? Math.round(
                            (originalTotalPoints / originalCheckedQty) *
                              newCheckedQty
                          )
                        : 0;

                    const newPassPoints = newTotalPoints - issuePoints;

                    // 3. Pass Percentage Recalculation
                    const passRateGarment =
                      newCheckedQty > 0
                        ? ((newOkGarment / newCheckedQty) * 100).toFixed(2) +
                          "%"
                        : "N/A";
...
export default ANFBuyerStyleView;
