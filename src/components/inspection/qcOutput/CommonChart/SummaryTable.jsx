  ...
  const tableData = useMemo(() => {
    ...
    data.forEach((day) => {
      (day[sourceKey] || []).forEach((item) => {
        if (
          ...
          items.push({
            ...item,
            date: isWeekly ? day.startDate : day.inspectionDate.split("T")[0],
            weekNo: isWeekly ? day.weekNo : null,
            weekLabel: isWeekly ? day.weekLabel : null,
            startDate: isWeekly ? day.startDate : null,
            endDate: isWeekly ? day.endDate : null,
            checkedQty,
            defectRate
          });
        }
    ...
    const sortedItems = [...items].sort((a, b) => {
      ...
      switch (sortConfig.type) {
        ...
        case "week": // ADD THIS CASE
          return order * (a.weekNo - b.weekNo);
        ...
        case "week-line": // ADD THIS CASE
          const weekCompare = a.weekNo - b.weekNo;
          if (weekCompare !== 0) return order * weekCompare;
          const lineA2 = parseInt(a.lineNo) || 0;
          const lineB2 = parseInt(b.lineNo) || 0;
          return order * (lineA2 - lineB2);
        ...
        case "week-mo": // ADD THIS CASE
          const weekCompareMO = a.weekNo - b.weekNo;
          if (weekCompareMO !== 0) return order * weekCompareMO;
          return order * (a.MONo || "").localeCompare(b.MONo || "");
    ...
    return sortedItems;
  }, [data, activeView, filters, sortConfig, isWeekly]);
  ...
  const headers = useMemo(() => {
    const baseHeaders = {
      ...
      Line: isWeekly
        ...
        : ["Date", "Line", "Checked", "Inside", "Outside", "Defects", "Rate"],
      MO: isWeekly
        ? [
            "Week",
            "Start Date",
        ...
        : ["Date", "MO", "Checked", "Inside", "Outside", "Defects", "Rate"],
      Buyer: isWeekly
        ? [
            "Week",
            "Start Date",
        ...
        : ["Date", "Buyer", "Checked", "Inside", "Outside", "Defects", "Rate"]
    };
    ...
    return baseHeaders;
  }, [showDetails, isWeekly]);

  // Determine which sort buttons to show based on active view
  const getAvailableSorts = () => {
    const commonSorts = isWeekly ? ["week", "defect"] : ["date", "defect"];

    if (activeView.includes("Line")) {
      return isWeekly
        ? ["week", "week-line", "defect"]
        : ["date", "date-line", "defect"];
    }
    if (activeView.includes("MO")) {
      return isWeekly
        ? ["week", "week-mo", "defect"]
    ...
    <div className="rounded-2xl bg-white dark:bg-gray-800 shadow-xl overflow-hidden">
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6">
        <div className="flex flex-wrap justify-between items-center gap-4">
          <div className="flex items-center gap-3">
            <BarChart3 className="w-6 h-6 text-white" />
            {isWeekly ? "Weekly Summary View" : "Daily Summary View"}
          </div>
