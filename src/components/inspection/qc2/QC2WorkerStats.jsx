...
const StatCard = ({ icon, label, value, color, trend, subtitle }) => (
  <div
    className={`group relative overflow-hidden bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border-l-4 ${color} transform hover:scale-105`}
  >
        ...
        <div className="flex items-center space-x-3">
          ...
          >
            {React.cloneElement(icon, {
              className: `w-6 h-6 ${color.replace("border-l-", "text-")}`
            })}
          </div>
...
const PerformanceBadge = ({ stats }) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      <div
        className={`px-3 py-1 rounded-full text-xs font-bold text-white ${getBadgeColor()}`}
      >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-semibold text-green-600 dark:text-green-400">
                  {stats?.totalCheckedQty > 0
                    ? (
                        (stats?.totalQtyTask54 / stats?.totalCheckedQty) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
              ...
              <div>
                ...
                <span className="ml-2 font-semibold text-red-600 dark:text-red-400">
                  {stats?.totalCheckedQty > 0
                    ? (
                        (stats?.totalQtyTask84 / stats?.totalCheckedQty) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
