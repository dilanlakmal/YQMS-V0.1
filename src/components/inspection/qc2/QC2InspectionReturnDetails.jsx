...
const QC2InspectionReturnDetails = ({
  ...
  const getTranslatedDefectName = (defectName) => {
    ...
    return defectEntry
      ? defectEntry[localLanguage] || defectName
      : defectName;
  };
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-8">
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-white/60 font-medium uppercase tracking-wide">MO No</p>
                  <p className="text-lg font-bold text-white">{defectTrackingDetails.moNo}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-white/60 font-medium uppercase tracking-wide">Line No</p>
                  <p className="text-lg font-bold text-white">{defectTrackingDetails.lineNo}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-white/60 font-medium uppercase tracking-wide">Color</p>
                  <p className="text-lg font-bold text-white">{defectTrackingDetails.color}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-4 shadow-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-white/60 font-medium uppercase tracking-wide">Size</p>
                  <p className="text-lg font-bold text-white">{defectTrackingDetails.size}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          {loadingBGrade && (
            <div className="flex items-center gap-2 bg-blue-50 border border-blue-200 rounded-xl px-4 py-2 shadow-sm">
              ...
              <span className="text-sm font-medium text-blue-700">Loading B-Grade status...</span>
            </div>
          )}
          ...
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-gray-500">
                {defectTrackingDetails.garments?.reduce((total, garment) => 
                  total + garment.defects.filter(d => d.pass_bundle !== "Pass").length, 0
                )} defects found
              </div>
            </div>
          </div>
                      ...
                      .map((defect, index) => {
                        ...
                        return (
                          <tr
                            ...
                            className={`transition-all duration-300 hover:shadow-md ${getEffectiveRowClassName(garment, defect)}`}
                          >
                    ...
                    <td colSpan={5} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-500">All garments have passed inspection</p>
                        </div>
                      </div>
                    </td>
                  </tr>
