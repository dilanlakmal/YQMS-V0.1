  ...
  try {
    ...
    if (warehouseEditContext) {
      formDataToSubmit.append(
        "editedByWarehouse",
        warehouseEditContext.editedByWarehouse === true ? "true" : "false",
      );
      if (warehouseEditContext.editorUserId != null)
        formDataToSubmit.append(
          "editorUserId",
          String(warehouseEditContext.editorUserId),
        );
      if (warehouseEditContext.editorEmpId != null)
        formDataToSubmit.append(
          "editorEmpId",
          String(warehouseEditContext.editorEmpId),
        );
      if (warehouseEditContext.editorUserName != null)
        formDataToSubmit.append(
          "editorUserName",
          String(warehouseEditContext.editorUserName),
        );
      if (warehouseEditContext.editorName != null)
        formDataToSubmit.append(
          "editorName",
          String(warehouseEditContext.editorName),
        );
    }
    ...
    formDataToSubmit.append(
      "reportType",
      editFormData.reportType || "Home Wash Test",
    );
    ...
    formDataToSubmit.append(
      "reportSampleSizes",
      JSON.stringify(editFormData.sampleSize || []),
    );

    // Care Label Information (if applicable)
    if (editFormData.careSymbols) {
      const symbolsValue =
        typeof editFormData.careSymbols === "string"
...
export const handleImageUpdateSubmit = async (
  ...
  try {
    ...
    const result = await updateReportImagesAPI(
      reportId,
      editingImageType,
      formData,
    );
    ...
    showToast.error(
      error.message || "Failed to update images. Please try again.",
    );
  } finally {
 ...
 */
export const handleEditImageUpload = (
  files,
  type,
  currentImages,
  setImages,
  ...
  if (filesToHandle.length > availableSlots) {
    showToast.info(
      `Only ${availableSlots} more image(s) can be added (Limit: ${limit}).`,
    );
  }
