 ...
 * @param {number} limit - Image limit
 */
export const handleCameraInputChange = (
  e,
  currentImages,
  setFormData,
  inputRef,
  ...
  if (validFiles.length > 0) {
    ...
    if (currentCount + validFiles.length > limit) {
      showToast.warning(
        `Total images exceed limit of ${limit}. Only the first ones were added.`,
      );
    }
 ...
 * @param {string} section - Section name for error message
 */
export const triggerCameraInput = (
  currentImages,
  inputRef,
  limit = IMAGE_LIMITS.INITIAL,
  section = "Initial Step",
