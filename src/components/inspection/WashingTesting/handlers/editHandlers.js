    ...
    if (report.ymStyle) {
        try {
            ...
            if (currentReportSizes.length > 0) {
                ...
                } else if (orderDetails.sizeList && Array.isArray(orderDetails.sizeList)) {
            ...
            const allAvailableColors = new Set([...(orderDetails.colors || []), ...currentColors]);
            setEditAvailableColors([...allAvailableColors]);

            const currentPOs = ensureArray(report.po);
            const allAvailablePOs = new Set([...(yorksysData.pos || []), ...currentPOs]);
            setEditAvailablePOs([...allAvailablePOs]);

            const currentETDs = ensureArray(report.exFtyDate);
            const allAvailableETDs = new Set([...(yorksysData.etds || []), ...currentETDs]);
