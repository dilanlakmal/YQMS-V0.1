...
export const useFormStore = create((set, get) => ({
    ...
    handleOrderNoSelect: async (orderNo) => {
        ...
        const { setFormData, formData, setShowOrderNoSuggestions, setOrderNoSuggestions } = get();
    ...
    handleInputChange: (field, value) => {
        ...
        _handleInputChangeBase(field, value, (f, v, newData, prev) => {
            if ((f === "ymStyle" && prev.ymStyle !== v) || (f === "style" && prev.style !== v)) {
                newData.color = [];
                newData.po = [];
                newData.exFtyDate = [];
        ...
        if (isStyleField && value.length >= 2) {
            handleOrderNoSearch(value, setOrderNoSuggestions, setShowOrderNoSuggestions, setIsSearchingOrderNo);
                ...
                _colorFetchTimer = setTimeout(async () => {
                    if (shouldAutoFetchColors(value)) {
                        ...
                        if (suggestions && suggestions.length === 1) {
                            ...
                            handleOrderNoSelect(suggestions.find((s) => s.toUpperCase() === value.toUpperCase()));
                        } else {
                            const isPrefix = suggestions && suggestions.some(
                                (s) => s.toUpperCase().startsWith(value.toUpperCase()) && s.length > value.length,
                            );
                            if (!isPrefix) {
    ...
    searchOrderNo: (value) => {
        if (value && value.length >= 2) {
            const { setOrderNoSuggestions, setShowOrderNoSuggestions, setIsSearchingOrderNo } = get();
            handleOrderNoSearch(value, setOrderNoSuggestions, setShowOrderNoSuggestions, setIsSearchingOrderNo);
        }
    },
    ...
    handleFileInputChange: (e) => {
        ...
        handleFileInputChangeHandler(e, formData.images, setFormData, _fileInputRef, IMAGE_LIMITS.INITIAL);
    },

    handleCameraInputChange: (e) => {
        const { formData, setFormData } = get();
        handleCameraInputChangeHandler(e, formData.images, setFormData, _cameraInputRef, IMAGE_LIMITS.INITIAL);
    },

    triggerFileInput: () => {
        const { formData } = get();
        triggerFileInputHandler(formData.images, _fileInputRef, IMAGE_LIMITS.INITIAL, "Initial Step");
    },

    triggerCameraInput: () => {
        const { formData } = get();
        triggerCameraInputHandler(formData.images, _cameraInputRef, IMAGE_LIMITS.INITIAL, "Initial Step");
    },

    handleRemoveImageWrapper: (index) => {
        const { formData, setFormData } = get();
        const { imageRotations, setImageRotations, handleRemoveImage } = useImageStore.getState();
        handleRemoveImage(
            index,
            formData.images,
            (updater) =>
