...
export const useImageStore = create((set, get) => ({
    ...
    setImageRotations: (v) =>
        set({ imageRotations: typeof v === "function" ? v(get().imageRotations) : v }),
    setReceivedImageRotations: (v) =>
        set({ receivedImageRotations: typeof v === "function" ? v(get().receivedImageRotations) : v }),
    setCompletionImageRotations: (v) =>
        set({ completionImageRotations: typeof v === "function" ? v(get().completionImageRotations) : v }),
    ...
    handleRemoveImage: (index, images, setImages, rotationState, setRotationState) => {
    ...
    rotateFormImage: (index, direction = "cw") => get()._rotateImage(index, direction, "imageRotations"),
    rotateReceivedImage: (index, direction = "cw") => get()._rotateImage(index, direction, "receivedImageRotations"),
    rotateCompletionImage: (index, direction = "cw") => get()._rotateImage(index, direction, "completionImageRotations"),
    ...
    rotateReportImage: (reportId, imageKey, direction = "cw") => {
        ...
        set((s) => {
            ...
            const step = direction === "cw" ? 90 : -90;
            return {
                reportImageRotations: { ...s.reportImageRotations, [key]: (current + step) % 360 },
            };
        });
    },

    // ─── Image viewer actions ─────────────────────────────────────────
    openImageViewer: (imageUrl, imageTitle = null, images = null, currentIndex = 0) => {
    ...
    goToPreviousImage: () => {
        set((s) => {
            const v = s.imageViewer;
            if (v.images.length <= 1) return s;
            const prevIndex = (v.currentIndex - 1 + v.images.length) % v.images.length;
    ...
    rotateImageViewer: (direction = "ccw") => {
        set((s) => {
            ...
            if (v.imageUrl) {
                ...
                try {
                    localStorage.setItem("washingMachineImageRotations", JSON.stringify(updatedSaved));
                } catch (error) {
        ...
        if (imageViewer.zoom > 1) {
            set((s) => ({
                imageViewer: {
                    ...s.imageViewer,
                    isDragging: true,
                    dragStart: { x: e.clientX - s.imageViewer.panX, y: e.clientY - s.imageViewer.panY },
                },
            }));
        } else {
            toggleZoom();
