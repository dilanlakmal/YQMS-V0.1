...
export const useModalStore = create((set, get) => ({
      ...
      receivedModal: {
        ...
        images:
          typeof images === "function"
            ? images(s.receivedModal.images)
            : images,
      },
    })),
  setReceivedNotes: (notes) =>
    set((s) => ({ receivedModal: { ...s.receivedModal, notes } })),
  setShouldUpdateReceivedStatus: (v) =>
    set((s) => ({
      receivedModal: { ...s.receivedModal, shouldUpdateStatus: v },
    })),
  ...
  setCompletionImages: (images) =>
    set((s) => ({
      completionModal: {
        ...
        images:
          typeof images === "function"
            ? images(s.completionModal.images)
            : images,
      },
    })),
  ...
  setEditImages: (images) =>
    set((s) => ({
      editImagesModal: {
        ...
        images:
          typeof images === "function"
            ? images(s.editImagesModal.images)
            : images,
      },
    })),
  setEditNotes: (notes) =>
    set((s) => ({ editImagesModal: { ...s.editImagesModal, notes } })),
  setIsUpdatingImages: (v) =>
    set((s) => ({ editImagesModal: { ...s.editImagesModal, isUpdating: v } })),
  ...
  _makeImageUploadHandler: (getImages, setImages) => (files) => {
    ...
    if (arr.length > slots)
      showToast.info(`Only ${slots} more image(s) can be added (Limit: 5).`);
  ...
  handleReject: async (reportId, rejectedNotes = "") => {
    ...
    const filters =
      useWashingFilterStore.getState()[
        tab === "warehouse" ? "warehouse" : "standard"
      ];
    ...
    const success = await reportStore.rejectReport(
      ...
      reportId,
      {
        receiver_emp_id: _currentUser?.emp_id || _currentUser?.id,
        rejectedNotes,
      },
      filters,
    );
  ...
  handleEditSubmit: (e) => {
    ...
    const { _currentUser, causeAssignHistory } =
      useAssignControlStore.getState();
    const { isWarehouseUser } = computeUserRoles(
      _currentUser,
      causeAssignHistory,
    );
    handleEditFormSubmit(
  ...
  handleAcceptReceivedFromCard: async (report) => {
    ...
    if (!reportId) return;
    const { _currentUser } = useAssignControlStore.getState();
    const { showReportDateQR, setShowReportDateQR, setShowReportDateScanner } =
      get();
    await useWashingReportsStore
      .getState()
      .saveReceivedStatus(reportId, [], "", true, _currentUser, (id) => {
  ...
  handleCompletionImageUpload: (files) => {
    const { completionModal, setCompletionImages, _makeImageUploadHandler } =
      get();
    _makeImageUploadHandler(
  ...
  handleCompletionSubmit: async () => {
    ...
    let report = reports.find(
      (r) =>
        r._id === completionModal.reportId || r.id === completionModal.reportId,
    );
    if (!report)
      report = whReports.find(
        (r) =>
          r._id === completionModal.reportId ||
          r.id === completionModal.reportId,
      );

    const reportType = report?.reportType || "Garment Wash Report";

    const { causeAssignHistory, _currentUser } =
      useAssignControlStore.getState();
    const activeAssign =
  ...
  handleUpdateImages: async () => {
    ...
    try {
      ...
      const fieldName =
        editImagesModal.type === "initial"
          ? "images"
          : editImagesModal.type === "received"
            ? "receivedImages"
            : "completionImages";

      const newFiles = editImagesModal.images.filter(
        (img) => img instanceof File,
      );
      const existingUrls = editImagesModal.images.filter(
        (img) => typeof img === "string",
      );

      newFiles.forEach((file) => fd.append(fieldName, file));
      if (existingUrls.length > 0)
        fd.append(`${fieldName}Urls`, JSON.stringify(existingUrls));

      let notesFieldName;
      if (editImagesModal.type === "initial") notesFieldName = "notes";
      else if (editImagesModal.type === "received")
        notesFieldName = "receivedNotes";
      else
        notesFieldName = getCompletionNotesField(
          editImagesModal.report.reportType,
        );
