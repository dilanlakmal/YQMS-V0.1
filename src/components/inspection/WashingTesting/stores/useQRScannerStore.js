...
import {
  getQRCodeBaseURL,
  parseQRCodeScanResult,
  buildQRCodeURLWithMeta,
} from "../helpers/qrHelpers.js";
...
const _getReportById = (reportId) => {
  ...
  const fromStandard = standard?.reports?.find(
    (r) => (r._id || r.id) === reportId,
  );
  const fromWarehouse = warehouse?.reports?.find(
    (r) => (r._id || r.id) === reportId,
  );
...
const _drawQRReportImage = (qrDataURL, report, idQr) => {
  return new Promise((resolve, reject) => {
    ...
    const formatDate = (report) => {
      const raw =
        report?.createdAt || report?.submittedAt || report?.reportDate;
      return raw
        ...
        if (report) {
          _drawQRReportImage(qrDataURL, report, idQr)
            ...
            .catch((err) => {
              ...
              _dataURLToJPEG(qrDataURL)
                .then(doDownload)
                .catch(() => doDownload(qrDataURL));
            });
        } else {
          _dataURLToJPEG(qrDataURL)
            .then(doDownload)
  ...
  _processQRResult: async (targetReportId, reportId) => {
    ...
    try {
      ...
      const reportResponse = await fetch(
        `${API_BASE_URL}/api/report-washing/${id}`,
      );
      ...
      if (currentStatus === "pending" || !currentStatus) {
        ...
        formStore.setFormData({
          ...
          reportSampleSizes: (() => {
            const s =
              currentReport.reportSampleSizes ?? currentReport.sampleSize;
  ...
  handleQRCodeFileUpload: async (event, reportId) => {
    ...
    const processResult = async (decodedText) => {
      ...
      if (targetId && targetId !== reportId) {
        ...
        showToast.info(
          "Detected QR from a different report. Opening that report.",
        );
      }
    ...
    try {
      ...
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          console.debug(
            "[QR] Primary decoder found no QR code, trying jsQR fallback...",
          );
        }
      }
      ...
      try {
        ...
        const looksLikeOurDownloadedQR =
          fileName.includes("qr-code-report-") &&
          idStr &&
          fileName.includes(idStr);
