...
export const useOrderDataStore = create((set, get) => ({
  ...
  fetchOrderColors: async (orderNo, setFormData) => {
    ...
    if (
      isLoadingColors ||
      (_lastFetchedColorStyle?.toUpperCase() === normalizedStyle &&
        ...
        if (
          ...
          if (
            ...
            set({
              ...
              fabricContent: result.data.FabricContent,
            });
            if (setFormData)
  ...
  fetchLastCareSymbols: async (ymStyle, setFormData) => {
    ...
    try {
      ...
      if (typeof latestReport.careSymbols === "string") {
        try {
          careSymbols = JSON.parse(latestReport.careSymbols);
        } catch {
          return;
        }
      ...
      if (setFormData) {
        setFormData((prev) => ({
          ...
          careSymbols:
            !prev.careSymbols || Object.keys(prev.careSymbols).length === 0
              ? careSymbols
              : prev.careSymbols,
        }));
      }
      ...
      console.debug(
        "Could not fetch last care symbols for style:",
        ymStyle,
        error,
      );
    }
