...
export const useWashingReportsStore = create((set, get) => ({
  ...
  refreshAllReports: async () => {
    const {
      ...
      warehouse: wf,
      easy_scan: ef,
    } = useWashingFilterStore.getState();
    ...
    if (ymStyle || style)
      promises.push(
        useOrderDataStore.getState().fetchUsedColors(ymStyle || style),
      );
  ...
  submitReport: async (formData, user, onSuccess) => {
    ...
    const getSampleSizeArr = (data) => {
      if (!data) return [];
      if (
        Array.isArray(data.reportSampleSizes) &&
        data.reportSampleSizes.length > 0
      )
        return data.reportSampleSizes.filter(Boolean);
      if (Array.isArray(data.sampleSize))
        return data.sampleSize.filter(Boolean);
    ...
    try {
      ...
      const hasShrinkageData =
        Array.isArray(shrinkageRows) &&
        shrinkageRows.some(
          (r) => r.selected !== false && (r.beforeWash || r.afterWash),
        );
      const currentSize =
        dataToSubmit.sampleSize != null
      ...
      if (hasShrinkageData && currentSize) {
        ...
        reportSampleSizes =
          Array.isArray(dataToSubmit.reportSampleSizes) &&
          dataToSubmit.reportSampleSizes.length > 0
            ? dataToSubmit.reportSampleSizes
                .map((s) => String(s).trim())
                .filter(Boolean)
      ...
      const skipFields = [
        ...
        "styleNo",
        "moNo",
        "date", // Redundant — only ymStyle is stored
      ];
      ...
      if (
        ...
        if (existingUrls.length > 0)
          fd.append("careLabelImageUrls", JSON.stringify(existingUrls));
        ...
        dataToSubmit.images.forEach((imgFile) => {
          if (imgFile instanceof File) {
            if (!allowedTypes.includes(imgFile.type.toLowerCase()))
              throw new Error(
                `Invalid image file: ${imgFile.name}. Only JPEG, PNG, GIF, and WebP images are allowed.`,
              );
              ...
              .openSizeFollowUpModal(
                ...
                Array.isArray(formData.color)
                  ...
                  : formData.color
                    ...
                    : [],
                formData.reportType || "Garment Wash Report",
                Array.isArray(formData.po)
                  ? formData.po
                  : formData.po
                    ? [formData.po]
                    : [],
                Array.isArray(formData.exFtyDate)
                  ? formData.exFtyDate
                  : formData.exFtyDate
                    ? [formData.exFtyDate]
                    : [],
  ...
  saveReceivedStatus: async (
    ...
    try {
      ...
      receivedImages.forEach((f) => {
        if (f instanceof File) fd.append("receivedImages", f);
      });
      ...
      showToast.error(
        error.message || "An error occurred while saving received.",
      );
      return false;
    } finally {
  ...
  saveCompletionStatus: async (
    ...
    try {
      ...
      showToast.error(
        error.message || "An error occurred while completing the report.",
      );
      return false;
    } finally {
  ...
  updateReport: async (reportId, editFormData) => {
    ...
    try {
      ...
      Object.keys(editFormData).forEach((key) => {
        if (
          ...
          if (key === "shrinkageRows" && Array.isArray(editFormData[key])) {
            ...
            fd.append(key, JSON.stringify(editFormData[key]));
      ...
      const appendImages = (arr, fieldName) => {
        ...
        if (urls.length > 0)
          fd.append(`${fieldName}Urls`, JSON.stringify(urls));
        arr.forEach((item, i) => {
      ...
      if (
        ...
        const urls = editFormData.careLabelImage.filter(
          (item) => typeof item === "string",
        );
        if (urls.length > 0)
          fd.append("careLabelImageUrls", JSON.stringify(urls));
  ...
  handlePrintPDF: async (report) => {
    ...
    try {
      ...
      const qrCodeDataURL = await useQRScannerStore
        .getState()
        .generateQRCodeDataURL(qrCodeValue, 100);
