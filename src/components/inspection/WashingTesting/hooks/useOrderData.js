...
export const useOrderData = () => {
  ...
  // ─── fetchOrderColors ─────────────────────────────────────────────────────
  const fetchOrderColors = useCallback(
    async (orderNo, setFormData) => {
      ...
      const { isLoadingColors, availableColors, custStyle } =
        useOrderDataStore.getState();
      if (
        isLoadingColors ||
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `Failed to fetch order details for ${trimmedOrderNo}: ${response.status}`,
          );
          setAvailableColors([]);
        }
      } catch (error) {
  ...
  // ─── fetchUsedColors ──────────────────────────────────────────────────────
  const fetchUsedColors = useCallback(
    async (orderNo) => {
      if (!orderNo) {
        setUsedColors([]);
        return;
  ...
  const fetchYorksysOrderETD = useCallback(
    async (orderNo, setFormData) => {
      if (!orderNo || !isValidStyleFormat(orderNo)) {
        setAvailablePOs([]);
        setAvailableETDs([]);
      ...
      if (
        lastFetchedYorksysStyleRef.current?.toUpperCase() === normalizedStyle
      ) {
        ...
        if (response.ok) {
          ...
          if (
            ...
            if (
              ...
              if (setFormData)
                setFormData((prev) => ({ ...prev, fabrication: fabString }));
            } else {
              setFabrication("");
              setFabricContent([]);
              if (setFormData)
                setFormData((prev) => ({ ...prev, fabrication: "" }));
            }
          } else {
            setAvailablePOs([]);
            setAvailableETDs([]);
          ...
          setSeason("");
          if (setFormData)
            setFormData((prev) => ({ ...prev, season: "", fabrication: "" }));
        } else {
          console.warn(
  ...
  // ─── fetchAnfSpecs ────────────────────────────────────────────────────────
  const fetchAnfSpecs = useCallback(
    async (moNo, size) => {
      if (!moNo || !size) {
        setAnfSpecs([]);
        return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (response.status !== 404) {
            console.warn(
              `Failed to fetch ANF specs for MO ${moNo} Size ${size}: ${response.status}`,
            );
          }
          setAnfSpecs([]);
        }
