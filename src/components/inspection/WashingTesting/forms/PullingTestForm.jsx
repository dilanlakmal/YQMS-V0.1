...
const PullingTestForm = ({
  ...
  const getFilteredOptions = (field) => {
    ...
    sortedHistory.forEach((item) => {
      ...
      if (preparedId) {
        const current = userRolesMap.get(preparedId) || {
          preparedBy: false,
          checkedBy: false,
          approvedBy: false,
        };
  ...
  useEffect(() => {
    ...
    if (formData.preparedBy) {
      if (preparedByOptions.some((o) => o.value === formData.preparedBy))
        return;
      const byName = users.find(
        (u) =>
          u.name === formData.preparedBy ||
          formData.preparedBy === `(${u.emp_id}) ${u.name}`,
      );
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 pb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="relative">
              ...
              {showOrderNoSuggestions &&
                orderNoSuggestions &&
                orderNoSuggestions.filter((on) => on !== formData.ymStyle)
                  .length > 0 && (
            ...
            <div>
              ...
              <TimePicker
                value={
                  formData.testTime
                    ? (() => {
                        const t = dayjs(formData.testTime, [
                          "h:mm A",
                          "HH:mm",
                          "h:mm a",
                        ]);
                ...
                onChange={(date, dateString) =>
                  handleInputChange("testTime", dateString || "")
                }
                format="h:mm A"
                use12Hours
                  ...
                  <tr key={index}>
                    ...
                    <td className="border border-gray-300 dark:border-gray-600 px-2 py-1">
                      <input
                        ...
                        onChange={(e) =>
                          handleRowChange(index, "pullingForce", e.target.value)
                        }
                        className="w-full px-2 py-1 border-0 focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                    ...
                    <td className="border border-gray-300 dark:border-gray-600 px-2 py-1">
                      <input
                        ...
                        onChange={(e) =>
                          handleRowChange(index, "pullingTime", e.target.value)
                        }
                        className="w-full px-2 py-1 border-0 focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                    ...
                    <td className="border border-gray-300 dark:border-gray-600 px-2 py-1">
                      <input
                        ...
                        value={row.results}
                        onChange={(e) =>
                          handleRowChange(index, "results", e.target.value)
                        }
                        className="w-full px-2 py-1 border-0 focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
            ...
            <textarea
              ...
              placeholder={
                isCompleting
                  ...
                  : "Add any additional notes or comments about this pulling test report..."
              }
            />
          </div>
        </div>
