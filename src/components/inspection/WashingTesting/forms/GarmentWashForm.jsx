...
const GarmentWashForm = ({
  ...
  // Do not pre-fill CHECKED BY / APPROVED BY from assignment; keep default placeholder "Select User"

  // Validate current selection against allowed options; normalize legacy name to emp_id so display works
  useEffect(() => {
    if (users.length === 0) return;
    if (formData.checkedBy) {
      const matchByValue = checkedByOptions.some(
        (o) => o.value === formData.checkedBy,
      );
      if (matchByValue) return;
      const byName = users.find(
        (u) =>
          u.name === formData.checkedBy ||
          formData.checkedBy === `(${u.emp_id}) ${u.name}`,
      );
    ...
    if (formData.approvedBy) {
      const matchByValue = approvedByOptions.some(
        (o) => o.value === formData.approvedBy,
      );
      if (matchByValue) return;
      const byName = users.find(
        (u) =>
          u.name === formData.approvedBy ||
          formData.approvedBy === `(${u.emp_id}) ${u.name}`,
      );
      if (byName && approvedByOptions.some((o) => o.value === byName.emp_id)) {
        handleInputChange("approvedBy", byName.emp_id);
      } else {
  ...
  const filteredColors = isCompleting
    ? initialRecordColorsRef.current.length > 0
  ...
  const [isLoadingMeasurementSpecs, setIsLoadingMeasurementSpecs] =
    useState(false);
  ...
  useEffect(() => {
    if (
      !formData.colorFastnessRows ||
      formData.colorFastnessRows.length === 0
    ) {
      ...
      if (
        ...
        JSON.stringify(formData.colorFastnessRows)
      ) {
    ...
    if (
      !formData.colorStainingRows ||
      formData.colorStainingRows.length === 0
    ) {
      ...
      if (
        JSON.stringify(updatedRows) !==
        JSON.stringify(formData.colorStainingRows)
      ) {
    ...
    if (!formData.visualAssessmentRows) {
      handleInputChange("visualAssessmentRows", [
        ...
        {
          item: "Trimmings / Accessories",
          accepted: true,
          rejected: false,
          comments: "",
        },
        {
          item: "Others / Care label",
          accepted: true,
          rejected: false,
          comments: "",
        },
  ...
  const fetchMeasurementSpecs = async (orderNo) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/washing/measurement-specs/${orderNo}`,
      );
  ...
  const getFilteredAvailableSizes = () => {
    ...
    if (!sizeField && hasCompletedShrinkage && formData.sampleSize != null) {
      const fullList =
        Array.isArray(formData.reportSampleSizes) &&
        formData.reportSampleSizes.length > 0
          ...
          : Array.isArray(formData.sampleSize) && formData.sampleSize.length > 0
            ...
            : typeof formData.sampleSize === "string" &&
                formData.sampleSize.trim()
    ...
    const groupedSpecs = isBeforeWash
    ...
    if (
      ...
      userSizes = formData.reportSampleSizes
        .map(normalizeSizeToken)
        .filter(Boolean);
    }
    ...
    if (
      userSizes.length === 0 &&
      formData.sampleSize != null &&
      Array.isArray(formData.sampleSize) &&
      formData.sampleSize.length > 0
    ) {
    ...
    if (userSizes.length > 0) {
      ...
      if (intersected.length > 0) {
        ...
        const single = Array.isArray(formData.sampleSize)
          ? formData.sampleSize[0]
          : formData.sampleSize;
  ...
  const getEffectiveSampleSize = () => {
    ...
    if (Array.isArray(s) && s.length > 0)
      return typeof s[0] === "string" ? s[0].trim() : String(s[0]);
    return undefined;
  };
  ...
  const getAllSizeSpecs = () => {
    ...
    const comparisonData = specsToUse.map((spec) => {
      const row = {
        measurementPoint:
          spec.MeasurementPointEngName || spec.measurementPoint || "UNKNOWN",
        sizes: {},
      };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (
        orderNo &&
        orderNo.length >= 3 &&
        orderNo !== lastFetchedOrderNoRef.current
      ) {
  ...
  useEffect(() => {
    ...
    if (specsToUse.length > 0) {
      ...
      const sizeChanged =
        prevSizeRef.current !== "" && prevSizeRef.current !== size;
      ...
      beforeWashSpecs.forEach((spec) => {
        const pointName =
          spec.MeasurementPointEngName || spec.measurementPoint || "";
        if (pointName) {
      ...
      const dynamicRows = specsToUse.map((spec, idx) => {
        ...
        if (isBeforeWash) {
          ...
          afterWashSpecValue = afterWashSpec
            ? getSizeSpecValue(afterWashSpec, size)
            : "";
        } else {
          afterWashSpecValue = getSizeSpecValue(spec, size);
          const beforeWashSpec = beforeWashSpecsMap[locationName];
          beforeWashSpecValue = beforeWashSpec
            ? getSizeSpecValue(beforeWashSpec, size)
            : "";
        }

        // Only preserve existing measurements if size hasn't changed
        const existingRow = !sizeChanged
          ? currentRows.find((r) => r.location === locationName)
          : null;
  ...
  useEffect(() => {
    if (
      fabrication &&
      fabrication !== "" &&
      (!formData.mainFabric || formData.mainFabric === "")
    ) {
  ...
  useEffect(() => {
    const headerColor = Array.isArray(formData.color)
      ? formData.color.join(", ")
      : formData.color || "";
    if (headerColor !== "") {
      ...
      if (
        JSON.stringify(updatedFastness) !==
        JSON.stringify(formData.colorFastnessRows)
      ) {
        handleInputChange("colorFastnessRows", updatedFastness);
      }
      if (
        JSON.stringify(updatedStaining) !==
        JSON.stringify(formData.colorStainingRows)
      ) {
  ...
  const handleCareLabelImageChange = (e) => {
    ...
    if (files.length > 0) {
      const currentImages = Array.isArray(formData.careLabelImage)
        ? formData.careLabelImage
        : formData.careLabelImage
          ? [formData.careLabelImage]
          : [];
  ...
  const parseFraction = (val) => {
    ...
    if (parts.length === 2) {
      ...
      if (fractionParts.length === 2) {
        return (
          integer + parseFloat(fractionParts[0]) / parseFloat(fractionParts[1])
        );
      }
      return integer;
    }
  ...
  const updateRow = (field, index, key, value) => {
    ...
    if (field === "shrinkageRows") {
      ...
      // Priority 3: Fallback - Measurements Only (no specs available)
        ...
        rows[index].shrinkage =
          (actualShrinkage >= 0 ? "+" : "") + actualShrinkage.toFixed(2) + "%";
              ...
              <div className="flex items-center gap-1">
                ...
                <div className="flex items-center bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <button
                    type="button"
                    onClick={() => {
                      setIntegerPart(Math.max(0, integerPart - 1));
                      setUseManualMode(false);
                    }}
    ...
    <div className="space-y-8">
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              onKeyDown={(e) => {
                ...
                if (
                  ...
                  const firstValidSuggestion = orderNoSuggestions.filter(
                    (on) => normalize(on) !== normalize(formData.moNo),
                  )[0];
              ...
              orderNoSuggestions.filter(
                ...
                <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-auto">
                  {orderNoSuggestions
                    .filter(
                      (orderNo) =>
                        normalize(orderNo) !== normalize(formData.moNo),
                    )
                    .map((orderNo, index) => (
                      <div
                        key={index}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white cursor-not-allowed"
              required
              readOnly
            />
          </div>
        ...
        <div className="bg-yellow-50 dark:bg-gray-700/30 p-4 rounded-md border border-yellow-200 dark:border-gray-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={formData.mainFabric || ""}
                onChange={(e) =>
                  handleInputChange("mainFabric", e.target.value)
                }
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  handleInputChange("washingMethod", e.target.value)
                }
                rows={2}
                className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white resize-vertical"
                required
              />
            </div>
            ...
            <div className="md:col-span-2 mt-2 relative">
              ...
              <input
                type="text"
                value={
                  Object.keys(formData.careSymbols || {}).length > 0
                    ? "selected"
                    : ""
                }
                required
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            ...
            <button
              type="button"
              onClick={() => addRow("colorFastnessRows")}
            ...
            <table className="w-full text-sm text-left">
              ...
              <tbody>
                {(formData.colorFastnessRows || []).map((row, index) => (
                  <tr
                    key={index}
                    className="dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800/50"
                  >
                    ...
                    <td className="p-2 border text-center">
                      <button
                        type="button"
                        onClick={() => removeRow("colorFastnessRows", index)}
        ...
        <div>
          <div className="flex justify-between items-center mb-2">
            <h3 className="font-bold text-gray-800 dark:text-white">
              Color Fastness to Washing (Color Staining)
            </h3>
            <button
              type="button"
              onClick={() => addRow("colorStainingRows")}
              className="text-blue-600 text-sm flex items-center hover:underline"
            >
              <Plus size={16} className="mr-1" /> Add Row
            </button>
                ...
                {(formData.colorStainingRows || []).map((row, index) => (
                  ...
                  >
                    ...
                    <td className="p-2 border text-center">
                      <button
                        type="button"
                        onClick={() => removeRow("colorStainingRows", index)}
                        className="text-red-500 hover:text-red-700"
                      >
                        <Trash2 size={16} />
                      </button>
        ...
        <div>
          <h3 className="font-bold text-gray-800 dark:text-white mb-2">
            Visual Assessment of the garment after washing
          </h3>
          <div className="overflow-x-auto border rounded-md">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-bold">
                  ...
                  >
                    ...
                    <td className="p-2 border text-center">
                      <button
                        type="button"
                        onClick={() =>
                          toggleVisualAssessment(index, "accepted")
                        }
                    ...
                    <td className="p-2 border text-center">
                      <button
                        type="button"
                        onClick={() =>
                          toggleVisualAssessment(index, "rejected")
                        }
                  ...
                  <div className="relative flex items-center gap-1.5 sm:gap-3 px-1.5 sm:px-4 py-1.5 bg-white dark:bg-gray-800 rounded-xl border border-blue-100 dark:border-blue-800 shadow-sm transition-all hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-md">
                    ...
                    >
                      <button
                        ...
                        className={`px-1.5 sm:px-3 py-1 rounded-md text-[10px] font-black uppercase tracking-wider transition-all duration-200 min-h-[40px] sm:min-h-0 ${
          ...
          <div className="relative overflow-x-auto border rounded-xl shadow-sm bg-white dark:bg-gray-900 overflow-hidden">
            ...
            <table className="w-full text-sm text-left border-collapse">
              <thead className="sticky top-0 z-10 bg-gray-50/95 dark:bg-gray-800/95 backdrop-blur-md text-gray-500 dark:text-gray-400 font-extrabold text-[10px] uppercase tracking-wider shadow-sm">
                <tr>
                  <th
                    className={`p-4 border-b dark:border-gray-700 w-10 text-center first:rounded-tl-xl ${isShrinkageSaved ? "hidden" : ""}`}
                  >
                    <div className="flex items-center justify-center">
                      <label className="relative flex items-center justify-center cursor-pointer group">
                        ...
                        <Check
                          size={14}
                          strokeWidth={3}
                          className="absolute text-white opacity-0 peer-checked:opacity-100 transform scale-50 peer-checked:scale-100 transition-all duration-200 ease-out"
                        />
                      </label>
                    </div>
                  </th>
                  <th className="p-4 border-b dark:border-gray-700 w-14 text-center">
                    NO
                  </th>
                  ...
                  <th
                    className={`p-4 border-b dark:border-gray-700 w-24 text-center transition-all duration-300 ${formData.washType !== "After Wash" ? "bg-blue-50/80 dark:bg-blue-900/40 ring-inset ring-2 ring-blue-400/30" : "opacity-40 grayscale-[0.5]"}`}
                  >
                    <div className="flex flex-col text-[10px] text-blue-600 dark:text-blue-400">
                      ...
                      {formData.washType !== "After Wash" && (
                        <span className="text-[8px] mt-0.5 bg-blue-600 text-white px-1 rounded-full uppercase">
                          Active
                        </span>
                      )}
                    </div>
                  ...
                  <th
                    className={`p-4 border-b dark:border-gray-700 w-24 text-center transition-all duration-300 ${formData.washType === "After Wash" ? "bg-purple-50/80 dark:bg-purple-900/40 ring-inset ring-2 ring-purple-400/30" : "opacity-40 grayscale-[0.5]"}`}
                  >
                    <div className="flex flex-col text-[10px] text-purple-600 dark:text-purple-400">
                      ...
                      {formData.washType === "After Wash" && (
                        <span className="text-[8px] mt-0.5 bg-purple-600 text-white px-1 rounded-full uppercase">
                          Active
                        </span>
                      )}
                    </div>
                  ...
                  <th className="p-4 border-b dark:border-gray-700 w-32 text-center">
                    Shrinkage %
                  </th>
                  <th className="p-4 border-b dark:border-gray-700 w-24 text-center">
                    Req.
                  </th>
                  <th className="p-4 border-b dark:border-gray-700 text-center w-20">
                    PASS
                  </th>
                  <th className="p-4 border-b dark:border-gray-700 text-center w-20">
                    FAIL
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {!formData.sampleSize ? (
                    ...
                    <td colSpan={11} className="p-12 text-center">
                      <div className="flex flex-col items-center justify-center opacity-40">
                        ...
                        <p className="text-lg font-bold text-gray-500">
                          Select a Test Size to Load Specifications
                        </p>
                        <p className="text-sm">
                          Specifications will automatically load once a size is
                          selected
                        </p>
                      </div>
                    </td>
                  ...
                  (formData.shrinkageRows || []).map((row, index) => (
                    <tr
                      key={row.id || index}
                      className={`group dark:border-gray-700 border-b border-gray-50 last:border-none hover:bg-blue-50/60 dark:hover:bg-blue-900/10 transition-colors duration-200 ${isShrinkageSaved && !row.selected ? "hidden" : ""}`}
                    >
                      <td
                        className={`p-3 text-center ${isShrinkageSaved ? "hidden" : ""}`}
                      >
                        <div className="flex items-center justify-center">
                          <label className="relative flex items-center justify-center cursor-pointer group">
                            <input
                      ...
                      <td
                        className={`p-3 border-l border-r border-blue-50 transition-all duration-300 ${formData.washType !== "After Wash" ? "bg-blue-50/50 dark:bg-blue-900/20 scale-[1.02] z-10 shadow-sm" : "opacity-40 dark:border-blue-900/30"}`}
                      >
                        <div className="flex flex-col items-center justify-center gap-0.5">
                          {row.isFromSpec ? (
                            <div className="relative">
                              <span
                                className={`font-black text-lg tracking-tight tabular-nums ${formData.washType !== "After Wash" ? "text-blue-600 dark:text-blue-400" : "text-gray-400"}`}
                              >
                                {row.beforeWashSpec}
                              </span>
                              <span className="absolute -top-1 -right-3 text-[9px] font-bold text-blue-300">
                                IN
                              </span>
                            </div>
                          ) : (
                      ...
                      <td
                        className={`p-3 border-l border-r border-purple-50 transition-all duration-300 ${formData.washType === "After Wash" ? "bg-purple-50/50 dark:bg-purple-900/20 scale-[1.02] z-10 shadow-sm" : "opacity-40 dark:border-purple-900/30"}`}
                      >
                        <div className="flex flex-col items-center justify-center gap-0.5">
                          {row.isFromSpec ? (
                            <div className="relative">
                              <span
                                className={`font-black text-lg tracking-tight tabular-nums ${formData.washType === "After Wash" ? "text-purple-600 dark:text-purple-400" : "text-gray-400"}`}
                              >
                                {row.afterWashSpec}
                              </span>
                              <span className="absolute -top-1 -right-3 text-[9px] font-bold text-purple-300">
                                IN
                              </span>
                            </div>
                          ) : (
                      ...
                      <td className="p-3 relative">
                        <div
                          className={`group/cell rounded-xl border transition-all duration-200 flex flex-col items-center justify-center w-full relative overflow-visible
                      ...
                      <td className="p-3 text-center">
                        {row.shrinkage ? (
                          <div className="flex flex-col items-center gap-1">
                            ...
                            {(() => {
                              const hasBeforeSpec =
                                row.beforeWashSpec && row.beforeWashSpec !== "";
                              const hasAfterSpec =
                                row.afterWashSpec && row.afterWashSpec !== "";
                              const hasG1 =
                                row.beforeWash && row.beforeWash !== "";
                              const hasG2 =
                                row.afterWash && row.afterWash !== "";
                              ...
                              if (hasBeforeSpec && hasG1 && hasG2) {
                                return (
                                  <span className="text-[9px] font-mono text-gray-400 dark:text-gray-500 font-normal leading-tight">
                                    ...
                                    {parseFraction(row.beforeWashSpec).toFixed(
                                      3,
                                    )}
                                    ) × 100
                                  </span>
                              ...
                              // Priority 2: SPEC (AFTER) + Measurements (no SPEC BEFORE)
                                ...
                                return (
                                  <span className="text-[9px] font-mono text-gray-400 dark:text-gray-500 font-normal leading-tight">
                                    ...
                                    {parseFraction(row.afterWashSpec).toFixed(
                                      3,
                                    )}
                                    ) × 100
                                  </span>
                          ...
                          <span className="text-gray-300 font-bold text-xs uppercase tracking-wider">
                            Pending
                          </span>
                        )}
                      </td>
                          ...
                          {row.shrinkage && row.requirement && (
                            <span className="text-[9px] font-mono text-gray-400 dark:text-gray-500 font-normal leading-tight">
                              {(() => {
                                ...
                                const reqValue = reqMatch
                                  ? parseInt(reqMatch[1])
                                  : 5;
                                const shrinkageValue = parseFloat(
                                  row.shrinkage.replace("%", ""),
                                );
                                const isPass =
                                  Math.abs(shrinkageValue) <= reqValue;
                      ...
                      <td className="p-3 text-center">
                        {!row.isFromSpec && (
                          <button
                            type="button"
                            onClick={() => removeRow("shrinkageRows", index)}
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleInputChange("beforeWashComments", e.target.value)
              }
              rows={2}
              className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white resize-vertical"
            />
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                handleInputChange("afterWashComments", e.target.value)
              }
              rows={3}
              className="w-full px-3 py-2 border rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white resize-vertical"
            />
        ...
        <div className="pt-8 mt-8 border-t">
          <h3 className="font-bold text-gray-800 dark:text-white mb-4 uppercase tracking-wider text-sm">
      ...
      {showSizeComparisonModal &&
        (() => {
          ...
          return createPortal(
            <div
              className="fixed inset-0 z-[9999] h-screen w-screen flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200"
              onClick={(e) => {
                e.stopPropagation();
              }}
            >
              ...
              <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden border border-gray-200 dark:border-gray-700 flex flex-col animate-in zoom-in-95 duration-200">
                ...
                <div className="flex items-center justify-between p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/30">
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Showing {comparisonData.data.length} measurement points
                    across {comparisonData.sizes.length} sizes
                  </p>
                  <button
                    type="button"
                    onClick={() => {
