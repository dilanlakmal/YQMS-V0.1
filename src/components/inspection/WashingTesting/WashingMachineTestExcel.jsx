 ...
 */
const generateWashingMachineTestExcel = async (
  report,
  apiBaseUrl = "",
  users = [],
) => {
  try {
    ...
    const submittedByDisplay =
      getNameForView(report.reporter_emp_id, report.reporter_name, users) ||
    ...
    if (
      ...
      const checkedByName = getNameForView(
        report.checkedBy,
        report.checkedByName,
        users,
      );
      const approvedByName = getNameForView(
        report.approvedBy,
        report.approvedByName,
        users,
      );
      const checkedByDisplay = checkedByName
        ? checkedByName !== report.checkedBy
          ? `${checkedByName} (${report.checkedBy})`
          : checkedByName
        : report.checkedBy || "N/A";
      const approvedByDisplay = approvedByName
    ...
    addInfoRow(
      "Colors:",
      report.color && report.color.length > 0 ? report.color.join(", ") : "N/A",
    );
    addInfoRow(
      "PO:",
      report.po && report.po.length > 0 ? report.po.join(", ") : "N/A",
    );
    addInfoRow(
      "Ex Fty Date:",
      report.exFtyDate && report.exFtyDate.length > 0
        ? report.exFtyDate.join(", ")
        : "N/A",
    );
    ...
    const skipFields = [
      ...
      "approvedBy",
      "checkedByName",
      "approvedByName", // Legacy / shown above
    ];

    const extraFields = Object.keys(report).filter(
        ...
        const label1 =
          (
            k1
              .replace(/([A-Z])/g, " $1")
              .charAt(0)
              .toUpperCase() + k1.replace(/([A-Z])/g, " $1").slice(1)
          ).trim() + ":";
          ...
          label2 =
            (
              k2
                .replace(/([A-Z])/g, " $1")
                .charAt(0)
                .toUpperCase() + k2.replace(/([A-Z])/g, " $1").slice(1)
            ).trim() + ":";
    ...
    if (report.receivedDate || report.receivedAt) {
      const step2Date = report.receivedAt
        ...
        : report.receivedDate
          ? new Date(report.receivedDate).toLocaleString("en-GB", {
              hour12: true,
            })
          : "N/A";
    ...
    if (report.completedDate || report.completedAt) {
      const step3Date = report.completedAt
        ...
        : report.completedDate
          ? new Date(report.completedAt || report.completedDate).toLocaleString(
              "en-GB",
              { hour12: true },
            )
          : "N/A";
      ...
      if (report.completionNotes)
        addNotesBlock(report.completionNotes, "FFF1F8F1");
    }
    ...
    if (careEntries.length > 0) {
      ...
      for (let c = 0; c < careEntries.length; c++) {
        ...
        try {
          const buffer = Uint8Array.from(atob(base64Data), (ch) =>
            ch.charCodeAt(0),
          ).buffer;
          const imageId = workbook.addImage({
            buffer: buffer,
