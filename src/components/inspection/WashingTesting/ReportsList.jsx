      ...
      <div className="animate-pulse">
              ...
              {[1, 2, 3, 4, 5].map((i) => (
                <tr key={i}>
                  <td className="px-3 py-4">
                    <div className="h-14 w-14 bg-gray-200 dark:bg-gray-700 rounded mx-auto" />
                  </td>
        ...
        <div className="sm:hidden space-y-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-gray-800 border border-amber-200/60 dark:border-amber-700/40 rounded-xl p-4 h-32 flex flex-col gap-3"
            >
  ...
  return (
    <div className="space-y-4 animate-pulse">
      {[1, 2, 3].map((i) => (
        <div
          key={i}
          className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6 h-64 shadow-sm"
        >
...
const ReportsList = ({
  ...
  const { isAdminUser, isWarehouseUser } = computeUserRoles(
    user,
    causeAssignHistory,
  );
  ...
  const onRefresh = React.useCallback(
    () => _fetchReports(tab, filters),
    [_fetchReports, tab, filters],
  );
  const onToggleReport = React.useCallback(
    (id) => _toggleReport(tab, id),
    [_toggleReport, tab],
  );
  ...
  const storageKey = React.useMemo(
    () =>
      user?.emp_id || user?.id
        ? `yqms_washing_notification_read_${user.emp_id || user.id}`
        : null,
    [user?.emp_id, user?.id],
  );
  const [readNotificationReportIds, setReadNotificationReportIds] =
  ...
  const fetchStyleSuggestions = async (searchTerm) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/report-washing/autocomplete/styles?search=${encodeURIComponent(searchTerm)}`,
      );
      if (response.ok) {
  ...
  const fetchColorSuggestions = async (searchTerm) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/report-washing/autocomplete/colors?search=${encodeURIComponent(searchTerm)}`,
      );
      if (response.ok) {
        const result = await response.json();
        if (result.success) {
        ...
        <div className="flex items-center rounded-lg h-10 px-1">
          ...
          <div className="w-[70px]">
            <Select
              ...
              onChange={(option) =>
                setFilterLimit && setFilterLimit(option.value)
              }
              options={[
                { value: 10, label: "10" },
                { value: 25, label: "25" },
          ...
          {pagination && pagination.totalRecords > 0 && (
            <div className="text-sm font-semibold text-slate-700 dark:text-gray-200 whitespace-nowrap px-2">
              {Math.max(1, (pagination.currentPage - 1) * pagination.limit + 1)}{" "}
              -{" "}
              {Math.min(
          ...
          <div className="flex items-center">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${isLoadingReports ? "animate-spin" : ""}`}
              />
              {newRecordsCount > 0 && (
            ...
            <button
              onClick={() =>
                setFilterPage && setFilterPage(Math.max(1, filterPage - 1))
              }
              disabled={
                !pagination || pagination.currentPage === 1 || isLoadingReports
              }
            ...
            <button
              onClick={() =>
                setFilterPage &&
                setFilterPage(Math.min(pagination.totalPages, filterPage + 1))
              }
              disabled={
                !pagination ||
      ...
      {tab === "easy_scan" ? (
          ...
          <div className="flex-1 flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Select
                value={
                  filterSearch
                    ? { value: filterSearch, label: filterSearch }
                    : null
                }
                onChange={(option) =>
                  setFilterSearch && setFilterSearch(option ? option.value : "")
                }
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-7 gap-4 bg-gray-50 dark:bg-gray-700/30 p-4 rounded-lg">
          ...
          <div className="relative">
            <Select
              value={
                filterColor ? { value: filterColor, label: filterColor } : null
              }
              onChange={(option) =>
                setFilterColor && setFilterColor(option ? option.value : "")
              }
          ...
          <div>
            <Select
              value={
                filterReportType
                  ? { value: filterReportType, label: filterReportType }
                  : null
              }
              onChange={(option) =>
                setFilterReportType &&
                setFilterReportType(option ? option.value : "")
              }
              options={[
          ...
          <div>
            <Select
              value={
                filterFactory
                  ? { value: filterFactory, label: filterFactory }
                  : null
              }
              onChange={(selectedOption) =>
                setFilterFactory &&
                setFilterFactory(selectedOption ? selectedOption.value : "")
              }
              options={[
                { value: "", label: "All Factories" },
                ...(factories && factories.length > 0
                  ? factories.map((f) => ({
                      value: f.name || f.factory,
                      label: f.name || f.factory,
                    }))
                  : [
                      { value: "Newy", label: "Newy" },
                      { value: "Newy2", label: "Newy2" },
          ...
          <div>
            <Select
              ...
              onChange={(option) =>
                setFilterStatus && setFilterStatus(option ? option.value : "")
              }
              options={[
            ...
            <AntDatePicker.RangePicker
              ...
              onChange={(dates, dateStrings) => {
                if (dates && dates[0] && dates[1]) {
                  setFilterStartDate &&
                    setFilterStartDate(dates[0].format("YYYY-MM-DD"));
                  setFilterEndDate &&
                    setFilterEndDate(dates[1].format("YYYY-MM-DD"));
                } else {
      ...
      {isLoadingReports ? (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600 whitespace-nowrap">
              <thead className="bg-amber-50 dark:bg-amber-900/20">
                <tr>
                  <th
                    scope="col"
                    className="px-3 py-3 text-left text-xs font-semibold text-amber-800 dark:text-amber-200 uppercase tracking-wider w-0"
                  >
                    Scan
                  </th>
                  <th
                    scope="col"
                    className="px-4 py-3 text-left text-xs font-semibold text-amber-800 dark:text-amber-200 uppercase tracking-wider"
                  >
                    ID (QR)
                  </th>
                  <th
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-medium rounded ${statusClass}`}
                        >
          ...
          <div className="sm:hidden space-y-4">
            {reports.map((report) => {
              ...
              return (
                <div
                  key={reportId}
                  className="bg-white dark:bg-gray-800 border border-amber-200/60 dark:border-amber-700/40 rounded-xl p-4 shadow-sm flex flex-col gap-3"
                >
                  <div className="flex justify-between items-start border-b border-gray-100 dark:border-gray-700 pb-3">
                    <div className="flex flex-col flex-1 pr-3">
                      <span className="text-[10px] font-bold text-amber-800 dark:text-amber-400 uppercase tracking-wider mb-1">
                        ID (QR)
                      </span>
                    ...
                    <span
                      className={`inline-flex items-center justify-center px-2 py-0.5 text-[10px] font-bold tracking-wide uppercase rounded shrink-0 ${statusClass}`}
                    >
                      {status}
                    </span>
                  </div>
                  ...
                  <div className="flex justify-between items-center bg-gray-50/50 dark:bg-gray-900/30 rounded-lg p-1.5">
                    <div className="flex flex-col pl-2">
                      <span className="text-[10px] font-bold text-amber-800 dark:text-amber-400 uppercase tracking-wider mb-0.5">
                        Style
                      </span>
        ...
        <div className="space-y-4">
          {reports.map((report) => {
            ...
            const isReporter =
              empId && String(report.reporter_emp_id) === String(empId);
            const notYetAssignedReport =
              !report.receiver_emp_id &&
            ...
            const reportHasColorEdit =
              !!(
                report.colorEditedByWarehouseAt &&
                report.colorEditedByWarehouseBy
              ) ||
              !!(
                report.notificationHistory?.length &&
                report.notificationHistory.some(
                  (e) => e.type === "COLOR_UPDATE",
                )
              );
            const reportHasAdminEdit =
              !!report.editedByAdminAt ||
              !!(
                report.notificationHistory?.length &&
                report.notificationHistory.some((e) => e.type === "ADMIN_EDIT")
              );
            ...
            const hasNotification =
              hasWarehouseNotification || hasAdminNotification;
            const notificationParts = [];
            if (notYetAssignedReport) {
              if (isReporter)
                notificationParts.push("Not yet assigned by warehouse.");
              else if (isWarehouseUser || isAdminUser)
                notificationParts.push(
                  "Not yet assigned – scan QR to receive.",
                );
            }
            if (reportHasColorEdit && (isReporter || isAdminUser)) {
              const lastColorEntry = report.notificationHistory
                ?.filter((e) => e.type === "COLOR_UPDATE")
                .pop();
              const name =
                report.colorEditedByWarehouseName ||
                report.colorEditedByWarehouseBy ||
                lastColorEntry?.userName ||
                lastColorEntry?.userId ||
                "Warehouse user";
              const prevN =
                (report.colorUncheckedByWarehouse &&
                  report.colorUncheckedByWarehouse.length) ||
                0;
              const currentN = (report.color && report.color.length) || 0;
              const totalPrev =
                lastColorEntry != null &&
                lastColorEntry.previousColorCount != null
                  ? lastColorEntry.previousColorCount
                  : prevN + currentN;
              const currentForMsg =
                lastColorEntry != null && lastColorEntry.newColorCount != null
                  ? lastColorEntry.newColorCount
                  : currentN;
              const removed = report.colorUncheckedByWarehouse?.length
                ? ` Removed: ${report.colorUncheckedByWarehouse.join(", ")}.`
                : lastColorEntry?.rejectedColors?.length
                  ? ` Removed: ${lastColorEntry.rejectedColors.join(", ")}.`
                  : "";
            ...
            if (reportHasAdminEdit) {
              const name =
                report.editedByAdminName || report.editedByAdminBy || "Admin";
        ...
        >
          ...
          >
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
              ...
              <span className="font-semibold uppercase tracking-wide text-gray-800 dark:text-white">
                {notificationReport.ymStyle || "—"}
              </span>
            </p>
            ...
            notificationReport.editedByAdminAt ? (
              <div className="space-y-3 text-sm">
                ...
                {(notificationReport.notificationHistory?.length > 0
                    ...
                    ? [
                        {
                          ...
                          previousColorCount:
                            notificationReport.color?.length || 0,
                    ...
                    : [
                        {
                          ...
                          userName:
                            notificationReport.colorEditedByWarehouseName,
                          userId: notificationReport.colorEditedByWarehouseBy,
                          previousColorCount:
                            (notificationReport.colorUncheckedByWarehouse
                              ?.length || 0) +
                            (notificationReport.color?.length || 0),
                          newColorCount: notificationReport.color?.length || 0,
                          rejectedColors:
                  ...
                  return (
                    <div
                      key={entry.at ? new Date(entry.at).getTime() : idx}
                      className={boxClass}
                    >
                      <p className={titleClass}>
                        {entry.type === "COLOR_UPDATE"
                          ? "Color update"
                          : entry.type === "ADMIN_EDIT"
                            ? "Admin edit"
                            : entry.type || "Update"}
                      </p>
              ...
              <div className="rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 p-4 text-center">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-500 mt-0.5">
                  {notificationReport.status
                    ? String(notificationReport.status)
                        .charAt(0)
                        .toUpperCase() +
                      String(notificationReport.status).slice(1)
                    : ""}
                </p>
              </div>
            )}
      ...
      {tab === "easy_scan" &&
        ...
        (() => {
          const report = reports.find(
            (r) => (r._id || r.id) === easyScanQRPopupId,
          );
