...
const GameAssignControl = ({ user }) => {
      ...
      try {
        ...
        if (currentId) {
          ...
          const response = await axios.post(
            `${API_BASE_URL}/api/assign-control`,
            payload,
          );
  ...
  const handleLoadAssignment = useCallback(
    (assignment) => {
      ...
      const safeAssignment = {
        preparedBy:
          typeof assignment.preparedBy === "object"
            ? assignment.preparedBy?.emp_id || assignment.preparedBy?._id
            : assignment.preparedBy,
        checkedBy:
          typeof assignment.checkedBy === "object"
            ? assignment.checkedBy?.emp_id || assignment.checkedBy?._id
            : assignment.checkedBy,
        approvedBy:
          typeof assignment.approvedBy === "object"
            ? assignment.approvedBy?.emp_id || assignment.approvedBy?._id
            : assignment.approvedBy,
        admin:
          typeof assignment.admin === "object"
            ? assignment.admin?.emp_id || assignment.admin?._id
            : assignment.admin,
        userWarehouse:
          typeof assignment.userWarehouse === "object"
            ? assignment.userWarehouse?.emp_id || assignment.userWarehouse?._id
            : assignment.userWarehouse,
        updatedBy:
          typeof assignment.updatedBy === "object"
            ? assignment.updatedBy?.emp_id || assignment.updatedBy?._id
      ...
      const firstUserId =
        safeAssignment.checkedBy ||
        safeAssignment.approvedBy ||
        safeAssignment.preparedBy ||
        safeAssignment.admin ||
        safeAssignment.userWarehouse;
      if (firstUserId) {
        // Use the same value format as userOptions (emp_id or name) so the Select displays correctly
        const matchedUser = users.find(
          (u) =>
            (u.emp_id || u.name) === firstUserId ||
            String(u.emp_id) === String(firstUserId),
        );
        const valueToSet = matchedUser
          ? matchedUser.emp_id || matchedUser.name
          : firstUserId;
      ...
      const checkField = (fieldValue) => {
        ...
        const val =
          typeof fieldValue === "object"
            ? fieldValue.emp_id || fieldValue.name
            : fieldValue;
        return (
          String(val) === String(userId) || String(val) === String(userName)
        );
      };
          ...
          <div className="buckets-row">
            {Object.entries(BUCKET_COLORS).map(([key, color]) => (
              <motion.div
                ...
                transition={{
                  delay: 0.3 + Object.keys(BUCKET_COLORS).indexOf(key) * 0.1,
                }}
              >
        ...
        >
          ...
          <div className="history-scroll-wrapper">
            ...
            <button
              ...
              style={{
                display: filteredAssignments.length > 5 ? "flex" : "none",
              }}
            >
              <LeftArrowIcon />
            </button>
              ...
              {filteredAssignments.map((assignment, index) => (
                ...
                >
                  ...
                  <div className="history-time">
                    ...
                    {activeAssignmentId === assignment._id && (
                      <span
                        ...
                        style={{ marginLeft: "auto", color: "#4ECDC4" }}
                      >
                        ●
                      </span>
                    )}
                  ...
                  <div className="history-badges">
                    <span className="badge prepared">
                      {getDisplayName(
                        assignment.preparedBy,
                        assignment.preparedByName,
                      )}
                    </span>
                    <span className="badge checked">
                      {getDisplayName(
                        assignment.checkedBy,
                        assignment.checkedByName,
                      )}
                    </span>
                    ...
                    <span className="badge admin">
                      {getDisplayName(assignment.admin, assignment.adminName)}
                    </span>
                    <span className="badge userWarehouse">
                      {getDisplayName(
                        assignment.userWarehouse,
                        assignment.userWarehouseName,
                      )}
                    </span>
                  </div>
                </motion.div>
