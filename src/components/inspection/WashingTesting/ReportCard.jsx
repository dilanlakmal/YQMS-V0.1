...
const getNameForView = (empId, storedName, users = []) => {
  ...
  const u = users.find(
    (user) =>
      String(user.emp_id) === String(empId) ||
      String(user.id) === String(empId),
  );
...
const ReportCard = ({
  ...
  const isCreator =
    (report.userSubmit && String(user?.emp_id) === String(report.userSubmit)) ||
    (report.reporter_emp_id &&
      String(user?.emp_id) === String(report.reporter_emp_id));
  ...
  const shouldHideEditButton =
    ...
    (!isAdminUser &&
      restrictEditStatuses &&
      restrictEditStatuses.includes(report.status));
  const shouldHideDeleteButton =
    ...
    (!isAdminUser &&
      restrictDeleteStatuses &&
      restrictDeleteStatuses.includes(report.status));
  ...
  const canReject =
    ...
    (report.status === "pending" || report.status === "" || !report.status);
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                  {hasWarehouseNotification && (
                    <button
                      ...
                      title={
                        notificationTitle || "Warehouse edited this report"
                      }
                    >
                  ...
                  {hasAdminNotification && (
                    ...
                    >
                      ...
                      {!notificationRead && (
                        <span
                          className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-violet-500 dark:bg-violet-400 ring-2 ring-white dark:ring-gray-800"
                          aria-hidden
                        />
                      )}
                    </button>
          ...
          <div className="flex gap-1.5 md:gap-2 flex-wrap">
            ...
            >
              ...
              <span className="hidden sm:inline">
                {printingReportId === reportId ? "Printing..." : "Print"}
              </span>
            </button>
            <button
            ...
            <div className="sm:col-span-2 lg:col-span-4 mt-1 pt-2 border-t border-gray-200 dark:border-gray-600">
              ...
              {report.rejectedAt && (
                <>
                  ...
                  <span className="text-gray-900 dark:text-white">
                    {new Date(report.rejectedAt).toLocaleDateString("en-GB", {
                      dateStyle: "medium",
                    })}
                  </span>
                </>
              ...
              {report.rejectedNotes && report.rejectedNotes.trim() && (
                <>
                  ...
                  <span
                    className="text-gray-700 dark:text-gray-300 italic truncate max-w-[200px] inline-block align-bottom"
                    title={report.rejectedNotes}
                  >
      ...
      {isExpanded && (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 mt-4">
            ...
            {report.status === "completed" &&
              (report.checkedBy || report.approvedBy) && (
                <>
                  <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">
                      Checked By
                    ...
                    <p className="text-sm text-gray-900 dark:text-white">
                      {(() => {
                        const name = getNameForView(
                          report.checkedBy,
                          report.checkedByName,
                          users,
                        );
                        return name
                          ? name !== report.checkedBy
                            ? `${name} (${report.checkedBy})`
                            : name
          ...
          {report.careSymbols &&
            (typeof report.careSymbols === "object"
              ? Object.keys(report.careSymbols).length > 0
              : report.careSymbols.length > 0) && (
              <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-3 bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                  {Object.entries(
                    typeof report.careSymbols === "string"
                      ? (() => {
                          try {
                            return JSON.parse(report.careSymbols);
                          } catch (e) {
                            return {};
                          }
                        })()
                    ...
                    const imageSource =
                      report.careSymbolsImages && report.careSymbolsImages[key]
                    ...
                    return (
                      <div
                        key={key}
                        className="relative group p-1 bg-white rounded-md flex items-center justify-center w-12 h-12"
                        title={key}
                      >
                        <img
                          src={imageSource}
          ...
          {(report.reportType === "EMB/Printing Testing" ||
            report.reportType === "HT Testing") &&
            (report.finalResult ||
              report.checkedBy ||
              report.checkedDate ||
              report.finalResults) && (
              <div className="mt-4 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p
                      className={`text-sm font-bold ${report.finalResult === "Accepted" || report.finalResults === "Accepted" ? "text-green-600" : "text-red-600"}`}
                    >
          ...
          {report.shrinkageRows &&
            Array.isArray(report.shrinkageRows) &&
            report.shrinkageRows.length > 0 && (
              <div className="mt-4">
                <p className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2">
                ...
                <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider">
                      <tr>
                        <th className="p-2 border-b dark:border-gray-700">
                          Point
                        </th>
                        <th className="p-2 border-b dark:border-gray-700 text-center">
                          Spec (B)
                        </th>
                        <th className="p-2 border-b dark:border-gray-700 text-center">
                          G1 (B)
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                      {report.shrinkageRows
                        ...
                        .map((row, idx) => (
                          ...
                          >
                            <td
                              className="p-2 font-medium text-gray-700 dark:text-gray-300 max-w-[150px] truncate"
                              title={row.location}
                            >
                              {row.location}
                            </td>
                            ...
                            <td className="p-2 text-center">
                              {row.passFail === "PASS" ? (
                                <span className="text-green-600 font-bold text-[10px] px-1.5 py-0.5 bg-green-50 dark:bg-green-900/20 rounded">
                                  PASS
                                </span>
                              ) : row.passFail === "FAIL" ? (
                                <span className="text-red-500 font-bold text-[10px] px-1.5 py-0.5 bg-red-50 dark:bg-red-900/20 rounded">
                                  FAIL
                                </span>
                              ) : (
          ...
          <ReportTimeline
            ...
            onEditInitialImages={
              isActionLocked || isRejected ? null : onEditInitialImages
            }
            onEditReceivedImages={
              isActionLocked || isRejected ? null : onEditReceivedImages
            }
            onEditCompletionImages={
              isActionLocked || isRejected ? null : onEditCompletionImages
            }
