...
export const fetchOrderDetailsAPI = async (ymStyle) => {
  ...
  // Process colors
  const colors =
    orderData.colors && Array.isArray(orderData.colors)
...
export const fetchYorksysOrderAPI = async (ymStyle) => {
  ...
  // Extract ETDs
  const allETDs = result.data.SKUData.map((sku) => sku.ETD)
  ...
  // Extract POs
  const allPOLines = result.data.SKUData.map((sku) => sku.POLine)
