...
const QRScannerModal = ({
  ...
  useEffect(() => {
    const handleScanSuccess = (e) => {
      ...
      const scannerEl = scannerElementId
        ? document.getElementById(scannerElementId)
        : null;
      ...
      if (overlay && rect) {
        ...
        if (
          qrBounds &&
          typeof qrBounds.x === "number" &&
          typeof qrBounds.y === "number" &&
          (video || scannerEl)
        ) {
          ...
          const padding = 24;
          const zoomSize = Math.min(
            maxFrameSize,
            Math.max(minFrameSize, Math.max(w, h) + padding),
          );
          const left = Math.max(
            0,
            Math.min(rect.width - zoomSize, centerX - zoomSize / 2),
          );
          const top = Math.max(
            0,
            Math.min(rect.height - zoomSize, centerY - zoomSize / 2),
          );
    ...
    return () =>
      window.removeEventListener("qr-scan-success", handleScanSuccess);
  }, [scannerElementId]);
      ...
      <div className="flex-1 flex items-center justify-center min-h-0 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 overflow-hidden relative">
        ...
        >
          ...
          >
            ...
            <div
              className={`absolute inset-0 blur-3xl rounded-2xl transition-colors duration-300 ${framePosition ? "bg-emerald-500/30" : "bg-blue-500/20"}`}
            />

            {/* Lock-on ring - appears when bracket locks on QR */}
            {framePosition && (
              <div
                className="absolute inset-2 rounded-lg border-2 border-emerald-400/90 animate-bracket-lock-ring pointer-events-none"
            ...
            <svg
              className={`absolute -top-1 -left-1 w-16 h-16 text-white transition-all ${framePosition ? "animate-bracket-lock-on" : isScanning ? "animate-corner-pulse" : ""}`}
              viewBox="0 0 80 80"
              fill="none"
              stroke="currentColor"
              strokeWidth="6"
            ...
            <svg
              className={`absolute -top-1 -right-1 w-16 h-16 text-white transition-all ${framePosition ? "animate-bracket-lock-on" : isScanning ? "animate-corner-pulse" : ""}`}
              viewBox="0 0 80 80"
              fill="none"
              stroke="currentColor"
              strokeWidth="6"
            ...
            <svg
              className={`absolute -bottom-1 -left-1 w-16 h-16 text-white transition-all ${framePosition ? "animate-bracket-lock-on" : isScanning ? "animate-corner-pulse" : ""}`}
              viewBox="0 0 80 80"
              fill="none"
              stroke="currentColor"
              strokeWidth="6"
            ...
            <svg
              className={`absolute -bottom-1 -right-1 w-16 h-16 text-white transition-all ${framePosition ? "animate-bracket-lock-on" : isScanning ? "animate-corner-pulse" : ""}`}
              viewBox="0 0 80 80"
              fill="none"
              stroke="currentColor"
              strokeWidth="6"
            ...
            {(isScanning || isProcessing) && !scanSuccess && (
              <div className="absolute inset-0 overflow-hidden rounded-lg">
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(to bottom, transparent, rgba(34, 197, 94, 0.9) 20%, rgba(34, 197, 94, 1) 50%, rgba(34, 197, 94, 0.9) 80%, transparent)",
                    boxShadow:
            ...
            {isProcessing && (
              <div className="absolute inset-0 flex flex-col items-center justify-end rounded-lg pb-6 pointer-events-none">
                <p
                  className={`text-lg font-medium drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] transition-colors ${framePosition ? "text-emerald-300" : "text-white"}`}
                >
