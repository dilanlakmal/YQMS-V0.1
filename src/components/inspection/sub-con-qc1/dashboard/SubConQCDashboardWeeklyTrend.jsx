  ...
  const processedData = useMemo(() => {
    ...
    filteredTrends.forEach((trend) => {
      ...
      if (!aggregationMap.has(key)) {
        aggregationMap.set(key, { ...rowData, weeklyData: {}, key });
      }

      const entry = aggregationMap.get(key);
      if (!entry.weeklyData[trend.weekRange]) {
        entry.weeklyData[trend.weekRange] = { qty: 0, checkedQty: 0 };
      }
      entry.weeklyData[trend.weekRange].qty += trend.qty;
      entry.weeklyData[trend.weekRange].checkedQty += trend.checkedQty;
    });

    let aggregatedRows = Array.from(aggregationMap.values());

    // 3. Get all unique weeks from the aggregated data and sort them
    const allWeeks = [
      ...new Set(aggregatedRows.flatMap((row) => Object.keys(row.weeklyData)))
    ].sort();

    // 4. Calculate Top N defects for each week
    const weeklyTopRowKeys = new Map();
    if (topN.value !== "All") {
      const topNValue = parseInt(topN.value, 10);
      allWeeks.forEach((week) => {
        const topRowsForWeek = aggregatedRows
          .map((row) => ({ key: row.key, qty: row.weeklyData[week]?.qty || 0 }))
          .filter((item) => item.qty > 0)
          .sort((a, b) => b.qty - a.qty)
          .slice(0, topNValue)
          .map((item) => item.key);
        weeklyTopRowKeys.set(week, new Set(topRowsForWeek));
      });
    }

    // 5. Filter rows to only include those that are "Top N" in at least one week
    const finalRows = aggregatedRows.filter((row) => {
      if (topN.value === "All") {
        return true;
      }
      for (const topKeysForAWeek of weeklyTopRowKeys.values()) {
        if (topKeysForAWeek.has(row.key)) {
          return true;
        }
      }
      return false;
    ...
    // 6. Create final pivot rows with sorting
    const rows = finalRows
      .map((row) => {
        const rowData = { ...row, weeks: {} };
        allWeeks.forEach((week) => {
          const data = row.weeklyData[week];
          const isTopDefectForWeek =
            topN.value === "All" ||
            (weeklyTopRowKeys.get(week) &&
              weeklyTopRowKeys.get(week).has(row.key));

          if (data && data.checkedQty > 0 && isTopDefectForWeek) {
            rowData.weeks[week] = {
              qty: data.qty,
              rate: (data.qty / data.checkedQty) * 100
            };
          } else {
            rowData.weeks[week] = null;
          }
        });
        return rowData;
      })
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
        <h3 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-2 md:mb-0">
          ...
          <span className="block text-sm font-normal text-indigo-500">
            {dateRangeText} - QC Weekly Defect Trend
          </span>
        </h3>
        ...
        <table className="min-w-full text-sm border-collapse">
          <thead className="sticky top-0 bg-gray-100 dark:bg-gray-700 z-20">
            <tr>
              ...
              {viewMode !== "line" && (
                <th
                  className="p-2 border-b border-r dark:border-gray-600 text-left font-semibold sticky z-10 bg-gray-100 dark:bg-gray-700 bg-opacity-95 backdrop-blur-sm w-32"
              ...
              <th
                className="p-2 border-b border-r dark:border-gray-600 text-left font-semibold sticky z-10 bg-gray-100 dark:bg-gray-700 bg-opacity-95 backdrop-blur-sm w-48"
                style={{
                  left:
                    viewMode === "default"
                      ? "224px"
              ...
              {processedData.headers.map((weekRange) => {
                const [start, end] = weekRange.split(" to ");
                return (
                  <th
                    key={weekRange}
                    className="p-2 border-b dark:border-gray-600 min-w-[120px] text-center font-semibold"
                  >
                    <div>{start}</div>
                    <div className="font-normal text-xs text-gray-500 dark:text-gray-400">
                      to {end}
                    </div>
                  </th>
                );
            ...
            {processedData.rows.length > 0 ? (
              processedData.rows.map((row) => (
                ...
                >
                  ...
                  {processedData.headers.map((week) => {
...
const SubConQCDashboardWeeklyTrend = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const res = await axios.get(
          `${API_BASE_URL}/api/subcon-qc-dashboard-weekly-trend`,
          { params }
        );
        setData(res.data);
      } catch (err) {
        console.error("Failed to fetch dashboard weekly trend data:", err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
      <div className="p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-md border dark:border-gray-700">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          ...
          <FilterControl label="End Date (Sun)">
            <DatePicker
...
export default SubConQCDashboardWeeklyTrend;
