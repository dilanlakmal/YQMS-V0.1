...
const QCWashingCheckpointForm = ({ onCheckpointAdded }) => {
      ...
      prev.map((checkpoint) => {
        if (checkpoint.id === mainId) {
          if (!subId) {
            return {
              ...checkpoint,
              options: [...checkpoint.options, newOption]
            };
          } else {
            return {
              ...checkpoint,
      ...
      prev.map((checkpoint) => {
        if (checkpoint.id === mainId) {
          ...
          if (!subId) {
            return {
              ...checkpoint,
              options: updateOptions(checkpoint.options)
            };
          } else {
            return {
              ...checkpoint,
              subPoints: checkpoint.subPoints.map((sub) =>
                sub.id === subId
                  ? { ...sub, options: updateOptions(sub.options) }
                  : sub
              )
            };
          }
      ...
      prev.map((checkpoint) => {
        if (checkpoint.id === mainId) {
          ...
          if (!subId) {
            return {
              ...checkpoint,
              options: filterOptions(checkpoint.options)
            };
          } else {
            return {
              ...checkpoint,
              subPoints: checkpoint.subPoints.map((sub) =>
                sub.id === subId
                  ? { ...sub, options: filterOptions(sub.options) }
                  : sub
              )
            };
          }
  ...
  const deleteCheckpoint = (mainId, subId = null) => {
    if (!subId) {
      setCheckpoints((prev) =>
        prev.filter((checkpoint) => checkpoint.id !== mainId)
      );
    } else {
      setCheckpoints((prev) =>
        prev.map((checkpoint) =>
          checkpoint.id === mainId
            ? {
                ...checkpoint,
                subPoints: checkpoint.subPoints.filter(
                  (sub) => sub.id !== subId
                )
              }
            : checkpoint
  ...
  const updateFailureImpact = (mainId, impact) => {
    setCheckpoints((prev) =>
      prev.map((checkpoint) =>
        checkpoint.id === mainId
          ? { ...checkpoint, failureImpact: impact }
          : checkpoint
      )
    );
  };
  ...
  const saveRemark = () => {
    ...
    setCheckpoints((prev) =>
      prev.map((checkpoint) => {
        if (checkpoint.id === mainId) {
          const updateOptionsWithRemark = (options) =>
            options.map((opt) => {
              if (opt.id === optionId) {
                return {
                  ...opt,
          ...
          if (!subId) {
            return {
              ...checkpoint,
              options: updateOptionsWithRemark(checkpoint.options)
            };
          } else {
            return {
              ...checkpoint,
              subPoints: checkpoint.subPoints.map((sub) =>
                sub.id === subId
                  ? { ...sub, options: updateOptionsWithRemark(sub.options) }
                  : sub
              )
            };
          }
  ...
  const removeRemark = (mainId, subId, optionId) => {
    setCheckpoints((prev) =>
      prev.map((checkpoint) => {
        if (checkpoint.id === mainId) {
          const updateOptionsRemoveRemark = (options) =>
            options.map((opt) => {
              if (opt.id === optionId) {
                return {
                  ...opt,
          ...
          if (!subId) {
            return {
              ...checkpoint,
              options: updateOptionsRemoveRemark(checkpoint.options)
            };
          } else {
            return {
              ...checkpoint,
              subPoints: checkpoint.subPoints.map((sub) =>
                sub.id === subId
                  ? { ...sub, options: updateOptionsRemoveRemark(sub.options) }
                  : sub
              )
            };
          }
  ...
  const prepareCheckpointData = (checkpoint) => {
    ...
    if (
      ...
      Array.isArray(checkpoint.subPoints) &&
      ...
      processedSubPoints = checkpoint.subPoints
        .filter((sub) => {
          ...
          const hasValidOptions = sub.options.some(
            (opt) => opt && opt.name && String(opt.name).trim().length > 0
          );

          console.log(
            `Sub point "${name}" has valid options: ${hasValidOptions}`
          );
          console.log(
            "Sub point options:",
            JSON.stringify(sub.options, null, 2)
          );
        ...
        .map((sub) => {
          ...
          const subPointData = {
            ...
            options: filterValidOptions(
              sub.options,
              sub.optionType || "passfail"
            )
          };
          console.log(
            "Mapped sub point data:",
            JSON.stringify(subPointData, null, 2)
          );
          return subPointData;
        });
    } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(
        "Valid checkpoints:",
        JSON.stringify(validCheckpoints, null, 2)
      );
      ...
      for (const checkpoint of validCheckpoints) {
        ...
        for (const subPoint of preparedData.subPoints || []) {
          console.log(
            "Validating sub point:",
            JSON.stringify(subPoint, null, 2)
          );
    ...
    return (
      <div className="grid grid-cols-1 gap-2 p-2 border rounded bg-gray-50 dark:bg-gray-700">
        {options.map((option, index) => (
          <div
            key={option.id}
            className="flex items-center space-x-2 p-2 border rounded bg-white dark:bg-gray-800"
          >
            ...
            <input
              ...
              value={option.name}
              onChange={(e) =>
                updateOption(mainId, subId, option.id, "name", e.target.value)
              }
        ...
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-200 dark:border-gray-600">
          <div className="flex items-center space-x-2 flex-1">
            ...
            <input
              ...
              onChange={(e) =>
                updateCheckpointName(checkpoint.id, null, e.target.value)
              }
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              <select
                ...
                onChange={(e) =>
                  changeOptionType(checkpoint.id, null, e.target.value)
                }
              ...
              {checkpoint.subPoints.map((subPoint, subIndex) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      onClick={() =>
                        deleteCheckpoint(checkpoint.id, subPoint.id)
                      }
                      className="p-1 text-red-500 hover:bg-red-100 dark:hover:bg-red-900 rounded"
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {subPoint.optionType === "custom" && (
                          <button
                            onClick={() =>
                              addOption(checkpoint.id, subPoint.id)
                            }
                            className="px-2 py-1 bg-blue-500 text-white rounded hover:bg-blue-600 text-xs"
                          >
                            + Option
  ...
  return (
    <div className="p-4 sm:p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg dark:shadow-gray-900/20">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
        Configure checkpoints with multiple sub-points and options. Each
        checkpoint can have multiple sub-points, and each can have custom
        options with optional remarks.
      </p>
      ...
      <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg mb-4">
        ...
        <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
          <div>
            • <strong>Main Point:</strong> Primary checkpoint (e.g., "Hand
            Feel")
          </div>
          <div>
            • <strong>Sub Points:</strong> Secondary checkpoints under main
            point (e.g., "Soft", "Smooth")
          </div>
          <div>
            • <strong>Options:</strong> Available choices for each point (e.g.,
            "Pass", "Fail", "Dry", "Rough")
          </div>
          <div>
            • <strong>Checkboxes:</strong> Def = Default option, Fail = Fail
            option
          </div>
          <div>
            • <strong>Remark:</strong> Optional multilingual remarks (English
            required, Khmer & Chinese optional)
          </div>
          <div>
            • <strong>Failure Impact:</strong> How sub-point failures affect
            main point result
          </div>
        </div>
      </div>
