...
const ExcelUploader = ({ label = "Excel File", onDataLoaded }) => {
  ...
  const detectDataType = (data) => {
    ...
    const hasDefectColumns = keys.some(
      (key) =>
        key.includes("疵点") ||
        key.includes("Defect") ||
        key.includes("缺陷") ||
    ...
    const hasWashingData = data.some((row) => {
      ...
      return (
        lineName.toLowerCase().includes("washing") ||
              ...
              <>
                ...
                <div className="flex items-center justify-center space-x-2 text-sm text-green-600 dark:text-green-400">
                  ...
                  <span className="font-medium">
                    ...
                    {dataType === "washing" &&
                      tableData.length !== filteredData.length &&
                      ` (${
                        tableData.length - filteredData.length
                      } non-washing records filtered out)`}
                  </span>
                </div>
              </>
            ) : (
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    <h4 className="font-bold text-white">
                      {getDataTypeLabel(dataType)} Preview
                    </h4>
                    <p className="text-purple-100 text-sm">
                      First {Math.min(PREVIEW_ROW_LIMIT, filteredData.length)}{" "}
