...
const SizeMeasurementTable = ({ data = [] }) => {
  ...
  const hierarchy = useMemo(() => {
    return data.reduce((acc, curr) => {
      ...
      if (!acc[orderNo])
        acc[orderNo] = {
          colors: {},
          totals: { pcs: 0, pts: 0, pass: 0, fail: 0 },
        };
      if (!acc[orderNo].colors[color])
        acc[orderNo].colors[color] = {
          sizes: [],
          totals: { pcs: 0, pts: 0, pass: 0, fail: 0 },
        };
  ...
  const currentOrderKeys = orderKeys.slice(
    startIndex,
    ...
    <div className="max-w-[1600px] mx-auto mt-8 bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-gray-800 transition-all">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-4">
        <div>
          <h3 className="text-xl font-black flex items-center gap-3 text-slate-800 dark:text-white uppercase">
            <Ruler className="text-indigo-600" /> Measurement Breakdown by
            Styles
          </h3>
            ...
            <div key={orderNo} className="mb-2">
              ...
              >
                <div className="col-span-4 flex items-center gap-3">
                  <span className="text-[9px] font-black text-slate-300 w-4">
                    {startIndex + idx + 1}
                  </span>
                  {isOrderExpanded ? (
                  ...
                  <span className="font-black text-slate-800 dark:text-white uppercase truncate">
                    {orderNo}
                  </span>
                </div>
                ...
                <div className="col-span-1 text-center font-black text-emerald-600">
                  {order.totals.pass}
                </div>
                <div className="col-span-1 text-center font-black text-rose-500">
                  {order.totals.fail}
                </div>
                ...
                <div className="col-span-2 text-right">
                  <span className="px-3 py-1 bg-white dark:bg-gray-700 rounded-lg text-[10px] font-black shadow-sm border border-slate-100 dark:border-gray-600">
                    {(
                      (order.totals.pass / (order.totals.pts || 1)) *
                      100
                    ).toFixed(1)}
                    %
                  </span>
              ...
              {/* LEVEL 2: COLOR ROWS */}
              {isOrderExpanded &&
                Object.keys(order.colors).map((colorName) => {
                    ...
                    <div key={colorName} className="ml-8 mt-1">
                      ...
                      >
                        <div className="col-span-4 flex items-center gap-3">
                          ...
                          <span className="text-xs font-bold text-slate-600 dark:text-gray-300 uppercase truncate">
                            {colorName}
                          </span>
                        </div>
                        ...
                        <div className="col-span-1 text-center text-xs font-bold text-emerald-500">
                          {color.totals.pass}
                        </div>
                        <div className="col-span-1 text-center text-xs font-bold text-rose-400">
                          {color.totals.fail}
                        </div>
                        <div className="col-span-2"></div>
                        <div className="col-span-2 text-right text-[10px] font-bold text-slate-400">
                          {(
                            (color.totals.pass / (color.totals.pts || 1)) *
                            100
                          ).toFixed(1)}
                          %
                        </div>
                      ...
                      {isColorExpanded && (
                        <div className="ml-8 mt-1 space-y-1">
                          {color.sizes.map((sizeData, sIdx) => (
                            <div
                              key={sIdx}
                              className="grid grid-cols-12 items-center px-6 py-2 bg-slate-50/30 dark:bg-gray-800/20 rounded-lg border-l-2 border-indigo-400 group hover:bg-indigo-50/30 transition-colors"
                            >
                              <div className="col-span-4 flex items-center gap-3">
                                <Layers
                                  size={12}
                              ...
                              <div className="col-span-1 text-center text-xs font-medium">
                                {sizeData.totalCheckedPcs}
                              </div>
                              <div className="col-span-1 text-center text-xs font-medium">
                                {sizeData.totalCheckedPoints}
                              </div>
                              <div className="col-span-1 text-center text-xs font-bold text-emerald-600">
                                {sizeData.totalPass}
                              </div>
                              <div className="col-span-1 text-center text-xs font-bold text-rose-500">
                                {sizeData.totalFail}
                              </div>
                              <div className="col-span-1 text-center text-xs text-orange-400 font-bold">
                                {sizeData.totalPlusFail}
                              </div>
                              <div className="col-span-1 text-center text-xs text-blue-400 font-bold">
                                {sizeData.totalMinusFail}
                              </div>
      ...
      <div className="mt-8 flex flex-col md:flex-row justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest gap-4 border-t border-slate-50 dark:border-gray-800 pt-6">
        <div>
          ...
          {Math.min(startIndex + itemsPerPage, orderKeys.length)} of{" "}
          {orderKeys.length} Unique Orders
        </div>
