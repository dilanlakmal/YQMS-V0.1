      ...
      <div className="space-y-4">
        {dbData?.passRateByOrder?.map((item, idx) => (
          <div key={idx} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-gray-800/50 rounded-2xl border border-slate-100 dark:border-gray-700">
            ...
            <div className={`text-lg font-black ${item.avgPassRate < 90 ? 'text-rose-500' : 'text-emerald-500'}`}>{item.avgPassRate?.toFixed(1)}%</div>
          </div>
        ))}
      </div>
    </div>
        ...
        {dbData?.passRateByReportType?.map((item, idx) => (
          <div key={idx} className="p-4 border border-slate-100 dark:border-gray-700 rounded-3xl flex flex-col items-center justify-center text-center bg-white dark:bg-gray-900 shadow-sm">
            <div className="relative w-16 h-16 flex items-center justify-center mb-3">
              <svg className="w-full h-full -rotate-90">
                ...
                <circle 
                  ...
                  className={item.avgPassRate >= 95 ? 'text-emerald-500' : 'text-rose-500'} 
                  strokeLinecap="round" 
                />
              </svg>
            ...
            <p className="text-[10px] font-bold text-slate-400 mt-1">
              {item.passReports}/{item.totalReports} <span className="text-[8px]">PASS</span>
            </p>
          </div>
        ))}
      </div>
        ...
        {dbData?.passRateByDate?.map((item, idx) => (
          <div key={idx} className="flex items-center gap-4">
            ...
            <div className="flex-1 h-2 bg-slate-100 dark:bg-gray-800 rounded-full overflow-hidden">
              <div className={`h-full transition-all duration-1000 ${item.avgPassRate > 95 ? 'bg-emerald-500' : 'bg-amber-500'}`} style={{ width: `${item.avgPassRate}%` }} />
            </div>
            <div className="text-[11px] font-black text-slate-700 dark:text-gray-300 w-10 text-right">{Math.round(item.avgPassRate)}%</div>
          </div>
        ))}
      </div>
    </div>
