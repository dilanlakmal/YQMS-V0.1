    ...
    <div className="max-w-[1600px] mx-auto mb-8 bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-slate-200 dark:border-gray-800 overflow-hidden">
      ...
      <div className="p-6 border-b border-slate-100 dark:border-gray-800 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-slate-50/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">
              Independent Factory Defect Ranks
            </h2>
        ...
        <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-gray-800 p-2 rounded-2xl shadow-inner border border-slate-200">
          ...
          <div className="flex items-center gap-2 pl-1">
            <span className="text-[10px] font-black text-slate-400 uppercase">Custom Rank:</span>
          ...
          <thead>
            <tr>
              ...
              {factories.map((factory) => (
                <th key={factory} className="px-6 py-5 border-b border-l border-slate-100 dark:border-gray-800 bg-slate-50/30">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
            {rows.map((row, idx) => (
              <tr key={idx} className="group hover:bg-slate-50/80 transition-colors">
                ...
                {/* Data Cells */}
                {factories.map((factory) => (
                  <td key={factory} className="px-6 py-5 border-l border-slate-100 dark:border-gray-800">
                    {row[factory] ? (
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="flex gap-2">
                          <div className="flex flex-col items-center px-3 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100">
                            <span className="text-[9px] font-black text-blue-400 uppercase leading-none mb-1">Qty</span>
                            <span className="text-xs font-black text-blue-700 dark:text-blue-300">{row[factory].qty}</span>
                          </div>
                          <div className="flex flex-col items-center px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-100">
                            <span className="text-[9px] font-black text-emerald-400 uppercase leading-none mb-1">Pcs</span>
                            <span className="text-xs font-black text-emerald-700 dark:text-emerald-300">{row[factory].pcs}</span>
                          </div>
                        </div>
                      </div>
                    ) : (
                      <div className="text-center text-slate-200 font-medium italic text-[10px] uppercase tracking-widest">---</div>
                    )}
                  </td>
                ))}
              </tr>
      ...
      {factories.length === 0 && (
        <div className="p-20 text-center bg-slate-50/30">
          ...
          <p className="text-slate-400 font-black uppercase tracking-widest text-xs">No defect ranking available</p>
        </div>
      )}
    </div>
  );
