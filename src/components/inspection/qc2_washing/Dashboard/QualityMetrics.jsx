          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-gray-800">
              {dbData?.styleColorMeasurement?.map((row, i) => (
                <tr
                  key={i}
                  className="group hover:bg-slate-50/50 dark:hover:bg-gray-800/50 transition-colors"
                >
                  <td className="py-4 text-sm font-black text-slate-700 dark:text-gray-200">
                    {row.style}
                  </td>
                  <td className="py-4">
                  ...
                  <td className="py-4 text-xs font-bold text-slate-500 uppercase tracking-tighter">
                    {row.color}
                  </td>
                  ...
                  <td className="py-4">
                    <div className="flex flex-col items-end gap-1">
                      <span
                        className={`text-sm font-black ${row.accuracy < 90 ? "text-rose-500" : "text-emerald-500"}`}
                      >
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-gray-800">
              {dbData?.styleColorDefects?.map((row, i) => (
                <tr
                  key={i}
                  className="group hover:bg-rose-50/10 dark:hover:bg-gray-800/50 transition-colors"
                >
                  <td className="py-4 text-sm font-black text-slate-700 dark:text-gray-200">
                    {row.style}
                  </td>
                  ...
                  <td className="py-4 text-center">
                    <div className="flex flex-col">
                      <span className="text-sm font-black text-slate-800 dark:text-gray-300">
                        {row.defectQty}
                      </span>
                      <span className="text-[9px] text-slate-400 font-bold uppercase">
                        In {row.inspectedQty} Checked Pcs
                      </span>
                    </div>
                  </td>
                  <td className="py-4 text-right">
                    ...
                    <div
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-black text-xs ${row.defectRate > 0 ? "bg-rose-100 text-rose-600" : "bg-emerald-100 text-emerald-600"}`}
                    >
