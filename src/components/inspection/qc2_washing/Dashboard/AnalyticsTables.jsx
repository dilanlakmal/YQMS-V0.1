...
const AnalyticsTables = ({ dbData, defectLimit, setDefectLimit }) => {
  ...
  const topFailingPoints =
    dbData?.pointFailureSummary?.slice(0, pointLimit) || [];

  // Calculate totals for percentage calculations using complete dataset
  const totalAllDefects =
    dbData?.summary?.totalDefects ??
    dbData?.defectSummary?.reduce(
      (sum, d) => sum + (d.totalDefectQty || 0),
      0,
    ) ??
    0;
  const totalAllFails =
    dbData?.pointFailureSummary?.reduce(
      (sum, p) => sum + (p.totalFail || 0),
      0,
    ) || 0;
            ...
            <table className="w-full border-collapse">
              <thead className="sticky top-0 bg-white dark:bg-gray-900 z-10">
                <tr className="text-xs font-black text-slate-400 uppercase tracking-widest border-b-2 border-slate-200 dark:border-gray-700">
                  <th className="pb-2 pt-2 px-2 text-left border-r border-slate-200 dark:border-gray-700">
                    Defect
                  </th>
                  <th className="pb-2 pt-2 px-2 text-center border-r border-slate-200 dark:border-gray-700">
                    Pcs
                  </th>
                  <th className="pb-2 pt-2 px-2 text-center border-r border-slate-200 dark:border-gray-700">
                    Qty
              ...
              <tbody>
                {displayedDefects.map((d, i) => {
                  const defectPercentage =
                    totalAllDefects > 0
                      ? ((d.totalDefectQty / totalAllDefects) * 100).toFixed(1)
                      : "0.0";
                  return (
                    <tr
                      key={i}
                      className="group hover:bg-slate-50 dark:hover:bg-gray-800/50 transition-colors border-b border-slate-100 dark:border-gray-800"
                    >
                      <td className="py-2 px-2 text-sm font-bold text-slate-700 dark:text-gray-300 border-r border-slate-100 dark:border-gray-800">
                        {d._id}
                      </td>
                      ...
                      <td className="py-2 px-2 text-center text-sm font-black text-rose-600 border-r border-slate-100 dark:border-gray-800">
                        {d.totalDefectQty}
                      </td>
          ...
          <table className="w-full">
            <thead className="sticky top-0 bg-white dark:bg-gray-900 z-10">
              <tr className="text-sm font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 dark:border-gray-800">
                ...
                <th className="pb-3 text-center px-1">
                ...
                <th className="pb-3 text-center">
                  <Sigma size={10} className="inline mr-1" />
                  Total
                </th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-gray-800">
              {topFailingPoints.map((p, i) => {
                const failPercentage =
                  totalAllFails > 0
                    ? ((p.totalFail / totalAllFails) * 100).toFixed(1)
                    : "0.0";
                return (
                  <tr
                    key={i}
                  ...
                  >
                    <td className="py-4 text-sm font-black text-slate-700 dark:text-gray-300">
                      <span className="block max-w-[200px]" title={p._id}>
                        {p._id}
                      </span>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {p.affectedSizes?.slice(0, 4).map((sz) => (
                          <span
                            key={sz}
                            className="text-sm bg-slate-100 dark:bg-gray-800 px-1 rounded text-slate-400 font-bold"
                          >
                            {sz}
                          </span>
                        ))}
              ...
              {topFailingPoints.length === 0 && (
                <tr>
                  <td
                    colSpan="5"
                    className="py-12 text-center text-slate-300 text-xs italic"
                  >
                    No measurement failures found
                  </td>
                </tr>
              )}
            </tbody>
