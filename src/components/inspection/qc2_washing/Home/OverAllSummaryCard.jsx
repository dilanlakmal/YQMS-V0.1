...
const OverAllSummaryCard = ({ summary, recordId, onSummaryUpdate }) => {
  ...
  const calculateOverallResult = () => {
    ...
    if (summary.inspectionDetails?.checkpointInspectionData) {
      const hasInspectionFail =
        summary.inspectionDetails.checkpointInspectionData.some(
    ...
    if (isSOP) {
      ...
      if (
        (summary.totalDefectCount || 0) === 0 &&
        (summary.rejectedDefectPcs || 0) === 0
      ) {
        overallResult = "Pass";
      } else {
        overallResult = "Fail";
  ...
  useEffect(() => {
    const saveUpdatedSummary = async () => {
      ...
      try {
        ...
        if (updatedSummary.overallFinalResult !== summary.overallFinalResult) {
          console.log(
            "OverAllSummaryCard: Overall result changed to:",
            updatedSummary.overallFinalResult
          );
        }
    ...
    summary?.orderNo,
    summary?.color,
    summary?.totalCheckedPcs,
    summary?.rejectedDefectPcs,
    summary?.totalDefectCount,
    summary?.defectRate,
    summary?.defectRatio,
    onSummaryUpdate
  ]);
