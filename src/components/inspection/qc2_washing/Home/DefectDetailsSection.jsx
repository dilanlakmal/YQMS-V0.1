...
const DefectDetailsSection = ({
  ...
  if (aql?.acceptedDefect === 0) {
    ...
    const totalDefects = Object.values(defectsByPc).reduce(
      (total, pcDefects) => {
        return (
          total +
          pcDefects.reduce(
            (pcTotal, defect) => pcTotal + (parseInt(defect.defectQty) || 0),
            0
          )
        );
      },
    ...
    setDefectsByPc((prev) => ({
      ...
      [pc]: prev[pc].map((d) =>
        d.id === defectId
          ? {
              ...d,
              defectImages: (d.defectImages || []).filter(
                (_, i) => i !== imageIndex
              )
            }
          : d
                ...
                acc[pc.pcNumber] = (pc.pcDefects || []).map((defect, index) => {
                  ...
                  return {
                    ...
                    defectName: defectObj
                      ...
                      : defect.defectName,
                    defectQty:
                      defect.defectQty === 0 ? 0 : defect.defectQty || 0,
                    isBodyVisible: true,
                    defectImages: (defect.defectImages || []).map((imgStr) => ({
                      file: null,
                      preview: normalizeImageSrc(imgStr),
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center space-x-4">
              <label className="w-28 text-sm font-medium dark:text-gray-300">
                Wash Qty:
              </label>
              <input
                type="number"
                ...
                <div className="md:col-span-2 bg-blue-50 dark:bg-slate-800 border border-blue-200 dark:border-slate-600 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <span
                        className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${statusColorClass}`}
                      >
                        {actualDefectStatus || "N/A"}
                      </span>
                    </div>
          ...
          <div className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
            ...
            {Object.keys(defectsByPc).map((pc) => (
              ...
              >
                {(Array.isArray(defectsByPc[pc]) ? defectsByPc[pc] : []).map(
                  (defect, index) => (
                    <div
                      key={index}
                      className="relative bg-gray-200 dark:bg-gray-800 rounded-lg shadow-md flex-shrink-0 p-3 mb-3 w-full"
                    >
                      <button
                      ...
                      <h5 className="text-sm font-medium dark:text-gray-300 mb-2">
                        Defect Details
                      </h5>
                      {defect.isBodyVisible && (
                        <>
                                ...
                                <button
                                  onClick={() =>
                                    handleDefectChange(
                                      ...
                                      Math.max(
                                        0,
                                        (parseInt(defect.defectQty, 10) || 0) -
                                          1
                                      )
                                    )
                                  }
          ...
          <div>
            <label
              htmlFor="comment"
              className="text-md font-semibold mb-2 block dark:text-white"
            >
              Comments
            </label>
            <textarea
