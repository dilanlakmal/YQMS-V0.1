...
// Helper to get month key from DD/MM/YYYY (e.g., "04/2025")
const getMonthKey = (dateStr) => {
  const [day, month, year] = dateStr.split("/").map(Number);
  return `${month.toString().padStart(2, "0")}/${year}`;
};

const QCSunriseMonthlyTrend = ({ filters }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (!queryParams.startDate || !queryParams.endDate) {
        ...
        queryParams.startDate = new Date(today.setFullYear(today.getFullYear() - 1))
          .toISOString()
  ...
  useEffect(() => {
    ...
    const monthsSet = new Set(summaryData.map((d) => getMonthKey(d.inspectionDate)));
    ...
    const tableRows = buildRows(hierarchy, groupingFields, sortedMonths);
    setRows(tableRows);
  ...
  const buildHierarchy = (data, groupingFields) => {
    if (groupingFields.length === 0) {
      const monthMap = {};
      data.forEach((doc) => {
        const monthKey = getMonthKey(doc.inspectionDate);
        if (!monthMap[monthKey]) {
          monthMap[monthKey] = { CheckedQty: 0, totalDefectsQty: 0, DefectArray: [] };
        }
        monthMap[monthKey].CheckedQty += doc.CheckedQty || 0;
        monthMap[monthKey].totalDefectsQty += doc.totalDefectsQty || 0;
        if (doc.DefectArray) {
          doc.DefectArray.forEach((defect) => {
            const existing = monthMap[monthKey].DefectArray.find(
              (d) => d.defectName === defect.defectName
            );
            if (existing) {
              existing.defectQty += defect.defectQty || 0;
            } else {
              monthMap[monthKey].DefectArray.push({ ...defect });
            }
          });
        }
      });
  ...
  const buildRows = (
    ...
    if (currentFieldIndex < groupingFields.length) {
      ...
      Object.keys(hierarchy)
        ...
        .forEach((value) => {
          ...
          months.forEach((month) => {
            const sum = getSumForGroup(subHierarchy, month);
            groupData[month] =
      ...
      const monthMap = hierarchy;
      const defectNames = new Set();
      Object.values(monthMap).forEach((doc) => {
  ...
  // Sum CheckedQty and totalDefectsQty for a group on a specific month
  const getSumForGroup = (currentHierarchy, month) => {
    if (typeof currentHierarchy !== "object" || Array.isArray(currentHierarchy)) {
      const doc = currentHierarchy[month];
      return doc
    ...
    for (const key in currentHierarchy) {
      const subSum = getSumForGroup(currentHierarchy[key], month);
  ...
  const prepareExportData = () => {
    ...
    exportData.push(["Monthly Defect Trend Analysis", ...Array(uniqueMonths.length).fill("")]);
    ratesMap.set("0-0", 0);

    exportData.push(Array(uniqueMonths.length + 1).fill(""));
    ratesMap.set("1-0", 0);

    const headerRow = ["Group / Defect", ...uniqueMonths];
    ...
    rows.forEach((row) => {
      ...
      uniqueMonths.forEach((month, colIndex) => {
    ...
    uniqueMonths.forEach((month, colIndex) => {
      const monthData = summaryData.filter((d) => getMonthKey(d.inspectionDate) === month);
      const totalChecked = monthData.reduce((sum, d) => sum + (d.CheckedQty || 0), 0);
      const totalDefects = monthData.reduce((sum, d) => sum + (d.totalDefectsQty || 0), 0);
  ...
  const downloadPDF = () => {
    ...
    tablePlugin(doc, {
      ...
      didDrawPage: () => {
        doc.text("Monthly Defect Trend Analysis", 14, 10);
      },
    });
  ...
  return (
    <div className="mt-6 bg-white shadow-md rounded-lg p-6 overflow-x-auto">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-sm font-medium text-gray-900">Monthly Defect Trend</h2>
...
export default QCSunriseMonthlyTrend;