...
const WashingSpecsDataPreview = ({ moNo, allSpecData }) => {
  // State to track the currently active K-Value tab
  ...
  // ---------------------------------------------------------
  // Natural Sort (Handles K1, K10, KA, KAA correctly)
  // ---------------------------------------------------------
  const sortedSheets = useMemo(() => {
    return [...allSpecData].sort((a, b) => {
      // localeCompare with numeric: true handles:
      // 1. Numbers naturally (K2 comes before K10)
          ...
          {sortedSheets.map((sheet, index) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {/* Optional: Show row count badge inside button */}
                <span
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-100 dark:bg-gray-700/80">
              ...
              <tr>
                <th
                  rowSpan="2"
                  className="px-4 py-3 text-left text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider border-r border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700 sticky left-0 z-20 min-w-[180px]"
                >
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {currentSpec.rows.map((row, index) => (
                ...
                >
                  <td className="px-4 py-3 text-xs font-medium text-gray-800 dark:text-gray-200 border-r border-gray-200 dark:border-gray-700 sticky left-0 bg-white dark:bg-gray-800 group-hover:bg-indigo-50/50 dark:group-hover:bg-indigo-900/10 z-10 shadow-[1px_0_2px_0_rgba(0,0,0,0.05)]">
