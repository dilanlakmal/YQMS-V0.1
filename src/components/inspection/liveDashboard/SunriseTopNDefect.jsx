...
const SunriseTopNDefect = ({ filteredData, topN, setTopN }) => {
  const topNDefects = () => {
    const defectCounts = {};
    let totalCheckedQty = 0;

    // Check if filteredData exists and is an array
    ...
    filteredData.forEach((row) => {
      ...
      // Check if DefectArray exists and is an array
      if (row.DefectArray && Array.isArray(row.DefectArray)) {
        row.DefectArray.forEach((defect) => {
          if (!defectCounts[defect.defectName]) {
            defectCounts[defect.defectName] = { defectsQty: 0 };
          }
          defectCounts[defect.defectName].defectsQty += defect.defectQty;
        });
      }
    });
...
export default SunriseTopNDefect;
