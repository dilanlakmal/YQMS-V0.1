...
const RovingData = ({ refreshTrigger }) => {
  ...
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/api/defect-definitions`); // Adjust endpoint as needed
    
    if (response.data && Array.isArray(response.data)) {
      setDefectDefinitions(response.data);
    } else {
      console.warn('Defect definitions response is not an array:', response.data);
...
const getDefectNameByLanguage = (defectName, language = 'en', defectDefinitions = []) => {
  if (!defectName || !defectDefinitions.length) {
    return defectName;
  }

  // Try to find the defect in the definitions
  const defectDef = defectDefinitions.find(def => 
                  ...
                  REPETITION_KEYS.forEach((repKey) => {
                    ...
                    if (inspectionEvent) {
                      ...
                      const constructTooltipContent = (
                        ...
                        let defectsTextList = [];
                        let defectsHtml = "<strong>Defects Found:</strong> None";
                        const defectsList = [];
                            ...
                            (garment, garmentIndex) => {
                              garment.defects.forEach((defect, defectIndex) => {
                                ...
                                defectsTextList.push(
                                  `  Garment ${garmentIndex + 1}, Defect ${defectIndex + 1}: ${localizedDefectName} (Qty: ${defect.count})`
                                );
                                ...
                                defectHtml += `<strong>Garment ${garmentIndex + 1}, Defect ${defectIndex + 1}:</strong> ${localizedDefectName} (Qty: ${defect.count})`;
                                defectHtml += `</div>`;
                        ...
                        const defectsText = defectsTextList.length > 0 ? "Defects Found:\n" + defectsTextList.join("\n") : "Defects Found: None";
                        if (defectsList.length > 0) {
                          ...
                          if (currentOperatorInspectionData.defectImages && currentOperatorInspectionData.defectImages.length > 0) {
                            ...
                            currentOperatorInspectionData.defectImages.forEach((image, imgIndex) => {
                              defectsHtml += `<img src="${image}" alt="Defect Image ${imgIndex + 1}" class="defect-image-clickable" style="max-width: 200px; max-height: 200px; margin: 4px; border: 1px solid #ccc; border-radius: 4px; cursor: pointer; transition: transform 0.2s; hover: transform: scale(1.05);" />`;
                            });
                            defectsHtml += `</div></div>`;
                          }
                        }
                          ...
                          html: `
                            <div style="text-align: left; font-family: monospace; font-size: 0.85rem; line-height: 1.4;">
                              ...
                              <div style="margin-bottom: 12px;">
                                <strong>Date:</strong> ${currentReportDetails.inspection_date || "N/A"}<br>
                                <strong>QC ID:</strong> ${qcId}<br>
                                <strong>Inspection Rep:</strong> ${qcRepName}<br>
                                <strong>Line No:</strong> ${currentReportDetails.line_no || "N/A"}<br>
                                <strong>MO No:</strong> ${currentReportDetails.mo_no || "N/A"}
                              </div>
                              <div style="margin-bottom: 12px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
                                <strong>Operator ID:</strong> ${currentOperatorInspectionData.operator_emp_id || "N/A"}<br>
                                <strong>Operator Name:</strong> ${currentOperatorInspectionData.operator_kh_name || currentOperatorInspectionData.operator_eng_name || "N/A"}<br>
                                <strong>Operation:</strong> ${currentOperatorInspectionData.operation_kh_name || currentOperatorInspectionData.operation_ch_name || "N/A"}<br>
                                <strong>Machine Code:</strong> ${currentOperatorInspectionData.ma_code || "N/A"}
                              </div>
                              <div style="margin-bottom: 12px; padding-top: 8px; border-top: 1px solid #e5e7eb;">
                                <strong>Type:</strong> ${currentOperatorInspectionData.type || "N/A"}<br>
                                <strong>Checked Qty:</strong> ${currentOperatorInspectionData.checked_quantity || "N/A"}<br>
                                <strong>SPI:</strong> ${currentOperatorInspectionData.spi || "N/A"}<br>
                                <strong>Meas:</strong> ${currentOperatorInspectionData.measurement || "N/A"}<br>
                                <strong>Total Defects (Op):</strong> ${currentTotalDefectsForOp}<br>
                                <strong>Overall Result (Op):</strong> ${currentOperatorInspectionData.qualityStatus || "N/A"}<br>
                                <strong>Overall Roving Status:</strong> ${currentOperatorInspectionData.overall_roving_status || "N/A"}
                              </div>
