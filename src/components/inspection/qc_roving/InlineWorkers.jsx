...
const LineWorkers = ({ onWorkerCountUpdated }) => {
  ...
  useEffect(() => {
    const fetchWorkers = async () => {
      try {
        ...
        setError(t('lineWorkersDb.fetchError', "Failed to fetch line summary data. Please try again later."));
        setLineSummaries([]);
  ...
  const handleEdit = (summary) => {
    ...
    setCurrentEditValue(String(summary.editedWorkerCount !== null && summary.editedWorkerCount !== undefined ? summary.editedWorkerCount : ''));
  };
  ...
  const handleSave = async (lineNoToSave) => {
    ...
    if (isNaN(newWorkerCount) || newWorkerCount < 0) {
      Swal.fire(t('lineWorkersDb.invalidCountTitle', "Invalid Count"), t('lineWorkersDb.invalidCountMsg', "Worker count must be a non-negative number."), "error");
      return;
    }
    ...
    try {
      ...
      await axios.put(`${API_BASE_URL}/api/line-sewing-workers/${lineNoToSave}`, {
        edited_worker_count: newWorkerCount, // Send as edited_worker_count
      });

      const updatedSummaries = lineSummaries.map(s =>
        s.lineNo === lineNoToSave ? { ...s, editedWorkerCount: newWorkerCount } : s
      );
      setLineSummaries(updatedSummaries);
      // filteredLineSummaries will update via useEffect
      setEditingLineNo(null);
      Swal.fire(t('lineWorkersDb.saveSuccessTitle', "Success"), t('lineWorkersDb.saveSuccessMsg', "Worker count updated successfully."), "success");
      ...
      Swal.fire(
        ...
        err.response?.data?.message || t('lineWorkersDb.saveErrorMsg', "Failed to update worker count."), 
        "error"
      );
    }
  };
  ...
  useEffect(() => {
    ...
    if (filters.lineNo) {
      currentSummaries = currentSummaries.filter(summary =>
        summary.lineNo && String(summary.lineNo).toLowerCase().includes(filters.lineNo.toLowerCase())
      );
    }
  ...
  if (loading) {
    return <div className="text-center p-4">{t('lineWorkers.loading', "Loading line data...")}</div>;
  }
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-semibold mb-6 text-gray-700">{t('lineWorkers.title', "Inline Worker Summary by Line")}</h2>
      <div className="overflow-x-auto bg-white shadow-md rounded-lg">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                {t('lineWorkers.headerLineNo', "Line No")}
                <input
                  type="text"
                  name="lineNo"
                  value={filters.lineNo}
                  onChange={handleFilterChange}
                  placeholder={t('lineWorkers.filterPlaceholderLineNo', "Filter by Line No...")}
                  className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                />
              </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                {t('lineWorkers.headerWorkerCount', "Number of Workers")}
                </th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                {t('lineWorkers.headerEditedCount', "Edited Count")}
                </th>
          ...
          {filteredLineSummaries.length > 0 ? (
              filteredLineSummaries.map((summary, index) => (
                <tr key={summary.lineNo || index} className="hover:bg-gray-50 transition-colors duration-150">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{summary.lineNo}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{summary.realWorkerCount}</td>
                      ...
                      {editingLineNo === summary.lineNo ? (
                        ...
                        <>
                          <span className="min-w-[30px] text-center">
                            {summary.editedWorkerCount !== null && summary.editedWorkerCount !== undefined
              ...
              <tr>
                <td colSpan="4" className="px-6 py-4 text-center text-sm text-gray-500"> {/* Adjusted colSpan */}
                {t('lineWorkers.noResults', "No line summaries found matching your criteria.")}
                </td>
              </tr>
            )}
          </tbody>
