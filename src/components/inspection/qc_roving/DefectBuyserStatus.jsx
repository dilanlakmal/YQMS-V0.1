...
const DefectBuyerStatus = () => {
  ...
  const handleCheckboxChange = (defectCode, buyerName, statusType) => {
    setDefectStatuses((prevStatuses) => {
      ...
      if (isChecked) {
        newStatuses[defectCode][buyerName].defectStatus = currentDefectStatuses.filter(
          (c) => c !== statusType
        );
      } else {
        newStatuses[defectCode][buyerName].defectStatus = [
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {allDefects.length === 0 && (
                <tr>
                  ...
                  >
                    {t(
                      'defectBuyerStatus.noDefectsFound',
                      'No defects found.'
                    )}
                  </td>
                </tr>
                  ...
                  {allBuyers.map((buyer) => (
                    <React.Fragment key={`${defect.code}-${buyer}`}>
                            ...
                            (classification) => (
                              ...
                              >
                                <input
                                  ...
                                  checked={
                                    defectStatuses[defect.code]?.[buyer]
                                      ?.defectStatus?.includes(
                                        classification
                                      ) || false
                                  }
                                  onChange={() =>
                      ...
                      <td className="px-2 py-3 align-top border-r border-gray-300">
                        ...
                        >
                          <option value="Minor">
                            {t('defectBuyerStatus.classifications.minor', 'Minor')}
                          </option>
                          <option value="Major">
                            {t('defectBuyerStatus.classifications.major', 'Major')}
                          </option>
