...
const YPivotQASectionsHeader = () => {
  ...
  const [editData, setEditData] = useState({ MainTitle: "", Options: [] });
  const [isAddingNew, setIsAddingNew] = useState(false);
  const [newRowData, setNewRowData] = useState({ MainTitle: "", Options: [] });
  ...
  const handleAddOption = () => {
    ...
    setEditData({
      ...editData,
      Options: [...editData.Options, { OptionNo: newOptionNo, Name: "" }]
    });
  };
  ...
  const handleShowAddNewRow = () => {
    setIsAddingNew(true);
    setNewRowData({ MainTitle: "", Options: [{ OptionNo: 1, Name: "" }] });
  };

  const handleCancelAddNew = () => {
    setIsAddingNew(false);
    setNewRowData({ MainTitle: "", Options: [] });
  };
  ...
  const handleAddNewOption = () => {
    ...
    setNewRowData({
      ...newRowData,
      Options: [...newRowData.Options, { OptionNo: newOptionNo, Name: "" }]
    });
  };
  ...
  const handleConfirmAddNew = async () => {
    ...
    const validOptions = newRowData.Options.filter((opt) => opt.Name.trim());
    if (validOptions.length === 0) {
      Swal.fire(
        "Validation Error",
        "At least one option with a name is required",
        "warning"
      );
      return;
    }
            ...
            <div>
              ...
              <div className="space-y-2">
                {newRowData.Options.map((option) => (
                  <div key={option.OptionNo} className="flex gap-2">
                    <input
                      type="text"
                      value={option.Name}
                      onChange={(e) =>
                      ...
                      placeholder={`Option ${option.OptionNo}`}
                      className="flex-1 p-2 border-2 border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 focus:ring-2 focus:ring-indigo-500"
                    />
                    <button
