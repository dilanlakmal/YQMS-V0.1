...
//   // --- 1. CRITICAL FIX: SYNC STATE FROM PROPS ---
//   // When switching tabs, 'savedState' changes completely. We must update local state.
//   useEffect(() => {
...
//     savedState.fullSpecsList,
//     savedState.selectedSpecsList,
//     savedState.sourceType,
//     savedState.isConfigured,
//     savedState.savedMeasurements,
//     savedState.orderSizes,
//     savedState.kValuesList
//   ]);
...
//   const updateParent = (updates) => {
//     if (onUpdateMeasurementData) {
//       onUpdateMeasurementData(updates);
//     }
//   };
...
//     initData();
//   }, [activeMoNo, measConfig]); // Dependency on measConfig triggers refetch when switching tabs
...
const YPivotQAInspectionMeasurementConfig = ({
  ...
  // FIX: Initialize from savedState if available
  const [selectedSize, setSelectedSize] = useState("");
  const [selectedKValue, setSelectedKValue] = useState("");
  ...
  useEffect(() => {
    ...
    // FIX: Restore selected K Value from saved state
  ...
  //   const stateKey = JSON.stringify({
  //     fullSpecsList: savedState.fullSpecsList?.length,
  //     selectedSpecsList: savedState.selectedSpecsList?.length,
  //     sourceType: savedState.sourceType,
  //     isConfigured: savedState.isConfigured,
  //     savedMeasurements: savedState.savedMeasurements?.length,
  //     orderSizes: savedState.orderSizes?.length,
  //     kValuesList: savedState.kValuesList?.length,
  //     lastSelectedKValue: savedState.lastSelectedKValue,
  //     lastSelectedSize: savedState.lastSelectedSize
  //   });
  ...
  //   console.log(
  //     `[MeasConfig] Syncing state for stage: ${measConfig}`,
  //     savedState
  //   );
  ...
  //   // FIX: Restore selected K Value and Size from saved state
  //   if (savedState.lastSelectedKValue !== undefined) {
  //     setSelectedKValue(savedState.lastSelectedKValue);
  //   }
  //   // Only restore size if not locked (completed)
  //   if (
  //     savedState.lastSelectedSize !== undefined &&
  //     !savedState.lastSizeWasCompleted
  //   ) {
  //     setSelectedSize(savedState.lastSelectedSize);
  //   }
  // }, [savedState, measConfig]);
  ...
  // FIX: Enhanced updateParent to persist selection state
  ...
  // FIX: Only clear size when group changes, NOT kValue
  useEffect(() => {
    setSelectedSize("");
  ...
  const fetchMeasurementSpecs = async (type, moNo, currentSizes) => {
    ...
    try {
      ...
      // FIX: Auto-select first K value if available and none selected
      ...
      updateParent(
        {
          ...
          kValuesList: newKValues,
          orderSizes: currentSizes,
          // FIX: Also persist the auto-selected K value
          lastSelectedKValue:
  ...
  // FIX: Persist K value after saving measurement
  const handleSaveMeasurement = (data) => {
    const enhancedData = {
      ...data,
      groupId: activeGroup?.id,
      ...
      qcUser: activeGroup?.activeQC,
      stage: measConfig, // Save stage from current config
      kValue: measConfig === "Before" ? selectedKValue : "" // FIX: Always save kValue for Before
    };
    ...
    // FIX: Update parent with measurement AND persist K value
    updateParent({
  ...
  // FIX: Handle K value change with parent persist
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            ...
            • {selectedSpecsList.length} Critical Points
            {/* FIX: Show current K value in header for debugging */}
            {measConfig === "Before" && selectedKValue && (
                ...
                {measConfig === "Before" && kValuesList.length > 0 && (
                  <div>
                    <label className="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5 flex items-center gap-2">
                      ...
                      {/* FIX: Show current selection status */}
                      {selectedKValue && (
