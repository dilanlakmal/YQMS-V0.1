...
const YPivotQATemplatesMeasurementSelection = () => {
  ...
  // --- Display Mode State ---
  const [displayMode, setDisplayMode] = useState("selected"); // "selected" or "all"
  ...
  const fetchMeasurementSpecs = async (type) => {
    ...
    try {
      ...
      // ==========================================================================
      // IMPORTANT: Use selected specs if from qa_sections, otherwise use all
      // ==========================================================================
  ...
  const handleSaveConfig = async (selectedIds) => {
    // ==========================================================================
    // Filter fullSpecsList by IDs to get ALL K value variants
    // Each measurement point may have multiple entries (one per K value)
    // with DIFFERENT spec values for sizes
    // ==========================================================================
    ...
    // ==========================================================================
    // IMPORTANT: Send ALL specs in allSpecs (original data)
    // and ALL selected specs in selectedSpecs (includes all K value variants)
    // ==========================================================================
    const payload = {
      moNo: selectedOrder.moNo,
      allSpecs: fullSpecsList, // Original full list
  ...
  const handleSaveMeasurement = (data) => {
    if (editingMeasurementIndex !== null) {
      ...
      updated[editingMeasurementIndex] = {
        ...data,
        displayMode: displayMode
      };
      setSavedMeasurements(updated);
      setEditingMeasurementIndex(null);
      setEditingMeasurementData(null); // Clear editing data
    } else {
      // Add new measurement
      setSavedMeasurements([
        ...savedMeasurements,
  ...
  const handleStartMeasuring = () => {
    setEditingMeasurementIndex(null);
    setEditingMeasurementData(null); // Clear any existing editing data
    setIsGridOpen(true);
  };
      ...
      <div className="max-w-5xl mx-auto mt-4 sm:mt-8 space-y-4 sm:space-y-6 px-4 sm:px-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                ...
                {selectedSpecsList.length} Points Selected
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
            ...
            {/* K Value Selection (Only for Before Wash) */}
          ...
          <div className="mb-6">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                Selected Only ({selectedSpecsList.length})
              </button>
              <button
              ...
              >
                ...
                Display All ({fullSpecsList.length})
              </button>
            </div>
          </div>
          ...
          {canSelectSizeAndK && !selectedSize && (
        ...
        {selectedSpecsList.length > 0 && (
          <div className="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
            <h4 className="text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">
              Configured Measurement Points Preview
            </h4>
            <div className="max-h-48 overflow-y-auto space-y-2">
              {selectedSpecsList.slice(0, 10).map((spec, idx) => (
                <div
                  key={spec.id || idx}
              ...
              {selectedSpecsList.length > 10 && (
                <p className="text-xs text-gray-500 text-center py-2">
                  +{selectedSpecsList.length - 10} more points...
                </p>
              )}
            </div>
          </div>
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 pb-20 animate-fadeIn">
      ...
      <YPivotQATemplatesMeasurementGridModal
        ...
        onClose={() => {
          setIsGridOpen(false);
          setEditingMeasurementIndex(null);
          setEditingMeasurementData(null); // Clear editing data on close
        }}
        specsData={displayMode === "all" ? fullSpecsList : selectedSpecsList}
