...
//   const isMultiOrderMode = specsByOrder && specsByOrder.length > 1;

//   // Merged full spec list (for "All" tab) — deduped by point name
//   const effectiveFullSpecs = useMemo(() => {
//     if (!isMultiOrderMode) return specsData;
//     const seen = new Set();
//     const merged = [];
//     specsByOrder.forEach(({ filteredFullSpecs = [] }) => {
//       filteredFullSpecs.forEach((s) => {
//         if (!seen.has(s.MeasurementPointEngName)) {
//           seen.add(s.MeasurementPointEngName);
//           merged.push(s);
//         }
//       });
//     });
//     return merged.length > 0 ? merged : specsData;
//   }, [isMultiOrderMode, specsByOrder, specsData]);

//   // Merged selected/critical spec list — deduped by point name
//   const effectiveSelectedSpecs = useMemo(() => {
//     if (!isMultiOrderMode) return selectedSpecsList;
//     const seen = new Set();
//     const merged = [];
//     specsByOrder.forEach(({ filteredSelectedSpecs = [] }) => {
//       filteredSelectedSpecs.forEach((s) => {
//         if (!seen.has(s.MeasurementPointEngName)) {
//           seen.add(s.MeasurementPointEngName);
//           merged.push(s);
//         }
//       });
//     });
//     return merged.length > 0 ? merged : selectedSpecsList;
//   }, [isMultiOrderMode, specsByOrder, selectedSpecsList]);
...
//   useEffect(() => {
//     if (isOpen && effectiveFullSpecs.length > 0) {
...
//         } else {
//           setAllMeasurements(initializeMeasurements(effectiveFullSpecs, 1));
//           setAllQty(1);
...
//         } else {
//           setCriticalMeasurements(
//             initializeMeasurements(effectiveSelectedSpecs, 2),
//           );
//           setCriticalQty(2);
...
//     let specs =
//       displayMode === "all" ? effectiveFullSpecs : effectiveSelectedSpecs;
...
//     (spec) => {
//       if (!effectiveSelectedSpecs || effectiveSelectedSpecs.length === 0)
//         return false;
//       return effectiveSelectedSpecs.some(
...
//                   {isMultiOrderMode &&
//                     (() => {
//                       const owners = specsByOrder
//                         .filter((entry) =>
//                           (entry.filteredSelectedSpecs || []).some(
//                             (s) =>
//                               s.MeasurementPointEngName ===
//                               spec.MeasurementPointEngName,
//                           ),
//                         )
//                         .map((e) => e.orderNo);
//                       // Only show badge if this spec belongs to a subset (not all orders)
//                       if (
//                         owners.length > 0 &&
//                         owners.length < specsByOrder.length
//                       ) {
//                         return (
...
//                               <span
//                                 key={o}
//                                 className="text-[8px] bg-amber-100 text-amber-700 px-1 rounded"
//                               >
//                                 {o}
//                               </span>
//                             ))}
