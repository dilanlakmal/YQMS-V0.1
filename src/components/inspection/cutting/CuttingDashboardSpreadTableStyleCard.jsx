...
// A small, reusable KPI card component (This is identical and can be reused)
...
// Detailed Issues Table Component (This is identical and can be reused)
...
// Display Component that handles the table (This is identical and can be reused)
...
// The main card component, adapted for Spread Tables
const SpreadTableStyleCard = ({ styleData }) => {
  const { theme } = useTheme();
  ...
  const allTablesData = useMemo(() => {
    ...
    styleData.issuesBySpreadTable.forEach((table) => {
      // <-- MODIFIED
  ...
  }, [styleData.issuesBySpreadTable]); // <-- MODIFIED

  const displayedData =
    activeTab === "All"
      ? allTablesData
      : styleData.issuesBySpreadTable.find((t) => t.spreadTable === activeTab)
          ?.issuesByGarmentType || []; // <-- MODIFIED

  const getTabClass = (tabName) => {
    const base =
      "px-3 py-1 text-sm font-semibold rounded-md transition-colors duration-200";
    if (tabName === activeTab) return `${base} bg-blue-600 text-white`;
    return `${base} ${
      theme === "dark"
  ...
  return (
    ...
    >
      <div>
        ...
        >
          {styleData.spreadTables.sort().join(" | ")} {/* <-- MODIFIED */}
        </p>
      </div>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4 flex-grow flex flex-col">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {styleData.spreadTables.sort().map(
            (
              table // <-- MODIFIED
            ) => (
              <button
...
// The main wrapper component for the new card type
const CuttingDashboardSpreadTableStyleCard = ({ data, title }) => {
  const { theme } = useTheme();
  if (!data || data.length === 0) {
    return (
      ...
      >
        <p className={theme === "dark" ? "text-gray-400" : "text-gray-600"}>
          No spread table-specific measurement data to display.
        </p>
      </div>
    );
  }
  return (
    <div className="mt-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {data.map((styleData) => (
          <SpreadTableStyleCard key={styleData.moNo} styleData={styleData} />
        ))}
      </div>
    </div>
  );
...
export default CuttingDashboardSpreadTableStyleCard;
