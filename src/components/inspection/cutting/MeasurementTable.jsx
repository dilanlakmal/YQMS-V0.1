...
const MeasurementTable = ({
      ...
      usedPanelIndices.forEach((panelIndex) => {
        for (let colIndex = 0; colIndex < numColumns; colIndex++) {
          ...
          const anyMeasurementOutOfTolerance = measurementsForPartInColumn.some(
            (value) =>
              value !== null &&
              (value < tolerance.min || value > tolerance.max),
          );
  ...
  useEffect(() => {
    // DO NOT run this logic, otherwise it will wipe the restored tableData.
    if (selectedParts.length > 0 && measurementPoints.length === 0) {
      return;
    }
