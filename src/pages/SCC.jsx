...
import EMBReport from "../components/inspection/scc/EMBReport";
import FinalConsolidateReport from "../components/inspection/scc/FinalConsolidateReport";
...
const SCCPage = () => {
  ...
  const [embReportData, setEmbReportData] = useState({
    ...initialEMBReportState
  });
  ...
  const tabs = useMemo(
    () =>
      [
        ...
        {
          id: "embReport",
          labelKey: "scc.tabs.embReport",
          icon: <Palette size={16} />,
          formType: "EMBReport",
          data: embReportData,
          setter: setEmbReportData,
          component: EMBReport
        },
        {
          id: "finalConsolidate",
          labelKey: "scc.tabs.finalConsolidate",
          icon: <BookCheck size={16} />,
          formType: "FinalConsolidateContainer", // A unique type for this container
          data: {}, // No form data for this one
          setter: () => {}, // No setter needed
          component: FinalConsolidateReport, // Assign the new component
          disabled: false // Enable the tab
        }
    ...
    [
      ...
      elasticReportData,
      embReportData,
      t // Add t to dependency array for placeholder component
    ]
  );
  ...
  const handleFormSubmit = useCallback(
    async (formTypeToSubmit, formDataFromChild = null) => {
      ...
      switch (formTypeToSubmit) {
        ...
        case "submitElasticSlotInspection":
          ...
          break;
        case "EMBReport":
          endpoint = "/api/scc/emb-report";
          successMessageKey = "sccEMBReport.reportSavedSuccess";
          currentSetterForReset = setEmbReportData;
          initialStateForReset = initialEMBReportState;
          payloadToSend = formDataFromChild;
          break;
        default:
      ...
      try {
        ...
        // Add specific payload handling for EMB Report
        else if (formTypeToSubmit === "EMBReport" && payloadToSend) {
          const reportDataFromChild = payloadToSend;
          // Basic validation
          if (
            ...
            Swal.fire(
              t("scc.validationErrorTitle"),
              t("sccEMBReport.validation.fillBasicPayload"),
              "warning"
            );
            throw new Error("Validation failed for EMB Report.");
          }
          let finalDefectImageUrl = reportDataFromChild.defectImageUrl;
          if (reportDataFromChild.defectImageFile) {
            const imageTypeIdentifier = `embDefect-${reportDataFromChild.factoryName}-${reportDataFromChild.moNo}-${reportDataFromChild.color}-${reportDataFromChild.batchNo}`;
            const imgData = await uploadSccImage(
              reportDataFromChild.defectImageFile,
              reportDataFromChild,
              imageTypeIdentifier
          ...
          payloadToSend = {
            ...reportDataFromChild,
            defectImageUrl: finalDefectImageUrl,
            ...currentUserInfoWithTime // Add inspector details
          };
          delete payloadToSend.defectImageFile;
        }
      } catch (error) {
      ...
      try {
        ...
        if (
          ...
          } else if (formTypeToSubmit === "EMBReport") {
            // Reset image file for EMB report
            resetState.defectImageFile = null;
          } else if (formTypeToSubmit === "DailyTesting") {
            resetState.afterWashImageFile = null;
    ...
    [
      ...
      dailyHTQCFormData, //new
      dailyFUQCFormData, //new
      htInspectionReportData, //new
      elasticReportData, //new
      embReportData, //new
      setHtFormData,
      setFuFormData,
      setDailyTestingFormData,
      setHtInspectionReportData,
      setEmbReportData //new
    ]
  );
