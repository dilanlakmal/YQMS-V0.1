...
const OPAPage = () => {
  ...
  const [selectedOpaTaskNo, setSelectedOpaTaskNo] = useState(OPA_TASK_OPTIONS[0].value);
  ...
  const fetchBundleData = async (randomId) => {
    try {
      ...
      if (response.ok) {
        ...
        const countResponse = await fetch(
          ...
          if (!countResponse.ok) throw new Error(`Failed to check OPA scan count for bundle ${data.bundle_id} with task ${selectedOpaTaskNo}`);
        const countData = await countResponse.json(); // Expects { count: number }
        
        if (countData.count >= 3) {
          const taskLabel = OPA_TASK_OPTIONS.find(opt => opt.value === selectedOpaTaskNo)?.label || selectedOpaTaskNo;
        ...
        const defectSpecificTaskNo = GOOD_TO_DEFECT_OPA_TASK_MAP[selectedOpaTaskNo];
        if (!defectSpecificTaskNo) {
          console.error("Invalid selectedOpaTaskNo for defect mapping:", selectedOpaTaskNo);
          throw new Error(`Internal error: No defect task mapping for selected OPA task ${selectedOpaTaskNo}.`);
        }

        const countResponse = await fetch(
          `${API_BASE_URL}/api/check-opa-exists/${trimmedId}-${defectSpecificTaskNo}`
        );
        if (!countResponse.ok) throw new Error(`Failed to check OPA scan count for defect card ${trimmedId} with task ${defectSpecificTaskNo}`);
        const countData = await countResponse.json(); // Expects { count: number }

        if (countData.count >= 3) {
          const originalTaskLabel = OPA_TASK_OPTIONS.find(opt => opt.value === selectedOpaTaskNo)?.label || selectedOpaTaskNo;
   ...
   const handleAddRecord = useCallback(async () => {
    try {
      ...
      if (isDefectCard) {
        ...
        if (!taskNoOPA) {
          throw new Error(`Internal error: Could not determine defect task number for selected OPA task ${selectedOpaTaskNo}.`);
        }
      } else {
      ...
      if (!response.ok) throw new Error(t("opa.error.failed_to_save", "Failed to save OPA record"));
   ...
   }, [isDefectCard, opaRecordId, scannedData, passQtyOPA, user, selectedOpaTaskNo, t]);
  ...
  const filteredOpaRecords = useMemo(() => {
    ...
    return opaRecords.filter(record => {
      ...
      // QC ID filter
      if (filters.qcId && String(record.emp_id_opa ?? '').toLowerCase() !== String(filters.qcId).toLowerCase()) {
        return false;
      }
  ...
  const opaStats = useMemo(() => {
    if (!filteredOpaRecords || filteredOpaRecords.length === 0) {
      return { totalGarmentsOPA: 0, totalBundlesProcessed: 0, totalStyles: 0, task60Garments: 0, task61Garments: 0, task62Garments: 0};
    }

    let totalGarments = 0;
    const uniqueStyles = new Set();
    let task60 = 0, task61 = 0, task62 = 0;
  ...
  const userTodayStats = useMemo(() => {
    ...
    opaRecords.forEach(record => {
      if (record.emp_id_opa === user.emp_id && record.opa_updated_date === today) {
        const qty = Number(record.passQtyOPA) || 0;
        total += qty;
        const taskNo = String(record.task_no_opa);
    ...
    return { task60: t60, task61: t61, task62: t62, task103: t103, task104: t104, task105: t105, total: total };
          ...
          <div className="space-y-6">
            {!loading && user && (
              <UserStatsCard
                ...
                stats={{
                  tasks: [
                    { label: t("opa.stats.card.task60", "OPA Task (T60)"), value: userTodayStats.task60},
            ...
            <div className="bg-white shadow-lg rounded-xl p-4 sm:p-6 flex items-center justify-between">
              ...
              <div className="flex items-center">
                <label htmlFor="autoAddCheckboxOpa" className="text-sm font-medium text-gray-700 mr-2">
              ...
              <div className="flex items-center">
                <label htmlFor="opaTaskSelector" className="text-sm font-medium text-gray-700 mr-2">
                ...
                {selectedOpaTaskNo && (
                  <span 
                    ...
                    title={OPA_TASK_OPTIONS.find(opt => opt.value === selectedOpaTaskNo)?.label}
                  >
                    ({OPA_TASK_OPTIONS.find(opt => opt.value === selectedOpaTaskNo)?.label})
                  </span>
                )}
              </div>
            </div>
                ...
                <div className="flex-1 flex items-center space-x-3 pr-3">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-semibold text-gray-700">
                      {loadingData ? "..." : opaStats.totalGarmentsOPA.toLocaleString()}
                    </p>
                  </div>
                </div>
                <div className="flex flex-col justify-around pl-3 border-l border-gray-200 space-y-1 min-w-[120px]">
                  <div><p className="text-xs text-gray-500 font-medium">{t("opa.stats.task60", "Task 60")}</p><p className="text-base font-semibold text-gray-700">{loadingData ? "..." : opaStats.task60Garments.toLocaleString()}</p></div>
                  <div><p className="text-xs text-gray-500 font-medium">{t("opa.stats.task61", "Task 61")}</p><p className="text-base font-semibold text-gray-700">{loadingData ? "..." : opaStats.task61Garments.toLocaleString()}</p></div>
                  <div><p className="text-xs text-gray-500 font-medium">{t("opa.stats.task62", "Task 62")}</p><p className="text-base font-semibold text-gray-700">{loadingData ? "..." : opaStats.task62Garments.toLocaleString()}</p></div>
