...
const BGradeDefect = () => {
  ...
  const onScanSuccess = async (decodedText) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch defect data: ${errorText || response.statusText}`);
      }
      ...
      if (!bundleResponse.ok) {
       const errorText = await bundleResponse.text();
        throw new Error(`Failed to fetch associated bundle data for this defect card. Please ensure the defect card is correctly linked to an inspection bundle. Error details: ${errorText || bundleResponse.statusText}`);
      }
      ...
      const newGarments = prevData.garments.map((garment) => {
        if (garment.garmentNumber === garmentNumber) {
          const newDefects = garment.defects.map((defect) =>
            defect.id === defectId ? { ...defect, decision: newDecision } : defect
          );
  ...
  const handleSave = async () => {
    ...
    const bGradeArrayWithConfirmations = scannedData.garments.flatMap(garment => {
      const isRejected = garment.defects.some(d => d.decision === 'rejected');
      const isAccepted = garment.defects.length > 0 && garment.defects.every(d => d.decision === 'pass');
    ...
    const decisionsMade = bGradeArrayWithConfirmations.some(d => d.confirmation !== "B-Grade");
        ...
        {scannedData && (
          <div>
            <Grid container spacing={3} className="mb-6">
              <Grid item xs={12} sm={4}>
                <Card sx={{ backgroundColor: "#e3f2fd" }}>
                  <CardContent>
                    <Typography variant="h5" component="div">All Garments</Typography>
                    <Typography variant="h3" color="text.secondary">{allGarmentsCount}</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={12} sm={4}>
                <Card sx={{ backgroundColor: "#c8e6c9" }}>
                  <CardContent>
                    <Typography variant="h5" component="div">Pass Garments</Typography>
                    <Typography variant="h3" color="text.secondary">{passGarmentsCount}</Typography>
                  </CardContent>
                </Card>
              </Grid>
              <Grid item xs={12} sm={4}>
                <Card sx={{ backgroundColor: "#ffcdd2" }}>
                  <CardContent>
                    <Typography variant="h5" component="div">Reject Garments</Typography>
                    <Typography variant="h3" color="text.secondary">{rejectGarmentsCount}</Typography>
                  </CardContent>
                </Card>
              </Grid>
            </Grid>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4">
              <h3 className="text-xl font-semibold text-gray-800 mb-4">B-Grade Defect Details</h3>
              <div className="flex justify-between mb-6 bg-gray-100 p-2 rounded">
                <p><strong>MO No:</strong> {scannedData.moNo}</p>
                <p><strong>Line No:</strong> {scannedData.lineNo}</p>
                <p><strong>Color:</strong> {scannedData.color}</p>
                <p><strong>Size:</strong> {scannedData.size}</p>
              </div>
              <div className="flex justify-end mb-4">
                <FormControl variant="outlined" sx={{ minWidth: 200 }}>
                  <InputLabel id="language-select-label">Select Language</InputLabel>
                  <Select labelId="language-select-label" value={language} onChange={handleLanguageChange} label="Select Language">
                ...
                <TableBody>
                  {scannedData.garments.map((garment) =>
                    { // Start of block for calculating rowBgColor per garment
                      const isGarmentRejected = garment.defects.some((d) => d.decision === "rejected");
                      const isGarmentAccepted = garment.defects.every((d) => d.decision === "pass");
                      let rowBgColor = "inherit"; // Default neutral
                      if (isGarmentRejected) {
                        rowBgColor = "#ffcdd2"; // Light red for rejected
                      } else if (isGarmentAccepted && garment.defects.length > 0) { // Ensure there are defects to be accepted
                        rowBgColor = "#c8e6c9"; // Light green for accepted
                      }

                      return garment.defects.map((defect, index) => (
                        <TableRow key={defect.id} sx={{ backgroundColor: rowBgColor }}>
                        {index === 0 && (
                          <TableCell rowSpan={garment.defects.length} className="px-2 py-1 text-sm text-gray-700 border border-gray-200 align-top">
                        ...
                        <TableCell className="px-2 py-1 text-sm text-gray-700 border border-gray-200">
                          <div className="flex space-x-2">
                            <Button
                              variant={defect.decision === "pass" ? "contained" : "outlined"} // Contained if 'pass', outlined otherwise
                              color={defect.decision === "pass" ? "success" : "inherit"} // Green if 'pass', inherit (gray) for outlined
                              onClick={() => handleDecisionChange(garment.garmentNumber, defect.id, "pass")}
                            >
                              Accept
                            </Button>
                            <Button
                              variant={defect.decision === "rejected" ? "contained" : "outlined"} // Contained if 'rejected', outlined otherwise
                              color={defect.decision === "rejected" ? "error" : "inherit"} // Red if 'rejected', inherit (gray) for outlined
                              onClick={() => handleDecisionChange(garment.garmentNumber, defect.id, "rejected")}
                            >
                              Reject
                            </Button>
                          </div>
