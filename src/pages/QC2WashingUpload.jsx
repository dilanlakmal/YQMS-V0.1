...
import ExcelUploader from '../components/inspection/QC2_Washing_Upload/ExcelUpload';
...
const processWashingDataForSave = (data) => {
  const groupedData = data.reduce((acc, row) => {
    const inspectionDate = row['日期'] || row['Date'] || new Date().toISOString().split('T')[0];
  ...
  const handleFinalResult = async (skipLoading, skipSetState) => {
    ...
    const hasRequiredData = (outputData.length > 0 && defectData.length > 0) || washingData.length > 0;
    ...
    try {
      ...
      if (washingResult.success) {
        Swal.fire({
          icon: 'success',
          title: 'Saved!',
          text: `Saved ${washingResult.data?.total || washingToSave.length} washing records successfully!`,
  ...
  const hasAnyData = outputData.length > 0 || defectData.length > 0 || washingData.length > 0;
  const hasRequiredData = (outputData.length > 0 && defectData.length > 0) || washingData.length > 0;
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          ...
          <button
            ...
            disabled={loadingSave || loadingProcess || loadingFetch || !hasRequiredData}
