...
// Main Schema
const AfterIroningSchema = new mongoose.Schema({
  date: { type: Date, default: Date.now },
  orderNo: { type: String, required: true },
  before_after_wash: { type: String, default: 'After Ironing' },
  ...
  washType: { type: String, default: 'Normal Ironing' },
  reportType: String,
  buyer: String,
  factoryName: String,
  colorName: String,
...
AfterIroningSchema.index({ orderNo: 1, date: 1, color: 1 });
AfterIroningSchema.index({ userId: 1, createdAt: -1 });
AfterIroningSchema.index({ status: 1 });
AfterIroningSchema.index({ 'inspectionDetails.checkpointInspectionData.checkpointId': 1 });

AfterIroningSchema.pre('save', function(next) {
...
export default (connection) => connection.model("AfterIroning", AfterIroningSchema);
