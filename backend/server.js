...
//       Seq_No: [...new Set(outputRows.map((r) => Number(r.SeqNo)))],
...
//     // console.log(
//     // "📊 Fetching order headers with size names and shipping data..."
//     // );
...
app.get("/api/anf-measurement/order-details/:moNo", async (req, res) => {
  try {
    const { moNo } = req.params;
    // Querying the dt_orders collection using the ymProdConnection
    const order = await ymProdConnection.db
      .collection("dt_orders")
      .findOne({ Order_No: moNo });
...
app.post("/api/qc-washing/save-summary/:recordId", async (req, res) => {
  try {
    ...
    if (
      qcRecord.measurementDetails &&
      qcRecord.measurementDetails.measurement
    ) {
