...
export const getQcOutputFilters = async (req, res) => {
  try {
    ...
    // --- MODIFICATION: Fetch both QC IDs and Defect Names in parallel ---
...
export const getQcOutputInspectionData = async (req, res) => {
  try {
    // --- MODIFICATION: Destructure new defectNames query parameter ---
    ...
    // --- MODIFICATION: Add defect name filtering to the match stage ---
...
//     const qcIds = await QCWorkers.distinct("QC_ID", matchStage);
//     res.json({ qcIds: qcIds.sort() });
//   } catch (error) {
//     console.error("Error fetching QC Output filter options:", error);
//     res.status(500).json({ message: "Server error fetching filter options" });
//   }
