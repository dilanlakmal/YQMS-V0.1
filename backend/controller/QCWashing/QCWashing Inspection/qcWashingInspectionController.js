...
export const saveQCWashingDefectData = async (req, res) => {
  try {
    ...
    // 1. Map uploaded files to their fieldnames
    const fileMap = {};
    for (const file of req.files || []) {
    ...
    if (defectDetails.defectsByPc) {
      defectDetails.defectsByPc.forEach((pc, pcIdx) => {
        ...
        const isMultiOrBatch = isMulti || isBatch;
          ...
          if (defect.defectImages) {
            defect.defectImages = defect.defectImages.map((img, imgIdx) => {
              ...
              if (typeof img === "string") {
                ...
                return normalizeInspectionImagePath({ preview: img });
              }
...
export const getqcwashingOrderSizes = async (req, res) => {
  ...
  try {
    ...
    if (!orders || orders.length === 0) {
      ...
      const alternativeOrders = await collection
        .find({
          $or: [
            { Order_No: orderNo },
            { Style: orderNo },
...
function calculateMeasurementSizeSummary(measurementDetail) {
  ...
  const washType =
    measurementDetail.before_after_wash === "Before Wash"
      ? "beforeWash"
      : measurementDetail.before_after_wash === "After Wash"
      ? "afterWash"
...
export const saveQCWashingMeasurementData = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: isEdit
        ? "Measurement detail updated"
        : "Measurement detail saved",
