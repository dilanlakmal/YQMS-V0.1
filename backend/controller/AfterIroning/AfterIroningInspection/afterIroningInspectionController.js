...
export const saveAfterIroningOrderData = async (req, res) => {
  try {
    ...
    if (
      ...
      Array.isArray(formData.defectDetails.defectsByPc)
    ) {
      formData.defectDetails.defectsByPc.forEach((pc) => {
        if (Array.isArray(pc.pcDefects)) {
          pc.pcDefects.forEach((defect) => {
            if (
              defect.defectImages &&
              defect.defectImages.some((img) => typeof img === "object")
            ) {
              defect.defectImages = defect.defectImages.filter(
                (img) =>
                  typeof img === "string" &&
    ...
    if (
      formData.inspectionDetails &&
      Array.isArray(formData.inspectionDetails.checkpointInspectionData)
    ) {
      formData.inspectionDetails.checkpointInspectionData.forEach(
        (checkpoint) => {
          // Clean up main checkpoint images
          if (
            checkpoint.comparisonImages &&
            checkpoint.comparisonImages.some((img) => typeof img === "object")
          ) {
            checkpoint.comparisonImages = checkpoint.comparisonImages.filter(
              (img) =>
                typeof img === "string" &&
                (img.startsWith("http") || img.startsWith("/storage"))
            );
          ...
          if (Array.isArray(checkpoint.subPoints)) {
            checkpoint.subPoints.forEach((subPoint) => {
              if (
                ...
                subPoint.comparisonImages.some((img) => typeof img === "object")
              ) {
                subPoint.comparisonImages = subPoint.comparisonImages.filter(
                  (img) =>
                    typeof img === "string" &&
                    (img.startsWith("http") || img.startsWith("/storage"))
                );
    ...
    if (formData._id) {
      ...
      if (record) {
        ...
        const measurementDetailsUpdate =
          formData.measurementDetails &&
          !Array.isArray(formData.measurementDetails)
            ? formData.measurementDetails
...
export const saveAfterIroningInspectionData = async (req, res) => {
  try {
    ...
    const checkpointInspectionData = JSON.parse(
      req.body.checkpointInspectionData || "[]"
    );
    ...
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to create image directory." });
    }
      ...
      checkpointInspectionData.forEach((item, idx) => {
        ...
        Object.keys(fileMap).forEach((fieldName) => {
          ...
          if (mainPattern.test(fieldName)) {
            ...
            console.log(
              `Found main checkpoint image: ${fieldName} -> ${fileMap[fieldName]}`
            );
          }
        });
        ...
        console.log(
          `Main checkpoint ${idx} final images:`,
          mainCheckpointImages
        );
          ...
          item.subPoints.forEach((subPoint, subIdx) => {
            ...
            Object.keys(fileMap).forEach((fieldName) => {
              ...
              const subPattern = new RegExp(
                `^checkpointImages_${idx}_sub_${subIdx}_\\d+$`
              );
              if (subPattern.test(fieldName)) {
                subPointImages.push(fileMap[fieldName]);
                console.log(
                  `Found sub-point image: ${fieldName} -> ${fileMap[fieldName]}`
                );
              }
            });
            ...
            console.log(
              `Sub-point ${idx}-${subIdx} final images:`,
              subPointImages
            );
          });
        }
    ...
    console.log(
      "Final checkpoint data saved:",
      JSON.stringify(groupedCheckpointData, null, 2)
    );
  ...
  try {
    ...
    if (checkpointInspectionData) {
      checkpointInspectionData.forEach((item, idx) => {
        ...
        if (item.comparisonImages) {
          item.comparisonImages = item.comparisonImages
            .map((img, imgIdx) => {
        ...
        if (item.subPoints && Array.isArray(item.subPoints)) {
          item.subPoints.forEach((subPoint, subIdx) => {
            if (subPoint.comparisonImages) {
              subPoint.comparisonImages = subPoint.comparisonImages
                .map((img, imgIdx) => {
                  const newImageUrl =
                    fileMap[`checkpointImages_${idx}_sub_${subIdx}_${imgIdx}`];
                  if (newImageUrl) return newImageUrl;
                  if (
                    typeof img === "string" &&
                    (img.startsWith("http") || img.startsWith("/storage"))
  ...
  try {
      ...
      defectDetails.defectsByPc.forEach((pc, pcIdx) => {
        if (pc.pcDefects && Array.isArray(pc.pcDefects)) {
          pc.pcDefects.forEach((defect, defectIdx) => {
            ...
            Object.keys(fileMap).forEach((fieldName) => {
              const defectPattern = new RegExp(
                `^defectImages_${pcIdx}_${defectIdx}_\\d+$`
              );
              if (defectPattern.test(fieldName)) {
                defectImages.push(fileMap[fieldName]);
                console.log(
                  `Found defect image: ${fieldName} -> ${fileMap[fieldName]}`
                );
              }
            });
            ...
            console.log(
              `Defect ${pcIdx}-${defectIdx} final data:`,
              processedDefect
            );
          });
        }
    ...
    Object.keys(fileMap).forEach((fieldName) => {
      ...
      if (additionalPattern.test(fieldName)) {
        ...
        console.log(
          `Found additional image: ${fieldName} -> ${fileMap[fieldName]}`
        );
      }
    });

    // Process existing images
    const processedAdditionalImages = additionalImages
      .map((img) => {
        if (typeof img === "string" && img.trim() !== "") {
          if (img.startsWith("http") || img.startsWith("/storage")) {
            return img;
    ...
    if (
      !defectDetails.checkedQty &&
      !defectDetails.washQty &&
      ...
      if (currentRecord) {
        ...
        const totalDefectCount =
          defectDetails.defectsByPc?.reduce(
            (sum, pc) =>
              sum +
              (pc.pcDefects?.reduce(
                (defSum, defect) => defSum + (parseInt(defect.defectQty) || 0),
                0
              ) || 0),
            0
          ) || 0;
        ...
        if (aql && typeof aql.acceptedDefect === "number") {
          defectDetails.result =
            totalDefectCount <= aql.acceptedDefect ? "Pass" : "Fail";
        } else {
  ...
  try {
          ...
          pc.pcDefects.forEach((defect, defectIdx) => {
            ...
            Object.keys(fileMap).forEach((fieldName) => {
              ...
              if (defectPattern.test(fieldName)) {
                defectImages.push(fileMap[fieldName]);
                console.log(
                  `Found updated defect image: ${fieldName} -> ${fileMap[fieldName]}`
                );
              }
            });
            ...
            console.log(
              `Updated defect ${pcIdx}-${defectIdx} final data:`,
              processedDefect
            );
          });
        }
    ...
    Object.keys(fileMap).forEach((fieldName) => {
      ...
      if (additionalPattern.test(fieldName)) {
        additionalImages.push(fileMap[fieldName]);
        console.log(
          `Found updated additional image: ${fieldName} -> ${fileMap[fieldName]}`
        );
      }
    });
    ...
    console.log(
      "Final updated defect details saved:",
      JSON.stringify(defectDetails, null, 2)
    );
      ...
      if (order.OrderColors && Array.isArray(order.OrderColors)) {
        order.OrderColors.forEach((colorObj) => {
          if (
            colorObj &&
            colorObj.OrderQty &&
            Array.isArray(colorObj.OrderQty)
          ) {
...
export const getmeasurmentSpec = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Error fetching buyerspectemplate for orderNo:",
        orderNo,
        error
      );
    }
...
export const saveAfterIroningSummary = async (req, res) => {
  try {
    ...
    if (
      summary.overallFinalResult &&
      ["Pass", "Fail"].includes(summary.overallFinalResult)
    ) {
      ...
      newOverallFinalResult =
        passRate >= 95 && defectResult === "Pass" ? "Pass" : "Fail";
      console.log(
        "Recalculating overall result on backend:",
        newOverallFinalResult
      );
    }
    ...
    console.log("Backend final calculation result:", {
      ...
      condition2: `Defect Result "${defectResult}" === "Pass": ${
        defectResult === "Pass"
      }`,
...
function calculateMeasurementSizeSummary(measurementDetail) {
  ...
  const washType =
    measurementDetail.before_after_wash === "Before Wash"
      ? "beforeWash"
      : measurementDetail.before_after_wash === "After Ironing"
      ? "afterIroning"
  ...
  try {
    ...
    if (!measurementDetail.before_after_wash || !measurementDetail.kvalue) {
      return res.status(400).json({
        success: false,
        message:
          "before_after_wash and kvalue are required in measurementDetail"
      });
    }
    ...
    console.log(
      `Updating measurement for record ${recordId} (status: ${record.status})`
    );
    ...
    if (measurementIndex !== -1) {
      record.measurementDetails.measurement[measurementIndex] =
        measurementDetail;
      ...
      console.log(
        `Added new measurement for size ${measurementDetail.size}, kvalue ${measurementDetail.kvalue}`
      );
    }

    // Find existing summary index
    let summaryIndex =
      record.measurementDetails.measurementSizeSummary.findIndex(
        (s) =>
    ...
    const summaryData =
      measurementDetail.summaryData ||
      calculateMeasurementSizeSummary(measurementDetail);

    // Update or add summary
    if (summaryIndex !== -1) {
      record.measurementDetails.measurementSizeSummary[summaryIndex] =
        summaryData;
      ...
      console.log(
        `Added new summary for size ${measurementDetail.size}, kvalue ${measurementDetail.kvalue}`
      );
    }
...
export const getAfterIroningDefects = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch defects" });
  }
};
...
export const getAfterIroningChecklist = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch checklist" });
  }
};
...
export const getAfterIroningCheckpointDefinitions = async (req, res) => {
  try {
    ...
    console.error(
      "Failed to fetch After Ironing checkpoint definitions:",
      error
    );
    res
      .status(500)
      .json({
        success: false,
        message: "Failed to fetch checkpoint definitions"
      });
  }
};
  ...
  try {
    ...
    if (qcWashingRecord) {
      ...
      res.json({
        ...
        exists: false,
        message:
          "No QC Washing record found - order must be washed first before After Ironing inspection",
  ...
  try {
    ...
    if (qcWashingRecord.measurementDetails?.measurement) {
      qcWashingRecord.measurementDetails.measurement.forEach((measurement) => {
        if (
          measurement.before_after_wash === "beforeWash" ||
          measurement.before_after_wash === "Before Wash"
        ) {
          measurementData.beforeWash.push(measurement);
        } else if (
          measurement.before_after_wash === "afterWash" ||
          measurement.before_after_wash === "After Wash"
        ) {
