...
const runDownloadTaskUbuntu = async (jobId, params,  userId) => {
    ...
    try {
        const { startRange, endRange, downloadAll, startDate, endDate, factoryName, poNumber, styleNumber, language = 'english', includeDownloaded = false } = params;
...
export const cancelBulkDownload = async (req, res) => {
    try {
        ...
        // Close Puppeteer if it exists
                if (jobInfo.browser) {
                    try { await jobInfo.browser.close(); } catch (e) {
                ...
                if (jobInfo.driver) {
                    try { await jobInfo.driver.quit(); } catch (e) {
