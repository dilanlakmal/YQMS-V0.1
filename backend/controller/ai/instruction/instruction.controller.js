...
const instruction = createProductionModel(ymEcoConnection);
...
const createInstruction = async (
    ...
    const doc = await instruction.findOneAndUpdate(
        { documentId },
        { $set: payload },
        {
            upsert: true,
...
const getInstruction = async (prodId) => {
    const data = await instruction.findById(prodId);
    return data;
}

const getInstructionByDocId = async (docId) => {
    const data = await instruction.findOne({ documentId: docId });
    return data;
}

export default createInstruction;
export { updateInstruction, getInstruction, getInstructionByDocId, update };