...
export const getOrderDetailsWashing = async (req, res) => {
  try {
    const collection = ymProdConnection.db.collection("dt_orders");
    const yorksysCollection = ymProdConnection.db.collection("yorksys_orders");
    const specCollection = ymProdConnection.db.collection(
      "buyer_spec_templates",
    );

    let order = await collection.findOne({
      $or: [{ Order_No: req.params.mono }, { CustStyle: req.params.mono }],
    });
    ...
    if (source === "yorksys_orders") {
      const colors = [
        ...new Set(
          order.SKUData.filter((s) => s.Color).map((s) => s.Color.trim()),
        ),
      ];
      return res.json({
        success: true,
    ...
    const response = {
      ...
      sizeList: [
        ...new Set([
          ...
          ...anfSizes,
          ...Array.from(colorMap.values()).flatMap((c) =>
            Array.from(c.sizes.keys()),
          ),
        ]),
      ].filter(Boolean),
