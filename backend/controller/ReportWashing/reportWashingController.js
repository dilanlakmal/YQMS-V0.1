...
const ASSET_BASE_PATH = path.resolve(
  __backendDir,
  "../public/assets/Wash-bold",
);
...
const getModelByReportType = (reportType) => {
  switch (reportType) {
    case "Home Wash Test":
      return ReportHomeWash;
    case "Garment Wash Report":
      return ReportGarmentWash;
    case "HT Testing":
      return ReportHTTesting;
    case "EMB/Printing Testing":
      return ReportEMBPrinting;
    case "EMB Testing":
      return ReportEMBPrinting; // Legacy support
    case "Pulling Test":
      return ReportPullingTest;
    default:
      return ReportHomeWash; // Default to Home Wash
  }
};
...
const findReportById = async (id) => {
  ...
  if (
    mongoose.Types.ObjectId.isValid(idVal) &&
    String(new mongoose.Types.ObjectId(idVal)) === idVal
  ) {
...
export const saveReportWashing = async (req, res) => {
  try {
    ...
    if (req.files) {
      ...
      const imagesToProcess = Array.isArray(req.files)
        ? req.files
        : req.files.images || [];
      ...
      if (
        ...
        req.files.careLabelImage &&
        req.files.careLabelImage.length > 0
      ) {
    ...
    const parsedExFtyDate =
      typeof exFtyDate === "string" ? JSON.parse(exFtyDate) : exFtyDate;
    ...
    const parsedReportSampleSizes = parseReportSampleSizes(
      req.body.reportSampleSizes ?? req.body.sampleSize,
    );
    ...
    const parsedVisualAssessmentRows = safeParseJSON(
      req.body.visualAssessmentRows,
    );
    ...
    if (req.body.careSymbols) {
      try {
        parsedCareSymbols =
          typeof req.body.careSymbols === "string"
            ? JSON.parse(req.body.careSymbols)
            : req.body.careSymbols;
      } catch (e) {
    ...
    const reportData = {
      ...
      exFtyDate: Array.isArray(parsedExFtyDate)
        ? parsedExFtyDate
        : [parsedExFtyDate],
      reportSampleSizes: parsedReportSampleSizes,
      factory: (req.body.factory || "").trim(),
      sendToHomeWashingDate: sendToHomeWashingDate
        ? new Date(sendToHomeWashingDate)
        : new Date(),
      images: imagePaths,
    ...
    for (const key of ALLOWED_EXTRA) {
      ...
      if (val !== undefined && val !== null) {
        if (key === "fabricColor")
          reportData[key] = Array.isArray(val)
            ? val
            : typeof val === "string"
              ? val
                  .split(",")
    ...
    console.log(
      "Selected Model:",
      ReportModel.modelName,
      "Collection:",
      ReportModel.collection.name,
    );
...
export const getReportWashing = async (req, res) => {
  try {
    ...
    const countPromises = modelsToQuery.map((model) =>
...
export const updateReportWashing = async (req, res) => {
  try {
    ...
    if (
      updateData.reporter_emp_id === undefined &&
      req.body.userId !== undefined
    )
      updateData.reporter_emp_id = req.body.userId;
    if (
      updateData.reporter_name === undefined &&
      req.body.userName !== undefined
    )
      updateData.reporter_name = req.body.userName;

    // Find the right model and doc
    const { model: ReportModel, doc: existingReport } =
    ...
    if (updateData.color) {
      try {
        const parsedColor =
          typeof updateData.color === "string"
            ? JSON.parse(updateData.color)
            : updateData.color;
        updateData.color = Array.isArray(parsedColor)
          ? parsedColor
          : [parsedColor];
      } catch (error) {
    ...
    const prevColor = Array.isArray(existingReport.color)
      ? existingReport.color
      : [];
    ...
    const editedByWarehouse =
      req.body.editedByWarehouse === true ||
      req.body.editedByWarehouse === "true";
    ...
    if (editedByWarehouse && colorChanged) {
      ...
      updateData.colorEditedByWarehouseBy =
        req.body.editorUserId || req.body.editorEmpId || "";
      updateData.colorEditedByWarehouseName =
        req.body.editorUserName || req.body.editorName || "";
    ...
    if (!editedByWarehouse && colorChanged) {
      ...
      updateData.editedByAdminBy =
        req.body.editorUserId || req.body.editorEmpId || "";
      updateData.editedByAdminName =
        req.body.editorUserName || req.body.editorName || "";
      notificationHistoryEntry = {
        type: "ADMIN_EDIT",
        at: now,
    ...
    if (updateData.colorFastnessRows)
      updateData.colorFastnessRows = safeParseJSON(
        updateData.colorFastnessRows,
      );
    if (updateData.colorStainingRows)
      updateData.colorStainingRows = safeParseJSON(
        updateData.colorStainingRows,
      );
    if (updateData.shrinkageRows)
      updateData.shrinkageRows = safeParseJSON(updateData.shrinkageRows);
    if (updateData.visualAssessmentRows)
      updateData.visualAssessmentRows = safeParseJSON(
        updateData.visualAssessmentRows,
      );

    if (updateData.po) {
      try {
        const parsedPO =
          typeof updateData.po === "string"
            ? JSON.parse(updateData.po)
            : updateData.po;
    ...
    if (updateData.exFtyDate) {
      try {
        const parsedExFtyDate =
          typeof updateData.exFtyDate === "string"
            ? JSON.parse(updateData.exFtyDate)
            : updateData.exFtyDate;
        updateData.exFtyDate = Array.isArray(parsedExFtyDate)
          ? parsedExFtyDate
          : [parsedExFtyDate];
      } catch (error) {
        ...
        if (Array.isArray(rawSizes)) {
          ...
          try {
            ...
            parsed = Array.isArray(p)
              ? p.map((s) => String(s).trim()).filter(Boolean)
              : [];
          } catch (e) {
            parsed = rawSizes
              .split(",")
    ...
    if (updateData.careSymbols) {
      try {
        updateData.careSymbols =
          typeof updateData.careSymbols === "string"
            ? JSON.parse(updateData.careSymbols)
            : updateData.careSymbols;
        ...
        if (
          updateData.careSymbols &&
          typeof updateData.careSymbols === "object"
        ) {
    ...
    if (updateData.sendToHomeWashingDate) {
      updateData.sendToHomeWashingDate = new Date(
        updateData.sendToHomeWashingDate,
      );
    }
    ...
    if (req.files && req.files.images && req.files.images.length > 0) {
      ...
      for (const file of req.files.images) {
        try {
          ...
          const finalDiskPath = path.join(
            washingMachineTestUploadPath,
            filename,
          );
      ...
      if (updateData.imagesUrls) {
        try {
          const existingUrls =
            typeof updateData.imagesUrls === "string"
              ? JSON.parse(updateData.imagesUrls)
              : updateData.imagesUrls;
          ...
          if (
            ...
            existingReport.images.length > 0
          ) {
    ...
    if (
      req.files &&
      req.files.receivedImages &&
      req.files.receivedImages.length > 0
    ) {
      ...
      if (updateData.receivedImagesUrls) {
        try {
          const existingUrls =
            typeof updateData.receivedImagesUrls === "string"
              ? JSON.parse(updateData.receivedImagesUrls)
              : updateData.receivedImagesUrls;
          ...
          if (
            existingReport &&
            existingReport.receivedImages &&
            existingReport.receivedImages.length > 0
          ) {
            updateData.receivedImages = [
              ...existingReport.receivedImages,
              ...receivedImagePaths,
            ];
          } else {
    ...
    const completionFiles = req.files?.completionImages
      ? Array.isArray(req.files.completionImages)
        ? req.files.completionImages
        : [req.files.completionImages]
      : [];
    if (completionFiles.length > 0) {
      ...
      if (updateData.completionImagesUrls) {
        try {
          const existingUrls =
            typeof updateData.completionImagesUrls === "string"
              ? JSON.parse(updateData.completionImagesUrls)
              : updateData.completionImagesUrls;
          ...
          if (
            ...
            existingReport.completionImages.length > 0
          ) {
            updateData.completionImages = [
              ...existingReport.completionImages,
              ...completionImagePaths,
    ...
    if (
      ...
      if (updateData.careLabelImageUrls) {
        try {
          const existingUrls =
            typeof updateData.careLabelImageUrls === "string"
              ? JSON.parse(updateData.careLabelImageUrls)
              : updateData.careLabelImageUrls;
          ...
          if (
            existingReport &&
            existingReport.careLabelImage &&
            Array.isArray(existingReport.careLabelImage)
          ) {
            updateData.careLabelImage = [
              ...existingReport.careLabelImage,
              ...careLabelImagePaths,
            ];
          } else {
    ...
    const updatedReport = await ReportModel.findByIdAndUpdate(id, updateOp, {
      new: true,
      runValidators: true,
    });
...
export const deleteReportWashing = async (req, res) => {
  try {
    ...
    const { model: ReportModel, doc: reportToDelete } =
      await findReportById(id);
    ...
    const allImages = [
      ...
      ...(Array.isArray(reportToDelete.careLabelImage)
        ? reportToDelete.careLabelImage
        : [reportToDelete.careLabelImage]),
    ...
    if (allImages.length > 0) {
      for (const imageUrl of allImages) {
        try {
          ...
          console.error(
            `Error deleting file for image ${imageUrl}:`,
            fileError,
          );
        }
      }
  ...
  try {
    ...
    for (const model of models) {
      ...
      const reports = await model
        .find(search ? { color: { $regex: search, $options: "i" } } : {}, {
          color: 1,
        })
        .limit(20)
        .lean();
