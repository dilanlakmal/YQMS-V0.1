...
export const getAssignControl = async (req, res) => {
  try {
    const data = await ReportAssignControl.find({})
      .sort({ updatedAt: -1 })
      .limit(20);
    ...
    res
      .status(500)
      .json({
        message: "Error fetching assignment control data",
        error: error.message,
      });
  }
};
...
export const saveAssignControl = async (req, res) => {
  try {
    ...
    const finalUserWarehouseName = admin
      ? null
      : userWarehouse
        ? userWarehouseName
        : null;
    ...
    const newAssignment = await ReportAssignControl.create({
      ...
      userWarehouse: finalUserWarehouse,
      userWarehouseName: finalUserWarehouseName,
    });

    console.log(
      "[AssignControl] Created successfully with ID:",
      newAssignment._id,
    );
    ...
    res
      .status(500)
      .json({
        message: "Error creating assignment control data",
        error: error.message,
      });
  }
};
...
export const updateAssignControl = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error updating assignment control data",
        error: error.message,
      });
  }
};
...
export const deleteAssignControl = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error deleting assignment control data",
        error: error.message,
      });
  }
};
