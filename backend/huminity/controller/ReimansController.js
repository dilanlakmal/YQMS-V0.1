import { ymProdConnection, ReitmansReport } from "../../controller/MongoDB/dbConnectionController.js";

// Helper to transform Reitmans-specific history
export const transformReitmansHistory = (payload) => {
    const records = Array.isArray(payload.inspectionRecords) ? payload.inspectionRecords : [];
...
export const updateReitmansReport = async (id, payload) => {
    ...
    return ReitmansReport.findByIdAndUpdate(
        id,
        { $set: payload },
        { new: true }
    );
...
export const getReitmansHumidityByMoNo = async (req, res) => {
    try {
        ...
        if (!doc && !rulesDoc) {
            return res.status(404).json({ success: false, message: "No reitmans_humidity record or rules found" });
        }

        console.log(`[ReitmansMatch] Found rulesDoc: ${rulesDoc?._id}, Found style doc: ${doc?._id}`);
        ...
        const skuColors = (order && Array.isArray(order.SKUData)) ? [...new Set(order.SKUData.map(s => s.Color).filter(Boolean))] : [];
        ...
        if (orderFabrics.length > 0) {
            ...
            const targetRules = (doc && doc.ReitmansName && doc.ReitmansName.length > 0) ? doc.ReitmansName : (rulesDoc ? rulesDoc.ReitmansName : []);
            ...
            if (targetRules.length > 0) {
                ...
                if (bestMatch) {
                    ...
                    finalValue = bestMatch.upperCentisimal || bestMatch.upperCentisimalIndex || bestMatch.value;
