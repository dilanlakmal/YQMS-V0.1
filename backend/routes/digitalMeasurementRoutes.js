export default function setupDigitalMeasurementRoutes(app, { UserMain }) {

  // Helper function to apply the same tolerance correction logic as in /api/measurement-details
  const applyToleranceCorrection = (sizeSpec) => {
    return sizeSpec.map((spec) => {
  ...
  app.get("/api/filter-options", async (req, res) => {
    try {
      ...
      const db = UserMain.db; // Assuming UserMain is connected to ym_eco_board

      const factories = await db
        .collection("dt_orders")
        .distinct("Factory", orderFilter);
      const monos = await db
        .collection("dt_orders")
        .distinct("Order_No", orderFilter);
      const custStyles = await db
        .collection("dt_orders")
  ...
  // GET /api/buyer-spec-order-details/:mono
  app.get("/api/buyer-spec-order-details/:mono", async (req, res) => {
    try {
      const db = UserMain.db; // Assuming UserMain is connected to ym_eco_board
      const collection = db.collection("dt_orders");
      ...
      // Apply the same tolerance correction logic
      const buyerSpec = applyToleranceCorrection(order.SizeSpec);
  ...
  app.get("/api/measurement-summary", async (req, res) => {
    try {
      ...
      selectedOrders.forEach((order) => {
        orderIdToSizeSpec[order._id.toString()] = applyToleranceCorrection(order.SizeSpec);
      });
  ...
  // GET /api/measurement-summary-per-mono
  ...
  // GET /api/measurement-details/:mono
  app.get("/api/measurement-details/:mono", async (req, res) => {
    try {
      ...
      const correctedSizeSpec = applyToleranceCorrection(order.SizeSpec);
  ...
  // PUT /api/update-measurement-value
  ...
  // DELETE /api/delete-measurement-record
